...
verify_artifacts() {
    ...
    # ---- Static-link / Termux-prebuilt guard -------------------------------
    # build_talloc() produces libtalloc.a and build_proot() links it with
    # `-ltalloc`, so a correctly built proot carries NO DT_NEEDED for
    # libtalloc.so / libtalloc.so.2. A DT_NEEDED for it means the binary was
    # linked against a shared libtalloc — which happens when someone drops in
    # a prebuilt proot from the Termux package instead of running this script.
    # On a non-Termux app (com.openminis.app) that binary fails at load time:
    #   CANNOT LINK EXECUTABLE ".../libproot.so": library "libtalloc.so.2"
    #   not found: needed by main executable
    # The same Termux prebuilt also DT_NEEDS libandroid-shmem.so and embeds
    # hardcoded paths under /data/data/com.termux/..., so detect any of those
    # markers and fail the build rather than ship a silently unusable APK.
    # Both installed copies (assets + jniLibs) are checked since they must be
    # byte-identical (install_asset() copies one to the other).
    for bin in "$OUTPUT_BIN" "$JNILIBS_BIN"; do
        local bname="${bin##*/}"

        # 1) No DT_NEEDED referencing talloc (the whole point of static link).
        #    `readelf -d` prints lines like: " 0x0000000000000001 (NEEDED) Shared library: [libtalloc.so.2]".
        local needed
        needed="$("$READELF" -d "$bin" 2>/dev/null)"
        if echo "$needed" | grep -qiE 'NEEDED.*talloc'; then
            local bad_lib
            bad_lib="$(echo "$needed" | grep -iE 'NEEDED.*talloc' | sed -E 's/.*\[([^]]*)\].*/\1/' | head -n1)"
            log_warn "BAD LINK: $bname has DT_NEEDED for '$bad_lib' — talloc must be linked statically, not dynamically."
            log_warn "  This binary was NOT built by build_proot.sh; it looks like a Termux prebuilt was committed by mistake."
            log_warn "  Fix: git submodule update --init deps/proot && ./build_proot.sh distclean && ./build_proot.sh"
            failed=1
        fi

        # 2) libandroid-shmem.so is a Termux-only shared lib; a fork-built
        #    proot never DT_NEEDS it (Android shared memory is handled by the
        #    loader, not a runtime dep). Its presence is a second Termux marker.
        if echo "$needed" | grep -qiE 'NEEDED.*libandroid-shmem'; then
            log_warn "BAD LINK: $bname has DT_NEEDED for 'libandroid-shmem.so' — a Termux-only dependency that has no place in a fork-built proot."
            failed=1
        fi

        # 3) Hardcoded Termux loader path. The OpenMinis fork retains a few
        #    termux-origin *string literals* (diagnostic messages like "It
        #    seems that termux-exec is active ..."), so a generic 'com.termux'
        #    grep would false-positive on a legitimate fork build. The real
        #    fingerprint of a Termux *prebuilt* is the hardcoded default loader
        #    extraction path `usr/libexec/proot/loader` burned into .rodata —
        #    a fork-built proot (loader supplied via PROOT_LOADER env, no
        #    embedded/loader-extraction path) never carries it.
        if grep -qa 'usr/libexec/proot/loader' "$bin"; then
            log_warn "TERMUX PREBUILT MARKER: $bname embeds the hardcoded Termux loader path (usr/libexec/proot/loader)."
            log_warn "  A fork-built proot never carries this path — the loader is supplied via PROOT_LOADER env."
            failed=1
        fi
    done
