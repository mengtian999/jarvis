...
class ExecutionCoordinatorInstrumentedTest {
    ...
    fun executeCreatesSessionBindMounts() = runBlocking {
        ...
        assertTrue(PRootKernel.bindMounts.containsKey("/var/jarvis/attachments"))
        assertTrue(PRootKernel.bindMounts.containsKey("/var/jarvis/offloads"))
        assertTrue(PRootKernel.bindMounts.containsKey("/var/jarvis/workspace"))
        assertTrue(PRootKernel.bindMounts.containsKey("/var/jarvis/browser"))

        // Verify global mounts
        assertTrue(PRootKernel.bindMounts.containsKey("/var/jarvis/memory"))
        assertTrue(PRootKernel.bindMounts.containsKey("/var/jarvis/skills"))
    }
    ...
    fun executeCreatesHostDirectories() = runBlocking {
        ...
        val sessionBase = File(context.filesDir, "jarvis-sessions/$sessionId")
    ...
    fun sessionBindMountsPointToCorrectHostDirs() = runBlocking {
        ...
        assertEquals(
            ...
            PRootKernel.bindMounts["/var/jarvis/workspace"]
        )

        val globalBase = File(context.filesDir, "jarvis-global")
        assertEquals(
            File(globalBase, "memory").absolutePath,
            PRootKernel.bindMounts["/var/jarvis/memory"]
        )
    }
    ...
    fun differentSessionsHaveIsolatedWorkspaces() = runBlocking {
        ...
        ExecutionCoordinator.execute(
            "session-iso-A",
            "echo 'from A' > /var/jarvis/workspace/test.txt"
        )

        // Check it exists in session A
        val resultA = ExecutionCoordinator.execute(
            "session-iso-A",
            "cat /var/jarvis/workspace/test.txt"
        )
    ...
    private fun cleanupSessionDirs() {
        listOf("session-mount-test", "session-A", "session-B", "session-X",
            ...
            File(context.filesDir, "jarvis-sessions/$id").deleteRecursively()
        }
        File(context.filesDir, "jarvis-global").deleteRecursively()
    }
}
