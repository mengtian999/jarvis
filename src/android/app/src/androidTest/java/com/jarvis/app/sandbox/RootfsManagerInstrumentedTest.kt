...
class RootfsManagerInstrumentedTest {
    ...
    fun installIfNeededExtractsRootfsFromAssets() = runBlocking {
        ...
        // Verify /var/jarvis subdirs created
        val expectedDirs = listOf("attachments", "offloads", "workspace", "skills", "memory")
        for (subdir in expectedDirs) {
            val dir = File(manager.rootfsDir, "var/jarvis/$subdir")
            assertTrue("var/jarvis/$subdir should exist", dir.exists())
            assertTrue("var/jarvis/$subdir should be directory", dir.isDirectory)
        }
