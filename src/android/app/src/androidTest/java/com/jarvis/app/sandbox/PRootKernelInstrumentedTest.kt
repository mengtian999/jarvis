...
class PRootKernelInstrumentedTest {
    ...
    fun addBindMountStoresMapping() {
        PRootKernel.addBindMount("/var/jarvis/workspace", "/data/host/workspace")
        assertEquals("/data/host/workspace", PRootKernel.bindMounts["/var/jarvis/workspace"])
    }

    @Test
    fun removeBindMountRemovesMapping() {
        PRootKernel.addBindMount("/var/jarvis/workspace", "/data/host/workspace")
        PRootKernel.removeBindMount("/var/jarvis/workspace")
        assertNull(PRootKernel.bindMounts["/var/jarvis/workspace"])
    }
    ...
    fun buildProotCommandIncludesBindMounts() = runBlocking {
        ...
        PRootKernel.addBindMount("/var/jarvis/workspace", "/data/user/0/com.jarvis.app/workspace")

        val cmd = PRootKernel.buildProotCommand("ls /var/jarvis/workspace")

        // Should contain -b host:linux format
        val bindStr = "/data/user/0/com.jarvis.app/workspace:/var/jarvis/workspace"
    ...
    fun resolveHostPathMatchesExactMount() {
        PRootKernel.addBindMount("/var/jarvis/workspace", "/host/workspace")

        val result = PRootKernel.resolveHostPath("/var/jarvis/workspace")
    ...
    fun resolveHostPathMatchesSubpath() {
        ...
        val result = PRootKernel.resolveHostPath("/var/jarvis/workspace/file.txt")
        assertNotNull(result)
    ...
    fun resolveHostPathUsesLongestPrefixMatch() {
        ...
        val result = PRootKernel.resolveHostPath("/var/jarvis/workspace/deep/file.txt")
        assertNotNull(result)
    ...
    fun resolveHostPathDoesNotMatchPartialPrefix() {
        PRootKernel.addBindMount("/var/jarvis/work", "/host/work")

        // "/var/jarvis/workspace" should NOT match "/var/jarvis/work" (no / after "work")
        val result = PRootKernel.resolveHostPath("/var/jarvis/workspace/file.txt")
        // Should fallback to rootfs or null, not match /var/jarvis/work
