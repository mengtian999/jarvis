...
class BackupPackageReaderTest {
    ...
    @Before
    fun setUp() {
        root = File.createTempFile("jarvisbak-reader", "").apply { delete(); mkdirs() }
    }
    ...
    fun `refuses a future format major version instead of guessing`() {
        writeManifest("""{"format":"jarvisbak/2","backup_id":"future"}""")
    ...
    fun `accepts a same-major minor bump`() {
        writeManifest("""{"format":"jarvisbak/1.4","backup_id":"newer-minor"}""")
    ...
    fun `an unsupported encryption scheme is refused`() {
        writeManifest(
            """
            {"format":"jarvisbak/1","encryption":{"scheme":"jarvisbak-enc/99",
    ...
    fun `an edited manifest fails the sidecar MAC`() {
        ...
        writeManifest(body.replace("jarvisbak/1\"", "jarvisbak/1\",\"device_name\":\"forged\""))
    ...
    fun `a stripped encryption block is caught when encrypted members remain`() {
        writeManifest("""{"format":"jarvisbak/1"}""")
        File(root, "data").mkdirs()
