 ...
 *
 * The bug this pins: `jarvis-config set chat.returnKey '"send"'` reported
 * `ok: true` with `old: "newline", new: "send"` — and nothing changed. The
 * field wrote `jarvis_settings/return_key_behavior` while
...
class ChatConfigPrefsBindingTest {
    ...
    private fun source(relative: String): String {
        ...
        val f = File("src/main/java/com/jarvis/app/$relative")
    ...
    fun `chat_returnKey binds to the appearance prefs the UI reads`() {
        ...
        assertTrue(
            "chat.returnKey must use appearancePrefs — writing jarvis_settings " +
                "makes `jarvis-config set` a silent no-op:\n$reg",
    ...
    fun `no builtin writes a UI-owned key into the wrong prefs store`() {
        ...
        // pointed at jarvis_settings, so it silently diverged.
        //
        ...
        assertEquals(
            "these builtins write a UI-owned key into a store the UI never " +
                "reads, so `jarvis-config set` silently no-ops: $offenders",
            emptyList<String>(),
            offenders,
        )
    }
    ...
    fun `the appearance prefs store is resolved from the shared constant`() {
        ...
        assertTrue(
            ...
            builtins.contains("com.jarvis.app.ui.settings.PREF_APPEARANCE"),
        )
    }
}
