...
class BackupFileTreeExporterTest {
    ...
    @Before
    fun setUp() {
        tmp = File.createTempFile("jarvisbak-tree", "").apply { delete(); mkdirs() }
    ...
    fun `skips backup artifacts so packages never nest`() {
        ...
        File(source, "backup-20260816-1200-abc.jarvisbak").writeText("previous package")
        File(source, "Backups").mkdirs()
        File(source, "Backups/backup-older.jarvisbak").writeText("older package")
