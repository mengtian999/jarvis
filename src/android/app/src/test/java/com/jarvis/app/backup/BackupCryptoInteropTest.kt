...
/**
 * Cross-platform crypto vectors for `jarvisbak-enc/1`.
 *
 ...
 */
@Ignore(
    "[T-jarvisbak-rename] Vectors are STALE: they were sealed under the old " +
    "`minisbak/{data,secrets,mac,verify}` HKDF infos and the `MBK1` magic, and " +
    "the .jarvisbak rename moved both (to `jarvisbak/...` + `JBK1`), which " +
    "changes every derived key. Regenerate them by re-running the iOS side of " +
    "this test against the renamed scheme (run BackupCryptoTests.swift on a Mac " +
    "and paste the fresh hex/b64 back here). Faking the constants would defeat " +
    "the test's whole purpose — proving Android reads what iOS wrote.",
)
class BackupCryptoInteropTest {
    ...
    fun `manifest sidecar MAC matches iOS over identical raw bytes`() {
        val k = keys()
        val raw = """{"format":"jarvisbak/1","backup_id":"vector"}""".toByteArray(Charsets.UTF_8)
        ...
        val tampered = """{"format":"jarvisbak/1","backup_id":"forged"}""".toByteArray(Charsets.UTF_8)
        var threw = false
        try {
     ...
     * `JBK1` magic, the big-endian segment length, the nonce‖ciphertext‖tag
    ...
    fun `decrypts a member sealed by iOS`() {
        ...
        assertEquals("hello jarvisbak — 跨平台备份\n", out.toString("UTF-8"))
    }
    ...
    private fun createTempFile() =
        java.io.File.createTempFile("jarvisbak-test", null).apply { deleteOnExit() }

    companion object {
        /**
         * One member as iOS actually wrote it: `JBK1` ‖ big-endian length ‖
