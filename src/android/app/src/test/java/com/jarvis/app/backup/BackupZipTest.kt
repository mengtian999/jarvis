...
class BackupZipTest {
    ...
    @Before
    fun setUp() {
        tmp = File.createTempFile("jarvisbak-zip", "").apply { delete(); mkdirs() }
    }
    ...
    private fun staging(): File = File(tmp, "staging").apply {
        mkdirs()
        File(this, "manifest.json").writeText("""{"format":"jarvisbak/1"}""")
    ...
    fun `round-trips content through archive and extract`() {
        ...
        assertEquals("""{"format":"jarvisbak/1"}""", File(dest, "manifest.json").readText())
    ...
    fun `refuses an archive with more entries than the classic EOCD can count`() {
        ...
        try {
            BackupZip.archive(dir, File(tmp, "too-many.jarvisbak"))
    ...
    fun `finds the package root whether or not iOS wrapped it`() {
        ...
        val inner = File(wrapped, "jarvisbak-1234").apply { mkdirs() }
    ...
    fun `reads a single entry by bare name even when iOS wrapped it`() {
        ...
        ZipOutputStream(wrapped.outputStream()).use { zos ->
            zos.setMethod(ZipOutputStream.STORED)
            val payload = """{"format":"jarvisbak/1"}""".toByteArray()
            val crc = CRC32().apply { update(payload) }
            val e = ZipEntry("jarvisbak-abcd/manifest.json").apply {
                method = ZipEntry.STORED
                size = payload.size.toLong()
                compressedSize = payload.size.toLong()
                this.crc = crc.value
        ...
        assertEquals("""{"format":"jarvisbak/1"}""", bytes?.toString(Charsets.UTF_8))
    }
}
