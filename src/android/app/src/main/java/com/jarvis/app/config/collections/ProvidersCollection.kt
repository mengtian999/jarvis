package com.jarvis.app.config.collections

import com.jarvis.app.config.ConfigCollection
import com.jarvis.app.config.ConfigError
import com.jarvis.app.config.ConfigField
import com.jarvis.app.config.ConfigRisk
import com.jarvis.app.config.ConfigSchema
import com.jarvis.app.config.ConfigValue
import com.jarvis.app.config.fields.ClosureField
import com.jarvis.app.config.fields.HiddenField
 ...
 *
 * [T-jarvis-config-provider-add] Add is OPEN (per user decision: writes
 ...
 *     from a multi-step browser flow that jarvis-config cannot drive.
 */
class ProvidersCollection(
    ...
    override fun fields(forId: String): List<ConfigField> {
        ...
        return listOf(
            ...
            HiddenField(
                ...
                reason = "OAuth tokens cannot be set via jarvis-config — use Settings UI",
            ),
        )
    }
        ...
        val resolved = envVars.getValue(key)
            ?: throw ConfigError.InvalidValue(
                "$fieldName references env var \$\$$key, but no such env var exists. " +
                    "Create it first via [Set $key](jarvis://settings/environments?create_key=$key&create_value=)."
            )
        if (resolved.isEmpty()) {
     ...
     * branded default (Jarvis/<version> (Android <release>; <model>) — see
    ...
    private fun apiKeyField(id: String): ConfigField =
        ClosureField(
            ...
            reader = {
                throw ConfigError.PermissionDenied(
                    "API keys are never read back via jarvis-config. To change one, write a new value."
                )
            },
            writer = { v ->
                val s = (v as? ConfigValue.Str)?.value ?: throw ConfigError.TypeMismatch("string")
