...
import com.jarvis.app.accessibility.MinisAccessibilityService
import com.jarvis.app.accessibility.NodeRegistry
import com.jarvis.app.accessibility.RestrictedSettingsManager
...
First-run: enable "Jarvis" under Settings → Accessibility, then `service ping`.
"""
...
"""
    ...
    override fun handle(request: NativeOffloadRequest): NativeOffloadResult {
                ...
                return err(
                    ...
                    if (restricted) {
                        "Android is blocking the accessibility toggle for this install " +
                            ...
                            "settings, then enable Jarvis under Settings → Accessibility. " +
                        ...
                        "Accessibility permission was revoked (this happens after a force-stop). " +
                            "Re-enable Jarvis under Settings → Accessibility, or use Settings → " +
        ...
        return try {
            ...
            err(args, "SERVICE_NOT_RUNNING",
                e.message ?: "Accessibility service is not running. Enable Jarvis under Settings → Accessibility.",
                exit = 77)
        } catch (e: Throwable) {
        ...
        return when (args.positional.getOrNull(1)) {
            ...
            "ping" -> {
                if (MinisAccessibilityService.getInstance() != null)
                    ...
                    NativeOffloadResult(77, "✗ Accessibility service is not running — go to Settings → Accessibility → Jarvis to enable\n")
            }
    ...
    private fun svcOrThrow(): MinisAccessibilityService =
        MinisAccessibilityService.getInstance()
            ?: throw NotRunning("Accessibility service is not running. Enable Jarvis under Settings → Accessibility.")
