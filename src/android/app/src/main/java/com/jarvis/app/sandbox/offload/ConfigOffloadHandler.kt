...
import com.jarvis.app.config.ConfigBridge
...
/**
 * `jarvis-config` offload handler. Mirrors iOS `ConfigOffload.m`.
 *
...
class ConfigOffloadHandler : NativeOffloadHandler {
    private companion object {
        ...
        const val HELP_TEXT =
            "jarvis-config - read or change Jarvis app settings (logged + revertable)\n" +
                "\n" +
                "USAGE:\n" +
    ...
    private fun cmdSet(args: OffloadArgs): NativeOffloadResult {
        ...
        // [T-android-jarvis-config-set-shell-escape] (issue #36) `--file <path>`
    ...
    /**
     * [T-android-jarvis-config-set-shell-escape] Read a file's text by its
