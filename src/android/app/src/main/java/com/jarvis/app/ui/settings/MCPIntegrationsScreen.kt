...
fun MCPIntegrationsScreen(
    ...
    envVarRepository: com.jarvis.app.data.repository.EnvVarRepository? = null,
) {
    val servers by mcpRepository.servers.collectAsState()

    // [T-android-mcp-list-reload-on-appear] MCPRepository reads servers.json
    // only in init() (app launch). A server the agent writes via jarvis-mcp-cli
...
private fun MCPAddSheet(
    mcpRepository: MCPRepository,
    envVarRepository: com.jarvis.app.data.repository.EnvVarRepository?,
...
private fun MCPFormTab(
    ...
    val appEnvEntries by (envVarRepository?.entries
        ?.collectAsState() ?: remember { mutableStateOf(emptyList<com.jarvis.app.data.repository.EnvVarRepository.EnvVarEntry>()) })
    ...
    var oauthClientSecret by remember {
        mutableStateOf(
            editServer?.id?.let {
                com.jarvis.app.mcp.oauth.MCPOAuthStore.clientSecret(context, it)
            } ?: "",
        )
    }
    ...
    var oauthAuthorized by remember {
        mutableStateOf(
            editServer?.id?.let {
                com.jarvis.app.mcp.oauth.MCPOAuthStore.isAuthorized(context, it)
            } ?: false,
        )
    }
    var oauthBusy by remember { mutableStateOf(false) }

    fun currentOAuthConfig(): com.jarvis.app.mcp.oauth.MCPOAuthConfig? {
        val cfg = com.jarvis.app.mcp.oauth.MCPOAuthConfig(
                    ...
                    if (oauthAuthorized) {
                        MinisTextButton(
                            ...
                            onClick = {
                                val sid = name.trim()
                                if (sid.isNotEmpty()) {
                                    com.jarvis.app.mcp.oauth.MCPOAuthStore.signOut(context, sid)
                        ...
                        onClick = {
                            ...
                            when {
                                ...
                                cfg == null ->
                                    ...
                                    com.jarvis.app.mcp.oauth.MCPOAuthStore.setClientSecret(
                                        context, sid, oauthClientSecret.trim().ifBlank { null },
                                    )
                                    oauthBusy = true
                                    oauthScope.launch {
                                        val result = com.jarvis.app.mcp.oauth.MCPOAuthController(context)
                                            .authorize(sid, url.trim(), cfg)
                                        oauthBusy = false
                                        when (result) {
                                            is com.jarvis.app.mcp.oauth.MCPOAuthController.Result.Success -> {
                                            ...
                                            is com.jarvis.app.mcp.oauth.MCPOAuthController.Result.Cancelled -> {}
                                            is com.jarvis.app.mcp.oauth.MCPOAuthController.Result.Failed -> {
