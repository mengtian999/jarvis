...
/**
 * Manages the memory directory (`jarvis-global/memory/`).
 * Mirrors iOS memory system:
 *   - GLOBAL.md: read-only for agent, user-maintained via Settings, SHARED across roles
 *   - YYYY-MM-DD.md: daily logs with timestamped entries, agent writes via memory_write,
 *     scoped PER ROLE under `roles/<roleId>/memory/` so different personas don't see
 *     each other's notes (role isolation,方案 B). GLOBAL.md stays shared.
 *   - memory_get: fuzzy keyword search across the active role's files (+ GLOBAL.md)
 *   - loadGlobalMemoryFragment() / loadRecentDailyMemoryFragment(roleId): emit
 *     two separate text blocks for the system prompt (mirrors iOS exactly)
 *
 * [T-role-memory-isolation] Daily-log paths are resolved by `roleMemoryDir(roleId)`,
 * which sits UNDER the role directory (`roles/<roleId>/memory/`). Because
 * `RoleStore.deleteRole` wipes the whole role dir with `deleteRecursively()`,
 * deleting a role also wipes its memory — matching the "isolate到底" contract.
 * `loadGlobalMemoryFragment` / `loadGlobalMd` / `saveGlobalMd` still read/write
 * the shared `memoryDir/GLOBAL.md` and are NOT role-scoped.
 */
class MemoryRepository(private val memoryDir: File) {
    ...
    companion object {
        ...
        // [T-role-memory-isolation] Default role id, mirrors RoleStore.DEFAULT_ROLE_ID.
        // Used as the fallback when a caller passes a null/blank roleId (legacy /
        // abnormal paths) and as the migration target for pre-isolation daily logs.
        private const val DEFAULT_ROLE_ID = "default"
        private const val ROLES_SUBDIR = "roles"
        private const val ROLE_MEMORY_SUBDIR = "memory"
        // Idempotency marker for the one-shot legacy → default-role migration.
        private const val LEGACY_MIGRATION_MARKER = ".legacy-memory-migrated"
        // [T-role-memory-isolation] A daily log filename is YYYY-MM-DD.md.
        // Only these files are role-scoped; everything else (GLOBAL.md,
        // SOUL.md, …) stays in the shared global memoryDir.
        private val DAILY_LOG_REGEX = Regex("""\d{4}-\d{2}-\d{2}\.md""")
    ...
    init {
        memoryDir.mkdirs()
        // [T-role-memory-isolation] One-shot migration of pre-isolation daily
        // logs (lived directly under memoryDir) into the default role's
        // per-role memory dir, so existing Jarvis notes aren't lost when the
        // daily-log path moves to roles/<roleId>/memory/. Idempotent via marker.
        migrateLegacyDailyLogs()
    }

    // [T-role-memory-isolation] ─────────────────────────────────────────────
    // Per-role daily-log directory: <memoryDir>/roles/<roleId>/memory/.
    // Sits under the role dir so RoleStore.deleteRole's deleteRecursively()
    // wipes a role's memory along with its persona — full isolation.
    // A null/blank roleId falls back to the default role ("default" = Jarvis).
    private fun effectiveRoleId(roleId: String?): String =
        roleId?.takeIf { it.isNotBlank() } ?: DEFAULT_ROLE_ID

    private fun roleMemoryDir(roleId: String?): File {
        val rid = effectiveRoleId(roleId)
        val dir = File(File(memoryDir, ROLES_SUBDIR).let { File(it, rid) }, ROLE_MEMORY_SUBDIR)
        if (!dir.exists()) dir.mkdirs()
        return dir
    }

    /**
     * Move any `YYYY-MM-DD.md` (and any non-GLOBAL .md) that still lives
     * directly under [memoryDir] — the pre-isolation layout — into the
     * default role's per-role memory dir. Runs once per install (guarded by
     * [LEGACY_MIGRATION_MARKER]). Safe to no-op when there's nothing to move.
     */
    private fun migrateLegacyDailyLogs() {
        val marker = File(memoryDir, LEGACY_MIGRATION_MARKER)
        if (marker.exists()) return
        try {
            val legacy = memoryDir.listFiles()
                ?.filter { it.isFile && it.extension == "md" && it.name != GLOBAL_FILE }
                ?: emptyList()
            if (legacy.isNotEmpty()) {
                val target = roleMemoryDir(DEFAULT_ROLE_ID)
                for (f in legacy) {
                    val dest = File(target, f.name)
                    if (!dest.exists()) f.copyTo(dest, overwrite = false)
                    f.delete()
                }
                Log.i(TAG, "Migrated ${legacy.size} legacy daily log(s) → roles/$DEFAULT_ROLE_ID/$ROLE_MEMORY_SUBDIR")
            }
            marker.writeText("1")
        } catch (e: Exception) {
            Log.w(TAG, "Legacy memory migration skipped: ${e.message}")
        }
    }
    // ──────────────────────────────────────────────────────────────────────
    ...
    /**
     * Fuzzy keyword search across the active role's memory files.
     * @param keywords space-separated, case-insensitive, ALL must match
     * @param scope "daily" (the role's logs only) or "all" (logs + GLOBAL.md)
     * @param roleId the role whose daily logs to search; GLOBAL.md is always
     *  global/shared regardless of role. Null/blank → default role.
     * @return search results with context lines
     */
    fun getMemory(keywords: String, scope: String, roleId: String? = null): String {
        ...
        // Daily logs for THIS role only (sorted descending). Other roles'
        // logs live in different roleMemoryDir siblings and are intentionally
        // invisible — that's the whole point of role memory isolation.
        val dailyFiles = roleMemoryDir(roleId).listFiles()
            ?.filter { isDailyLogFile(it.name) }
     ...
     * for [roleId] from its per-role memory dir, for system-prompt injection.
     * Mirrors iOS `AIChatViewModel.loadRecentDailyMemoryFragment()` exactly:
     * same header, same intro paragraph, same per-entry labels, same 200-line
     * cap, same "(N more lines, use memory_get to search)" continuation.
     */
    fun loadRecentDailyMemoryFragment(roleId: String? = null): String? {
        val dateFmt = SimpleDateFormat("yyyy-MM-dd", Locale.US)
        val now = Date()
        val dir = roleMemoryDir(roleId)
    ...
    /**
     * List all memory files for [roleId]: GLOBAL.md (shared, first), then any
     * shared non-date .md living at the top level (e.g. SOUL.md — identity,
     * NOT role-scoped), then the role's daily logs descending.
     * [roleId] only scopes date-pattern daily logs; GLOBAL.md / SOUL.md are
     * shared global files and always shown regardless of role.
     */
    fun listAllFiles(roleId: String? = null): List<MemoryFileInfo> {
        val dateFmt = SimpleDateFormat("yyyy-MM-dd HH:mm", Locale.US)
        val items = mutableListOf<MemoryFileInfo>()

        // GLOBAL.md always first (shared across roles)
        val globalFile = File(memoryDir, GLOBAL_FILE)
        ...
        // Shared non-date .md at the top level (SOUL.md etc.) — identity /
        // global files that pre-date role isolation and stay global. Excludes
        // GLOBAL.md (above) and date-pattern daily logs (now per-role below).
        val sharedExtras = memoryDir.listFiles()
            ?.filter { it.isFile && it.extension == "md" && it.name != GLOBAL_FILE && !isDailyLogFile(it.name) }
            ?.sortedByDescending { it.name }
            ?: emptyList()
        for (file in sharedExtras) {
            val content = try { file.readText() } catch (_: Exception) { "" }
            items.add(MemoryFileInfo(
                name = file.name,
                isGlobal = false,
        ...
        // Daily logs for THIS role only (date-pattern), sorted descending
        val dailyFiles = roleMemoryDir(roleId).listFiles()
            ?.filter { it.isFile && isDailyLogFile(it.name) }
            ?.sortedByDescending { it.name }
            ?: emptyList()
    ...
    // [T-role-memory-isolation] Only date-pattern daily logs (YYYY-MM-DD.md)
    // are role-scoped. GLOBAL.md, SOUL.md, and any other file stay in the
    // shared global memoryDir — they are identity / shared instruction files
    // that pre-date role isolation and must remain visible to all roles. This
    // precisely implements方案 B: per-role episodic logs, shared global state.
    private fun isDailyLogFile(name: String): Boolean =
        DAILY_LOG_REGEX.matches(name)

    private fun resolveFile(name: String, roleId: String?): File =
        if (isDailyLogFile(name)) File(roleMemoryDir(roleId), name)
        else File(memoryDir, name)

    fun readFile(name: String, roleId: String? = null): String {
        val file = resolveFile(name, roleId)
        return if (file.exists()) try { file.readText() } catch (_: Exception) { "" } else ""
    }

    fun saveFile(name: String, content: String, roleId: String? = null) {
        resolveFile(name, roleId).writeText(content)
    }

    fun deleteFile(name: String, roleId: String? = null): Boolean {
        if (name == GLOBAL_FILE) return false // Cannot delete GLOBAL.md
        return resolveFile(name, roleId).delete()
    }
     ...
     */
    fun revokeEntry(writtenContent: String, roleId: String? = null): EntryMutationResult {
     ...
     */
    fun replaceEntryBody(oldContent: String, newContent: String, roleId: String? = null): EntryMutationResult {
