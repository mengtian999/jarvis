...
/**
 * Passphrase-based encryption for `.jarvisbak` packages
 * (docs/backup-restore-design.md §5, scheme `jarvisbak-enc/1`).
 *
 ...
 *     block) with infos `jarvisbak/{data,secrets,mac,verify}`.
...
object BackupCrypto {
    ...
    const val SCHEME = "jarvisbak-enc/1"
    ...
    /** `JBK1` — identifies an encrypted member before any JSON parse attempt. */
    val MAGIC = byteArrayOf(0x4A, 0x42, 0x4B, 0x31)
    ...
    class Keys internal constructor(kek: ByteArray) {
        ...
        val dataKey: ByteArray = hkdfSha256(kek, "jarvisbak/data")
        /**
         ...
         */
        val secretsKey: ByteArray = hkdfSha256(kek, "jarvisbak/secrets")
        /** Authenticates manifest.json. */
        val macKey: ByteArray = hkdfSha256(kek, "jarvisbak/mac")
        /** Answers "is this passphrase right?" without touching any payload. */
        val verifierKey: ByteArray = hkdfSha256(kek, "jarvisbak/verify")
        ...
        /**
         * `HMAC(verifier_key, "jarvisbak-v1")` truncated to 16 bytes, base64
        ...
        val verifier: String
            get() {
                val mac = hmacSha256(verifierKey, "jarvisbak-v1".toByteArray(Charsets.UTF_8))
     ...
     *
     * Layout: `JBK1` then, per segment, `UInt32 big-endian length` followed by
    ...
    /** Decrypt a member whose `JBK1` magic has already been consumed. */
