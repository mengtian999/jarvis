...
import com.jarvis.app.scheduled.ScheduledRepeatMode
import com.jarvis.app.scheduled.ScheduledTargetMode
import com.jarvis.app.scheduled.ScheduledTask
import com.jarvis.app.scheduled.ScheduledTaskManager
...
/**
 * [T-android-scheduled-tasks-full] jarvis-scheduled — CLI surface for the
 ...
 *
 *   jarvis-scheduled list
 *   jarvis-scheduled create --label L --time HH:MM --prompt "..."
 ...
 *                          [--disabled]
 *   jarvis-scheduled delete --id <taskId>
 *   jarvis-scheduled enable  --id <taskId>
 *   jarvis-scheduled disable --id <taskId>
 *   jarvis-scheduled run     --id <taskId>     (fire immediately, off-schedule)
 *
...
class ScheduledTaskOffloadHandler(private val context: Context) : NativeOffloadHandler {
    ...
    override fun handle(request: NativeOffloadRequest): NativeOffloadResult {
        ...
        return try {
            when (val sub = args.positional.firstOrNull() ?: "list") {
                ...
                "run" -> handleRun(args)
                else -> NativeOffloadResult(2, "jarvis-scheduled: unknown subcommand '$sub'\n$HELP")
            }
        } catch (e: IllegalArgumentException) {
            NativeOffloadResult(2, "jarvis-scheduled: ${e.message}")
        } catch (e: Throwable) {
            AppLogger.warning(TAG, "handle failed: ${e.message}")
            NativeOffloadResult(1, "jarvis-scheduled: ${e.message}")
        }
    }
    ...
    private fun handleDelete(args: OffloadArgs): NativeOffloadResult {
        ...
        if (manager.get(id) == null) return NativeOffloadResult(1, "jarvis-scheduled: no task with id=$id")
        manager.delete(id)
    ...
    private fun handleRun(args: OffloadArgs): NativeOffloadResult {
        val id = args.get("id") ?: throw IllegalArgumentException("--id required")
        val task = manager.get(id) ?: return NativeOffloadResult(1, "jarvis-scheduled: no task with id=$id")
        ...
        val sessionId = runBlocking {
            com.jarvis.app.scheduled.ScheduledAgentRunner.run(context, task)
        }
        val out = JSONObject()
            .put("id", id)
    ...
    companion object {
        ...
        private val HELP = """
            jarvis-scheduled — manage timed AI tasks (mirrors the in-app Scheduled Tasks).
