...
import com.jarvis.app.deeplink.DeepLinkAction
import com.jarvis.app.deeplink.DeepLinkCoordinator
import com.jarvis.app.ui.settings.KEY_LAUNCH_SESSION
import com.jarvis.app.ui.settings.getAppearancePrefs
...
import com.jarvis.app.ui.sessions.SessionListScreen
import com.jarvis.app.ui.settings.AboutScreen
import com.jarvis.app.ui.settings.AddAgentLoopGroupsScreen
import com.jarvis.app.ui.settings.AddAgentLoopModelsScreen
import com.jarvis.app.ui.settings.AddCustomModelScreen
import com.jarvis.app.ui.settings.BackgroundSettingsScreen
import com.jarvis.app.ui.settings.AddModelsToGroupScreen
import com.jarvis.app.ui.settings.ShadowVoiceDetailScreen
import com.jarvis.app.ui.settings.AddProviderScreen
import com.jarvis.app.ui.settings.ModelEntryDetailScreen
import com.jarvis.app.ui.settings.ModelGroupDetailScreen
import com.jarvis.app.ui.settings.ModelGroupsScreen
import com.jarvis.app.ui.settings.ProviderDetailScreen
import com.jarvis.app.ui.settings.ProviderListScreen
import com.jarvis.app.ui.sandbox.FileBrowserScreen
import com.jarvis.app.ui.sandbox.FileBrowserViewModel
import com.jarvis.app.ui.sandbox.FileItem
import com.jarvis.app.ui.sandbox.FilePreviewScreen
import com.jarvis.app.ui.sandbox.RootfsManagementScreen
import com.jarvis.app.ui.settings.EnvironmentVariablesScreen
import com.jarvis.app.ui.settings.AppearanceScreen
import com.jarvis.app.ui.settings.SettingsScreen
import com.jarvis.app.ui.settings.SystemPermissionsScreen
import com.jarvis.app.ui.settings.SessionStorageDetailScreen
import com.jarvis.app.ui.settings.SkillDetailScreen
import com.jarvis.app.ui.settings.StorageManagementScreen
import com.jarvis.app.ui.settings.SkillFileViewerScreen
import com.jarvis.app.ui.settings.UsageStatsScreen
import com.jarvis.app.ui.settings.MinisSkillsBrowserScreen
import com.jarvis.app.ui.settings.MountDetailScreen
import com.jarvis.app.ui.settings.MountedFoldersScreen
import com.jarvis.app.ui.settings.SharedFolderDetailScreen
import com.jarvis.app.ui.settings.SharedFoldersScreen
import com.jarvis.app.ui.settings.SkillsManagementScreen
import com.jarvis.app.data.repository.EnvVarRepository
import com.jarvis.app.data.repository.MemoryRepository
import com.jarvis.app.data.repository.SkillRepository
import com.jarvis.app.ui.settings.LogDetailScreen
import com.jarvis.app.ui.settings.LogManagementScreen
import com.jarvis.app.ui.settings.MemoryFileEditScreen
import com.jarvis.app.ui.settings.MemoryManagementScreen
import com.jarvis.app.ui.settings.OffloadPermissionScreen
import com.jarvis.app.ui.settings.ShizukuPermissionScreen
import com.jarvis.app.sandbox.RootfsManager
import com.jarvis.app.sandbox.TerminalSession
import com.jarvis.app.ui.terminal.TerminalScreen
import com.jarvis.app.ui.onboarding.OnboardingModelSelectionScreen
...
object Routes {
    ...
    const val JARVIS_SKILLS_BROWSER = "jarvis_skills_browser"
    ...
    /** Chat-files browser: opens FileBrowser rooted at /var/jarvis for the session. */
    ...
    const val SOUL = "soul"
    /** [T-role-multi] Multi-role settings page. */
    const val ROLE = "role"
    const val MEMORY_FILE_EDIT = "memory_file/{fileName}/{isGlobal}/{roleId}"
    ...
    fun memoryFileEdit(fileName: String, isGlobal: Boolean, roleId: String?) =
        "memory_file/$fileName/$isGlobal/${roleId ?: "default"}"
...
fun AppNavigation(
    ...
    val mountedFoldersStore = remember {
        (context.applicationContext as com.jarvis.app.MinisApp).mountedFoldersStore
    }
    ...
    // STARTED state when we'd otherwise call navigate(). safeNavigate() �?    // designed to defang the back-then-tap race �?early-returns whenever
    ...
    LaunchedEffect(Unit) {
        ...
        val hasPendingShare =
            com.jarvis.app.share.ShareCoordinator.bufferVersion.value > 0
        ...
        // �? main-thread hangs, force mode = 3 (home) so we don't reopen
        ...
        val mode = if (
            com.jarvis.app.diagnostics.HangDetector.shouldForceHomeOnLaunch(context) ||
            com.jarvis.app.crash.CrashFrequencyDetector.shouldForceHomeOnLaunch(context) ||
            com.jarvis.app.diagnostics.LaunchCycleBeacon.shouldForceHomeOnLaunch()
        ) 3 else rawMode
        ...
        val target: String? = when {
            ...
            // ChatScreen consumer never mounts and the buffer expires �?            // exactly the symptom from the bug report. Force a new draft
    ...
    // Pinned-shortcut cold start: when launched via
     // `jarvis://session/<id>/<resource-path>`, set the pending HTML
    ...
    // its first LaunchedEffect tick. Mirrors the htmlShortcut path �?    // avoids a sessions-list flash and a duplicate back-stack entry.
    ...
    NavHost(
        ...
        composable(Routes.SETTINGS) {
            SettingsScreen(
                ...
                onRoleClick = { navController.safeNavigate(Routes.ROLE) },
        ...
        composable(Routes.BACKUP) {
            com.jarvis.app.ui.settings.backup.BackupAndRestoreScreen(
                onBack = { navController.safePopBackStack() },
        ...
        composable(
            ...
            val history = remember { com.jarvis.app.backup.BackupHistory.get(ctx) }
            ...
            if (record == null) {
                ...
                // the screen.
                val vm: com.jarvis.app.ui.settings.backup.BackupViewModel =
                    androidx.lifecycle.viewmodel.compose.viewModel()
                com.jarvis.app.ui.settings.backup.BackupHistoryDetailScreen(
                    record = record,
                    onBack = { navController.safePopBackStack() },
                    onRemove = {
                        history.remove(id)
        ...
        composable(
            ...
            val remote = remember(name) {
                com.jarvis.app.backup.remote.RcloneRemoteStore(ctx).remote(name)
            }
            if (remote == null) {
                ...
                val vm: com.jarvis.app.ui.settings.backup.BackupViewModel =
                    androidx.lifecycle.viewmodel.compose.viewModel()
                com.jarvis.app.ui.settings.backup.BackupDestinationBrowseScreen(
                    remote = remote,
                    vm = vm,
                    onBack = { navController.safePopBackStack() },
                )
        ...
        composable(Routes.RESTORE_SERVERS) {
            com.jarvis.app.ui.settings.backup.RestoreServersScreen(
                onBack = { navController.safePopBackStack() },
                onPickServer = { name ->
                    navController.safeNavigate(
                        "${Routes.RESTORE_BROWSE}/" + android.net.Uri.encode(name),
        ...
        composable(
            ...
            if (remote == null) {
                ...
                com.jarvis.app.ui.settings.backup.RestoreBrowseScreen(
                    remote = remote,
                    vm = vm,
                    onBack = { navController.safePopBackStack() },
        ...
        composable(Routes.BACKUP_DESTINATIONS) {
            com.jarvis.app.ui.settings.backup.RcloneDestinationsScreen(
                onBack = { navController.safePopBackStack() },
            )
        }
        ...
        composable(
            ...
            SharedFolderDetailScreen(
                ...
                onBrowseFiles = {
                    ...
                    val hostPath = java.io.File(rootfs.rootfsDir, "var/jarvis/$folderId")
                    val label = when (folderId) {
                        "shared" -> ctx.getString(com.jarvis.app.R.string.shared_folder_name_shared)
                        "skills" -> ctx.getString(com.jarvis.app.R.string.shared_folder_name_skills)
                        "memory" -> ctx.getString(com.jarvis.app.R.string.shared_folder_name_memory)
                        else -> folderId
                    }
                    FilePreviewHolder.fileBrowserViewModel = FileBrowserViewModel(
                        ...
                        // dirs that back /var/jarvis/{shared,skills,memory}
                        // resolve, matching how chat-files browse works.
                        linuxRootPath = "/var/jarvis/$folderId",
        ...
        composable(
            ...
            SessionStorageDetailScreen(
                ...
                onBrowseFiles = { rootPath ->
                    ...
                    // the per-session host dir (filesDir/jarvis-sessions/<sid>),
                    // whose immediate children (workspace/ attachments/ offloads/
                    // browser/) are exactly the PRoot /var/jarvis/* subdirs. The
                    // host listing already resolves correctly so we keep rootPath
                    // host-based (no linuxRootPath re-routing �?that would redirect
                    // /var/jarvis to the global/empty placeholder dir). We only pass
                    // displayLinuxPrefix = "/var/jarvis" so "Copy Absolute Path"
                    // emits the agent-visible /var/jarvis/workspace/foo.py instead of
                    // the opaque /data/user/0/.../jarvis-sessions/<sid>/... host path.
                    FilePreviewHolder.fileBrowserViewModel = FileBrowserViewModel(
                        rootPath = java.io.File(rootPath),
                        rootLabel = "Session Files",
                        displayLinuxPrefix = "/var/jarvis",
                    )
                    navController.safeNavigate(Routes.FILE_BROWSER)
                },
            )
        ...
        composable(
            ...
            val category = com.jarvis.app.ui.sandbox.MirrorCatalog.categoryFromKey(categoryKey)
                ?: return@composable
            com.jarvis.app.ui.sandbox.MirrorCategoryDetailScreen(
                category = category,
                onBack = { navController.safePopBackStack() },
            )
        }
        ...
        // Linux root, focused on /var/jarvis. Matches AIChatView.swift L490:
        //   FileBrowserView(rootPath: dataPath, initialPath: dataPath/var/jarvis,
        //                   rootLabel: "/")
        // so the user can navigate up out of /var/jarvis into the broader rootfs.
        composable(
            ...
            val context = androidx.compose.ui.platform.LocalContext.current
            val rootfs = RootfsManager.getInstance(context.applicationContext)
            val varJarvis = java.io.File(rootfs.rootfsDir, "var/jarvis")
            val sessionId = backStackEntry.arguments?.getString("sessionId") ?: return@composable
            val vm = remember(rootfs.rootfsDir.absolutePath, varJarvis.absolutePath, sessionId) {
                FileBrowserViewModel(
                    rootPath = rootfs.rootfsDir,
                    initialPath = varJarvis.takeIf { it.exists() },
                    rootLabel = "/",
                    // T121: route directory listings through PRootKernel bind
                    // mounts so /var/jarvis/{skills,memory,shared} resolve to
                    // their backing host dirs (filesDir/jarvis-global/<subdir>).
                    // Without this the browser walks the rootfs tarball
                    // directly and shows the empty placeholder dirs that ship
                    // inside Alpine's var/jarvis/ — every subdir reads as
        ...
        composable(Routes.SKILLS) {
            if (skillRepository != null) {
                SkillsManagementScreen(
                    ...
                    onMinisSkillsClick = { navController.safeNavigate(Routes.JARVIS_SKILLS_BROWSER) },
                )
            }
        }
        ...
        composable(Routes.JARVIS_SKILLS_BROWSER) {
            if (skillRepository != null) {
        ...
        composable(Routes.MEMORY) {
            if (memoryRepository != null) {
                MemoryManagementScreen(
                    memoryRepository = memoryRepository,
                    onBack = { navController.safePopBackStack() },
                    onFileClick = { fileName, isGlobal, roleId ->
                        navController.safeNavigate(Routes.memoryFileEdit(fileName, isGlobal, roleId))
                    },
                )
            }
        }
        ...
        composable(Routes.MCP) {
            if (mcpRepository != null) {
                com.jarvis.app.ui.settings.MCPIntegrationsScreen(
                    mcpRepository = mcpRepository,
                    onBack = { navController.safePopBackStack() },
                    envVarRepository = envVarRepository,
                )
        ...
        composable(Routes.SOUL) {
            com.jarvis.app.ui.settings.SoulSettingsScreen(
                onBack = { navController.safePopBackStack() },
            )
        }
        ...
        composable(Routes.ROLE) {
            com.jarvis.app.ui.settings.RoleSettingsScreen(
                onBack = { navController.safePopBackStack() },
            )
        }
        ...
        composable(
            ...
            arguments = listOf(
                ...
                navArgument("roleId") { type = NavType.StringType; defaultValue = "default" },
            ),
        ) { backStackEntry ->
        ...
        composable(Routes.SCHEDULED_TASKS) {
            com.jarvis.app.ui.scheduled.ScheduledTasksScreen(
                onBack = { navController.safePopBackStack() },
                onEditTask = { taskId ->
        ...
        composable(
            ...
            com.jarvis.app.ui.scheduled.ScheduledTaskRunsScreen(
                taskId = taskId,
                onBack = { navController.safePopBackStack() },
                onOpenSession = { sessionId ->
                    navController.safeNavigate(Routes.chat(sessionId))
        ...
        composable(
            ...
            com.jarvis.app.ui.scheduled.ScheduledTaskEditScreen(
                taskId = taskId,
                onBack = { navController.safePopBackStack() },
                onOpenSession = { sessionId ->
                    navController.safeNavigate(Routes.chat(sessionId))
