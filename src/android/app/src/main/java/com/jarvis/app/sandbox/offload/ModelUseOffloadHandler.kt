package com.jarvis.app.sandbox.offload
...
import com.jarvis.app.provider.safeOptString
import com.jarvis.app.sandbox.NativeOffloadHandler
import com.jarvis.app.sandbox.NativeOffloadRequest
...
/**
 * `jarvis-model-use` — list, search, and invoke LLM models from Alpine shell.
 * Mirrors iOS ModelUseOffload.m + ModelUseOffloadBridge.swift.
 *
 * Subcommands:
 *   jarvis-model-use list [--provider <name>] [--modality <m>]
 *   jarvis-model-use search <query> [--provider <name>] [--modality <m>]
 *   jarvis-model-use run --model <id_or_name> [--input <path>] [--output <path>]
...
class ModelUseOffloadHandler(
    ...
    override fun handle(request: NativeOffloadRequest): NativeOffloadResult {
        ...
        return try {
            when (val sub = args.positional[0]) {
                ...
                else -> NativeOffloadResult(2, "jarvis-model-use: unknown subcommand '$sub'\n$HELP")
            }
        } catch (e: Throwable) {
    ...
    private fun cmdSearch(args: OffloadArgs): NativeOffloadResult {
        val query = args.positional.getOrNull(1)
            ?: return NativeOffloadResult(
                2,
                "jarvis-model-use search: no query. Usage: jarvis-model-use search <query>\n",
            )
    ...
    private fun cmdRun(args: OffloadArgs, request: NativeOffloadRequest): NativeOffloadResult {
        val modelArg = args.get("model")
            ?: return NativeOffloadResult(2, "--model is required. Usage: jarvis-model-use run --model <id_or_name>\n")
            ...
            ?: return NativeOffloadResult(
                ...
                JSONObject().put("error", "model_not_found")
                    .put(
                        "message",
                        if (providerFilter != null)
                            "No model '$modelArg' under provider '$providerFilter'. Use 'jarvis-model-use list' to see available combinations."
                        else
        ...
        if (outputPath != null && !outputPath.startsWith("/") && !outputPath.startsWith("file://")) {
            return NativeOffloadResult(
                ...
                JSONObject().put("error", "invalid_output_path")
                    .put(
                        "message",
                        "--output must be an absolute path (e.g. /var/jarvis/workspace/out.jpg). " +
                            "Got the relative path '$outputPath', which cannot be resolved because " +
                            "jarvis-model-use does not inherit the shell's working directory.",
                    )
                    .toString() + "\n",
            )
        }
        ...
        if (requiredModality != null) {
            ...
            return NativeOffloadResult(
                ...
                JSONObject().put("error", "modality_not_supported")
                    .put("message", "Model '${entry.model.displayName}' does not support $requiredModality. Supported modalities: ${supported.joinToString(", ")}. Use 'jarvis-model-use list' to find a model with the required capability.")
                    .toString() + "\n",
            )
        }
        ...
        val inputText = when {
            args.get("input") != null -> readLinuxPath(args.get("input")!!)
                ?: return NativeOffloadResult(
                    2,
                    "jarvis-model-use run: cannot read --input '${args.get("input")}'\n",
                )
            else -> ""
        }
        val parsed = try {
                ...
                JSONObject().put("error", "modality_not_supported")
                    .put(
                        ...
                        "Model '${entry.model.displayName}' does not support audio_input, " +
                            ...
                            "Use 'jarvis-model-use list --modality audio' to find an audio-capable model.",
                    )
                    .toString() + "\n",
            )
        }
        ...
        val instance = providerRepository.instance(entry.providerInstanceId)
            ?: return NativeOffloadResult(2, "jarvis-model-use run: provider instance not found\n")
        ...
        if (hasAudioInput && provider !is com.jarvis.app.provider.openai.OpenAIProvider) {
            return NativeOffloadResult(
                2,
        ...
        (provider as? com.jarvis.app.provider.openai.OpenAIProvider)?.let { openAI ->
        ...
        if (outputPath != null) {
            ...
            // sessionId is null or the path isn't a session-scoped /var/jarvis
            ...
            val attachDir = sessionScopedHostFile("/var/jarvis/attachments", sessionId)
                ?: PRootKernel.resolveHostPath("/var/jarvis/attachments")
            if (attachDir != null) {
                attachDir.mkdirs()
                for ((idx, media) in response.mediaAttachments.withIndex()) {
                    ...
                    val responsePath = "/var/jarvis/attachments/$fileName"
    ...
    private data class ImageGenConfig(
        ...
        val endpointOverride: com.jarvis.app.data.model.ImageEndpointMode? = null,
    )
    ...
    private fun parseImageGenConfig(inputJson: String): ImageGenConfig {
        ...
        val endpoint = when (rawEndpoint.lowercase()) {
            "auto" -> com.jarvis.app.data.model.ImageEndpointMode.auto
            "images_generations", "images-generations", "images-gen" ->
                com.jarvis.app.data.model.ImageEndpointMode.imagesGenerations
            "chat_completions", "chat-completions", "chat" ->
                com.jarvis.app.data.model.ImageEndpointMode.chatCompletions
            else -> null
        }
    ...
    private fun performRawPassthrough(
        ...
        entry: ModelEntry,
        openAI: com.jarvis.app.provider.openai.OpenAIProvider,
        systemPrompt: String?,
    ...
    private fun tryImageGenerationRoute(
        ...
        val effectiveMode = when {
            ...
            isPureImageGenerator -> com.jarvis.app.data.model.ImageEndpointMode.imagesGenerations
            instance.imageEndpointMode == com.jarvis.app.data.model.ImageEndpointMode.auto &&
        ...
        if (effectiveMode == com.jarvis.app.data.model.ImageEndpointMode.chatCompletions) return null
        ...
        if (effectiveMode == com.jarvis.app.data.model.ImageEndpointMode.imagesGenerations) {
            val response = try {
                runBlocking {
        ...
        val response = try {
            runBlocking {
                ...
                providerRepository.setImageEndpointResolved(
                    instance.id, com.jarvis.app.data.model.ImageEndpointMode.imagesGenerations,
                )
            }
        } catch (e: Throwable) {
            val msg = e.message ?: ""
            ...
            if (routeMissing) {
                ...
                providerRepository.setImageEndpointResolved(
                    instance.id, com.jarvis.app.data.model.ImageEndpointMode.chatCompletions,
                )
    ...
    private fun writeImageResult(
        entry: ModelEntry,
        response: com.jarvis.app.data.model.LLMResponse,
        outputPath: String?,
        outputExt: String,
        endpointUsed: String,
        sessionId: String?,
    ...
    /**
     * [T-android-model-use-session-scoped-write] Resolve a `/var/jarvis/<sub>/...`
     ...
     * build, so PRootKernel.resolveHostPath("/var/jarvis/attachments") returns
     ...
     * response reports the abstract `/var/jarvis/attachments/...` path, which A's
     ...
     * buildSessionBindMounts). For those, host dir = filesDir/jarvis-sessions/
     ...
     * isn't a session-scoped `/var/jarvis/<sub>` path.
     */
    private fun sessionScopedHostFile(linuxPath: String, sessionId: String?): File? {
        if (sessionId == null) return null
        val m = Regex("^/var/jarvis/(attachments|offloads|workspace|browser)(/.*)?$").find(linuxPath)
            ?: return null
        val sub = m.groupValues[1]
        val rest = m.groupValues[2].removePrefix("/")
        val base = File(context.filesDir, "jarvis-sessions/$sessionId/$sub")
    ...
    private fun logModelUseWrite(responsePath: String, hostFile: File, sessionId: String?) {
        Log.i(
            ...
            "[ModelUseWrite] path=$responsePath hostFile=${hostFile.absolutePath} " +
                "sessionId=$sessionId globalAttachMount=${PRootKernel.bindMounts["/var/jarvis/attachments"]}",
        )
    }
    ...
    private fun entryDict(entry: ModelEntry): JSONObject =
        com.jarvis.app.offload.ModelUseManager.entryDict(
            entry,
     ...
     *  - `/var/jarvis/<scope>/<path>` or `/<abs/linux/path>` → via
     *    [PRootKernel.resolveHostPath] (which already handles
     *    `/var/jarvis/` bind mounts longest-prefix)
    ...
    private fun resolveImageUrl(url: String): LLMMessage.ImagePart {
        ...
        if (url.startsWith("http://") || url.startsWith("https://")) {
            throw ImageInputError(
                "http(s):// image URLs are not supported by jarvis-model-use. " +
                    "Download first with `shell_execute` (curl/wget) into " +
                    "/var/jarvis/workspace/, then reference the local path."
            )
        }

        // file:///abs/path → strip prefix, treat as host path; fall back
        // to resolveHostPath so bind-mounted /var/jarvis/* still works
        // when callers write file:///var/jarvis/... .
        val linuxPath = when {
            ...
            url.startsWith("/") -> url
                ...
                "Unsupported image_url '$url'. Use a data: URL, file:///host/path, " +
                    "/var/jarvis/<scope>/<path>, or an absolute Linux path."
            )
        }
    ...
    companion object {
        ...
        private const val USAGE_HINT =
            "To invoke a model, pass `--model <model_id>` to `jarvis-model-use run`. " +
        ...
        private const val HELP = """jarvis-model-use — list, search, and invoke LLM models

Usage:
  jarvis-model-use list [--provider <name>] [--modality <mod>]
  jarvis-model-use search <query> [--provider <name>] [--modality <mod>]
  jarvis-model-use run --model <id_or_name> [--provider <label_or_id>]
...
Examples:
  jarvis-model-use list
  jarvis-model-use list --modality image_input
  jarvis-model-use search gemini
  jarvis-model-use run --model claude-sonnet-4-6 --input /var/jarvis/workspace/prompt.json
  jarvis-model-use run --model deepseek/deepseek-v4-flash --input msgs.json   # qualified form
  jarvis-model-use run --model deepseek-v4-flash --provider deepseek --input msgs.json   # equivalent
  echo 'What is 2+2?' | jarvis-model-use run --model gpt-4o
  jarvis-model-use run --model gemini-2.5-flash --system 'You are a poet' \
                      --input msgs.json --output /var/jarvis/workspace/out.txt
"""
    }
}
