...
import com.jarvis.app.offload.ShizukuManager
...
fun OffloadPermissionScreen(
    ...
    val configEnabled by com.jarvis.app.config.MinisConfigPermissionStore.enabled.collectAsState()
    ...
    SettingsScaffold(
        ...
        // T-config: master switch for the jarvis-config CLI surface.
        SettingsSection(
            header = stringResource(R.string.perm_section_config_tool),
            footer = stringResource(R.string.perm_jarvis_config_desc),
        ) {
            SettingsSwitchRow(
                title = stringResource(R.string.perm_allow_jarvis_config),
                checked = configEnabled,
                onCheckedChange = {
                    com.jarvis.app.config.MinisConfigPermissionStore.setEnabled(it)
                },
                showDivider = false,
            )
        }
        ...
        if (a11yRestricted) {
            SettingsSection(
                ...
                SettingsRow(
                    ...
                    // Lands on Jarvis' own App info page, where "Allow
        ...
        AlertDialog(
            ...
            confirmButton = {
                MinisTextButton(onClick = {
                    ...
                    com.jarvis.app.config.MinisConfigPermissionStore.setEnabled(true)
                    AppLogger.info("PermissionsScreen", "user confirmed Reset All — all tool permissions cleared, jarvis-config switch reset to default")
