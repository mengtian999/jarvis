 ...
 *     also accepts a Linux file path under /var/jarvis/... resolved via
...
class SpeechOffloadHandler(private val context: Context) : NativeOffloadHandler {
    ...
    /**
     * [T-android-asr-two-layer-probe] Two-layer availability check mirroring
     * [com.jarvis.app.speech.SystemSpeechRecognitionEngine.isAvailable].
     * [SpeechRecognizer.isRecognitionAvailable] lies `true` on some Chinese
     * ROMs that actually ship no real recognition service, so backstop it
     * with a [RecognitionService.SERVICE_INTERFACE] intent query — no
     * registered service ⇒ not available, regardless of what the first
     * probe said. Without this the old single-layer path walked straight
     * into startListening and failed with ERROR_CLIENT /
     * ERROR_INSUFFICIENT_PERMISSIONS, which the user saw as "the mic button
     * does nothing / errors out".
     */
    private fun recognitionAvailable(): Boolean {
        val systemSaysYes = try { SpeechRecognizer.isRecognitionAvailable(context) }
    ...
    private fun cmdTranscribe(args: OffloadArgs): NativeOffloadResult {
        val available = recognitionAvailable()
        if (!available) {
            val body = JSONObject()
        ...
        // --source: default mic, accept "mic" / "system-mic" / any /var/jarvis/... path.
    ...
    /**
     * Resolve a `/var/jarvis/...`-style Linux path to a host File via
            ...
            if (r == OffloadPermissionManager.AndroidPermissionResult.DENIED) {
                r = OffloadPermissionManager.requestSettingsGate(
                    OffloadPermissionManager.SettingsGateRequest(
                        ...
                        message = "Jarvis needs microphone permission to transcribe speech. Open Settings to allow it.",
...
EXAMPLES:
  ...
  android-speech transcribe --source /var/jarvis/attachments/meeting.m4a
  android-speech languages
