...
import com.jarvis.app.MinisApp
import com.jarvis.app.browser.BrowserAction
import com.jarvis.app.browser.BrowserActionInput
import com.jarvis.app.browser.BrowserActionResult
...
/**
 * jarvis-browser-use — expose the agent's browser_use tool as a CLI inside the
 * PRoot sandbox.
 *
 ...
 * Screenshots are persisted under `/var/jarvis/browser/` and referenced via
 * `image_path` + `jarvis_url` — raw base64 is only included when the caller
...
class BrowserUseOffloadHandler(private val app: MinisApp) : NativeOffloadHandler {
    ...
    override fun handle(request: NativeOffloadRequest): NativeOffloadResult {
        ...
        com.jarvis.app.service.SessionActivityTracker.updateToolStatus(
        ...
        val result: BrowserActionResult = try {
            runBlocking {
                withTimeout(EXECUTE_TIMEOUT_MS) {
                    ...
                    // (`jarvis-browser-use`) is a serial human/script driver:
    ...
    private fun buildInputJson(args: OffloadArgs): JSONObject {
        ...
        // (mirrors `jarvis-config set --file`). Parse failures throw so the
    ...
    private fun encodeData(r: BrowserActionResult, withBase64: Boolean): JSONObject {
        ...
        // Persist screenshot bytes under /var/jarvis/browser/ so shells can
        // reference the JPEG via image_path + jarvis_url instead of piping
        // base64 through stdout.
        val browserHostDir: File? = PRootKernel.resolveHostPath(VAR_JARVIS_BROWSER)?.also {
        ...
        if (!base64.isNullOrEmpty() && browserHostDir != null) {
            ...
            if (bytes != null) {
                ...
                if (runCatching { dest.writeBytes(bytes) }.isSuccess) {
                    val linuxPath = "$VAR_JARVIS_BROWSER/$filename"
                    persistedImagePath = linuxPath
                    out.put("image_path", linuxPath)
                    out.put("jarvis_url", "jarvis://browser/$filename")
                } else {
                    Log.w(TAG, "Failed to persist screenshot to ${dest.absolutePath}")
                }
            }
        } else if (!base64.isNullOrEmpty()) {
            Log.w(TAG, "No /var/jarvis/browser mount — falling back to base64-only output")
        }
            ...
            if (data != null) {
                ...
                if (browserHostDir != null) {
                    ...
                    if (runCatching { dest.writeBytes(data) }.isSuccess) {
                        out.put("fetched_path", "$VAR_JARVIS_BROWSER/$fname")
                        out.put("fetched_jarvis_url", "jarvis://browser/$fname")
                    } else {
    ...
    companion object {
        ...
        private const val TOOL_NAME = "jarvis-browser-use"
        private const val VAR_JARVIS_BROWSER = "/var/jarvis/browser"
        ...
        private val HELP = """
jarvis-browser-use - Drive the in-app WebView from the shell

USAGE:
  jarvis-browser-use <action> [options]
  jarvis-browser-use --json '<json>'
  jarvis-browser-use --help
...
COMMON OPTIONS:
  ...
  --with-base64    Also include image_base64 in the screenshot output. Off by
                   default — screenshots are persisted to /var/jarvis/browser/
                   and referenced via image_path + jarvis_url.
...
OUTPUT:
  JSON envelope with a `data` object containing:
    ...
    image_path        Linux path of the persisted JPEG under /var/jarvis/browser/
    jarvis_url         jarvis://browser/<filename> — stable reference for
    ...
    fetched_path      Linux path of the persisted download under /var/jarvis/browser/
    fetched_jarvis_url jarvis://browser/<filename> for the download

EXAMPLES:
  jarvis-browser-use navigate --url https://example.com
  jarvis-browser-use screenshot
  jarvis-browser-use click --selector '.btn-primary'
  jarvis-browser-use type --selector 'input[name=q]' --text 'hello'
  jarvis-browser-use execute_js --script 'return document.title'
  jarvis-browser-use --json '{"action":"navigate","url":"https://x.com"}'
""".trimIndent() + "\n"
    }
}
