package com.jarvis.app.deeplink

import android.net.Uri
import com.jarvis.app.ui.navigation.Routes

/**
 * Parses jarvis:// deep link URIs into navigation actions.
 *
 * Supported routes (matching iOS):
 *   jarvis://share                              → open share flow
 *   jarvis://views/alarm                        → open alarm list
 *   jarvis://open_terminal?init_command=...      → open terminal with command
 *   jarvis://session/<id>                        → open specific session
 *   jarvis://settings                            → Settings home
 *   jarvis://settings/providers                  → Provider list
 *   jarvis://settings/providers/<instanceId>     → Provider detail
 *   jarvis://settings/model-groups               → Model Groups (incl. Agent Loop section)
 *   jarvis://settings/model-groups/<groupId>     → Model Group detail
 *   jarvis://settings/usage                      → Token usage
 *   jarvis://settings/skills                     → Skills management
 *   jarvis://settings/memory                     → Memory management
 *   jarvis://settings/storage                    → Storage management
 *   jarvis://settings/mount-external             → Mount External Folders list
 *   jarvis://settings/mounts                     → alias for mount-external
 *   jarvis://settings/shared-folders             → Shared Folders list (T235)
 *   jarvis://settings/shared_folders             → alias for shared-folders
 *   jarvis://settings/logs                       → Log management
 *   jarvis://settings/appearance                 → Appearance
 *   jarvis://settings/background                 → Background settings
 *   jarvis://settings/about                      → About
 *   jarvis://settings/permissions                → Permissions
 *   jarvis://settings/environments[?create_key=...&create_value=...&create_note=...]
 *                                               → Environment variables
 *   jarvis://settings/rootfs                     → Rootfs management (mirror config lives here)
 *   jarvis://settings/mirrors                    → alias for rootfs (mirrors live inside Rootfs UI)
 *
 ...
 *
 * Resource-class URIs (`jarvis://workspace/...`, `jarvis://skills/...`,
...
sealed class DeepLinkAction {
     ...
     *
     * Encoded as `jarvis://action/<name>` so the static shortcuts XML can
     * point to them via plain Intent.data without any custom extras —
     * matches the existing jarvis:// deep-link conventions.
     */
     ...
     * [com.jarvis.app.ui.navigation.Routes] does the rest. Routes
     ...
     * shortcut.
     *
     * Encoded as `jarvis://session/<sessionId>/<resource-path>` —
     * [sessionId] selects which chat to land in; [resourcePath] is the
     * resource path under `/var/jarvis/` (e.g. `/browser/snake.html`).
...
object DeepLinkHandler {
    fun parse(uri: Uri?): DeepLinkAction {
        if (uri == null || uri.scheme != "jarvis") return DeepLinkAction.Unknown
        ...
        return when (host) {
            ...
            "session" -> {
                // jarvis://session/<sessionId>                → OpenSession
                // jarvis://session/<sessionId>/<resource-path> → OpenHtmlPreview
    ...
    /**
     * T183: walk a `jarvis://settings/<path>` URI to the right NavHost
    ...
    private fun parseSettingsPath(uri: Uri): DeepLinkAction {
        ...
        return when (head) {
            ...
            "environments" -> {
                ...
                //   jarvis://settings/environments?create_key=GH_TOKEN&create_value=
