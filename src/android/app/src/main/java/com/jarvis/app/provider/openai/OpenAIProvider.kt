package com.jarvis.app.provider.openai
...
import com.jarvis.app.data.model.LLMResponse
...
import com.jarvis.app.provider.thinking.ThinkingResolveContext
import com.jarvis.app.provider.thinking.ThinkingRuleResolver
import com.jarvis.app.provider.LLMProvider
import com.jarvis.app.provider.applyUserAgentOverride
...
import com.jarvis.app.provider.failOnSilentEmptyCompletion
...
class OpenAIProvider private constructor(
     ...
     * user-authored custom rules from [com.jarvis.app.provider.thinking.ThinkingRuleResolver]'s
     ...
     * the app-level [com.jarvis.app.data.FastModePrefs] toggle that Codex
    ...
    internal fun resolvedServiceTier(): String? =
        if (supportsPriorityProcessing && com.jarvis.app.data.FastModePrefs.isEnabled()) {
            "priority"
        } else {
            null
        }
     ...
     * `jarvis-model-use` can pass provider-specific params our fixed schema never
    ...
    // jarvis-2026-05-27.log around 13:28 — 3:00 of silence then trip).
    ...
    private val client = OkHttpClient.Builder()
        ...
        .connectionPool(com.jarvis.app.network.NetworkMonitor.sharedLLMConnectionPool)
    ...
    override suspend fun sendMessageClamped(
        ...
        // jarvis-model-use (ModelUseOffloadHandler) — get them on
    ...
    private fun rawStreamMessage(
        ...
        val memBefore = com.jarvis.app.diagnostics.MemorySnapshot.capture()
        val serStartNs = System.nanoTime()
        val bodyStr = try {
            body.toString()
        } catch (t: Throwable) {
            com.jarvis.app.diagnostics.LargeAllocProbe.report(
                "openai.body.toString", -1, "model=${model.id} messages=${messages.size}",
                memBefore, serStartNs, failure = t,
            )
            throw t
        }
        if (bodyStr.length >= com.jarvis.app.diagnostics.LargeAllocProbe.NOTABLE_BYTES) {
            com.jarvis.app.diagnostics.LargeAllocProbe.report(
        ...
        if (!response.isSuccessful) {
            ...
            com.jarvis.app.logging.AppLogger.error(
                "OpenAIProvider",
            ...
            if (com.jarvis.app.BuildConfig.DEBUG) {
                com.jarvis.app.debug.LLMRequestLog.add(
                    com.jarvis.app.debug.LLMRequestLog.Entry(
                        provider = "openai",
                ...
                run {
                    ...
                    if (delta != null) {
                        ...
                        if (cLen + rcLen + rLen + tcLen > 0 || delta.has("role")) {
                            com.jarvis.app.logging.AppLogger.debug(
                                "OpenAIProvider",
     ...
     * jarvis-model-use returned `image_edit_not_supported` for every
                    ...
                    msg.role == LLMMessage.Role.USER -> {
                        ...
                        for (tr in toolResults) {
                            ...
                            if (trBytes != null && trBytes.isNotEmpty() && supportsImages) {
                                val safeBytes = com.jarvis.app.provider.ImageBudget
                                    ...
                                    when (part) {
                                        ...
                                        is AgentContentPart.ImageData -> {
                                            if (supportsImages) {
                                                ...
                                                val safeBytes = com.jarvis.app.provider.ImageBudget.compressUnderBudget(part.data)
    ...
    private suspend fun buildRequest(bodyStr: String): Request {
        ...
        if (isOAuth && !forceChatCompletions) {
            ...
            // Jarvis UA here: the ChatGPT OAuth backend validates the
    ...
    private fun injectThinkingParams(body: JSONObject, level: ThinkingLevel, maxTokens: Int) {
        ...
        // variable with a more complicated one. jarvis-config exposure is Phase 2.
        com.jarvis.app.logging.AppLogger.info(
            "Thinking",
    ...
    private fun responsesImageBlock(
        ...
        // T-imgsize: provider-boundary backstop.
        val safeBytes = com.jarvis.app.provider.ImageBudget.compressUnderBudget(data)
    ...
    internal fun buildResponsesAPIBody(
         ...
         * caller that supplies images this way (jarvis-model-use's `image_url`
        ...
        if (!body.has("service_tier") &&
            com.jarvis.app.data.FastModePrefs.isEnabled() &&
        ...
        for ((msgIndex, msg) in messages.withIndex()) {
            ...
            if (msg.contentParts.isNotEmpty()) {
                        ...
                        if (imgParts.isNotEmpty()) {
                            ...
                            for (part in msg.contentParts) {
                                when (part) {
                                    ...
                                    else -> com.jarvis.app.logging.AppLogger.error(
                                        "OpenAIProvider",
                            ...
                            // supplied top-level (jarvis-model-use / Vision
                ...
                // the jarvis-model-use path. The Responses API keeps the SAME
                ...
                // jarvis-model-use's image_url blocks produce. This builder had
