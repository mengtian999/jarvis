 ...
 * the `jarvis-sessions-cli list` offload command). The SELECT shape is
 ...
 * the `jarvis-sessions-cli search` offload command). Same RawQuery
...
interface ChatDao {
    ...
    // ─── T188: jarvis-sessions-cli backing queries ─────────────────────────────
     ...
     * table. Backs `jarvis-sessions-cli search`. SELECT must produce columns
    ...
    /**
     * Paginated message page for `jarvis-sessions-cli messages --offset --limit`.
    ...
    /** Used by `jarvis-sessions-cli messages` to surface the total count
