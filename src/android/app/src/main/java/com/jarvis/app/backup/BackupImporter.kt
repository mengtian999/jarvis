...
/**
 * Restores a `.jarvisbak` package on Android (docs/backup-restore-design.md §8),
...
class BackupImporter(
            ...
            for (category in ORDER.filter { it in wanted }) {
                ...
                val categoryReport = try {
                    when (category) {
                        ...
                        BackupCategory.ROLES -> importRoles(work)
    ...
    private suspend fun importChats(
        ...
        val sessionsRoot = File(context.filesDir, "jarvis-sessions")
    ...
    private fun importSharedFiles(
        ...
        val dest = File(context.filesDir, "jarvis-global/shared")
        val files = BackupRestoreFiles.restore(
    ...
    private fun importSkills(root: File, fileIndex: List<BackupFileIndexEntry>): CategoryReport {
        ...
        val dest = File(context.filesDir, "jarvis-global/skills")
    ...
    private fun importMemory(root: File): CategoryReport {
        ...
        val dest = File(context.filesDir, "jarvis-global/memory").apply { mkdirs() }
        ...
        // [T-backup-roles-category] Legacy packages (before ROLES was a
        // category of its own) carried the roles tree inside data/memory/roles/.
        // The whole-tree copy above may therefore have written role files —
        // refresh the in-memory caches either way so restored personas show up
        // without a relaunch. Cheap: a directory walk over a handful of dirs.
        com.jarvis.app.agent.RoleStore.refreshAfterRestore()
        return report
    }

    /**
     * Role personas → `jarvis-global/memory/roles/`. Verbatim tree copy with
     * the same containment rule as [importMemory]. Identical content is a
     * skip, not an import — restoring a package onto the device that produced
     * it must not report every file as freshly written (ZIP mtimes are
     * meaningless; compare bytes, these files are tiny).
     *
     * Legacy packages that carried roles inside `data/memory/roles/` are
     * handled by [importMemory]'s whole-tree copy, so both shapes restore.
     */
    private fun importRoles(root: File): CategoryReport {
        val report = CategoryReport(BackupCategory.ROLES.key)
        val src = File(root, "data/roles")
        if (!src.isDirectory) return report
        val dest = File(context.filesDir, "jarvis-global/memory/roles").apply { mkdirs() }
        val destRoot = dest.canonicalFile
        for (file in src.walkTopDown().filter { it.isFile }) {
            val rel = file.relativeTo(src).path.replace(File.separatorChar, '/')
            val out = File(dest, rel)
            // Same containment rule as the blob path: `data/roles` names come
            // from inside the package too.
            val parent = out.parentFile?.let { it.mkdirs(); it.canonicalFile }
            if (parent == null || !(parent.path == destRoot.path ||
                    parent.path.startsWith(destRoot.path + File.separator))
            ...
            if (out.isFile && out.length() == file.length() &&
                out.readBytes().contentEquals(file.readBytes())
            ) {
                report.skipped += 1
                continue
            }
        ...
        // The collection snapshot, the current-role pointer (current.json was
        // part of the restore) and the current-role StateFlow are all cached
        // in memory at startup; raw file writes leave them stale until
        // relaunch unless they are refreshed here.
        com.jarvis.app.agent.RoleStore.refreshAfterRestore()
        return report
    }
    ...
    private fun importMcpServers(root: File): CategoryReport {
        ...
        val dest = File(context.filesDir, "jarvis-global/mcp-servers/servers.json")
    ...
    companion object {
        ...
        internal data class LenientProviderConfig(
            val config: com.jarvis.app.data.model.ProviderConfig,
        ...
        internal fun parseProviderConfigLeniently(text: String): LenientProviderConfig {
            ...
            if (rawInstances == null) {
                return LenientProviderConfig(
                    BackupFormat.json.decodeFromJsonElement(
                        com.jarvis.app.data.model.ProviderConfig.serializer(), root,
                    ),
                    droppedInstances = 0,
                )
            }

            val knownTypes = com.jarvis.app.data.model.ProviderType.entries.map { it.name }.toSet()
            ...
            for (element in rawInstances) {
                ...
                val normalized = if (rawType != null && rawType !in knownTypes) {
                    ...
                    JsonObject(
                        obj + ("providerType" to kotlinx.serialization.json.JsonPrimitive(
                            com.jarvis.app.data.model.ProviderType.unsupported.name,
                        )),
                    )
                } else obj
                ...
                val decodes = runCatching {
                    BackupFormat.json.decodeFromJsonElement(
                        com.jarvis.app.data.model.ProviderInstance.serializer(), normalized,
                    )
                }
            ...
            return LenientProviderConfig(
                BackupFormat.json.decodeFromJsonElement(
                    com.jarvis.app.data.model.ProviderConfig.serializer(), patched,
                ),
        ...
        private val ORDER = listOf(
            ...
            // Roles after memory: both write under jarvis-global/memory, and a
            // role restore must not race a memory tree copy into the same
            // directory. Mirrors the export order.
            BackupCategory.ROLES,
