...
/**
 * Read-side entry point for a `.jarvisbak` package, implementing the §8.1
 * pre-flight order:
 *
    ...
    fun readManifest(): BackupManifest {
        ...
        if (major != BackupFormat.CURRENT) {
            throw BackupException(
                "This backup was created by a newer version of Jarvis (${manifest.format}). Please update the app."
            )
        }
        return manifest
    }
