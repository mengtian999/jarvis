...
import com.jarvis.app.data.model.normalizeModalityName
...
internal suspend fun performTest(
    ...
    if (kind == QuickTestKind.SPEECH_OUT || kind == QuickTestKind.TRANSCRIPTION) {
        val voice = com.jarvis.app.provider.voice.VoiceProviderFactory.make(instance, apiKey)
        ...
        return@withContext when (kind) {
            QuickTestKind.SPEECH_OUT -> {
                if (!voice.supportsVoiceOutput) {
                    ...
                    runCatching {
                        ...
                        val data = voice.synthesize(
                            com.jarvis.app.provider.voice.VoiceOutputRequest(
                                input = "Hi! This is Jarvis testing text to speech.",
                ...
                if (!voice.supportsVoiceInput) {
                    ...
                    val spoken = "Hello from Jarvis, testing speech to text."
                    ...
                    runCatching {
                        val resp = voice.transcribe(
                            com.jarvis.app.provider.voice.VoiceInputRequest(
                                audioData = clip,
                ...
                val resp = provider.sendMessage(
                    messages = listOf(
                        LLMMessage(
                            ...
                            content = "Hi! I'm setting you up in Jarvis. Say hello back in one short, friendly sentence.",
                        ),
                    ),
                    systemPrompt = null,
                    maxTokens = 128,
        ...
        QuickTestKind.IMAGE_GEN -> {
            ...
            runCatching {
                val resp = openAI.generateImage(
                    prompt = "A friendly cute mascot logo for an app called Jarvis, minimalist, centered, soft colors",
                    n = 1,
                    size = "1024x1024",
                    quality = null,
                )
