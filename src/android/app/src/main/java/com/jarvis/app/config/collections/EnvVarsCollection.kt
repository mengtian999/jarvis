 ...
 * `jarvis://settings/environments?create_key=…` so the user supplies
 * the value directly.
 */
class EnvVarsCollection(
    ...
    override fun fields(forId: String): List<ConfigField> {
        ...
        return listOf(
            ...
            HiddenField(
                ...
                reason = "Env var values are stored in encrypted prefs and never exposed to jarvis-config",
            ),
        )
    }
    ...
    override fun add(payload: ConfigValue): String {
        throw ConfigError.PermissionDenied(
            "Use the deep link `jarvis://settings/environments?create_key=…` so the user enters the value directly"
        )
    }
