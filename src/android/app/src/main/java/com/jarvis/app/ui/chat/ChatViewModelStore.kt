...
object ChatViewModelStore {
     ...
     * `jarvis-config session.*` reads this so reads/writes target the
    ...
    /**
     * [T-role-forward] One-shot "auto-respond after forward" request. The
     * source session's forwardMessagesToRole appends the forwarded material
     * directly to the target session's DB rows, then stashes this marker so
     * the target's ChatScreen — when it composes after navigation — starts
     * the agent loop over the just-injected history without the user having
     * to tap send. Same target-binding + TTL discipline as
     * [PendingTransfer]: only the addressed session may drain it, and a
     * stale marker is dropped rather than firing an unexpected reply.
     */
    data class PendingForwardRespond(
        val targetId: String,
        val stashedAtMs: Long = System.currentTimeMillis(),
    )

    private const val FORWARD_RESPOND_TTL_MS = 300_000L

    @Volatile
    private var pendingForwardRespond: PendingForwardRespond? = null

    fun stashPendingForwardRespond(request: PendingForwardRespond) {
        pendingForwardRespond = request
        Log.d(TAG, "stashPendingForwardRespond: target=${request.targetId}")
    }

    /**
     * Drain the forward-respond slot exactly once, only for [sessionId].
     * Expired or mis-addressed markers are dropped/ignored the same way
     * [consumePendingTransfer] treats a stale transfer.
     */
    fun consumePendingForwardRespond(sessionId: String): Boolean {
        val r = pendingForwardRespond ?: return false
        if (System.currentTimeMillis() - r.stashedAtMs > FORWARD_RESPOND_TTL_MS) {
            pendingForwardRespond = null
            Log.d(TAG, "consumePendingForwardRespond: dropping stale marker (target=${r.targetId})")
            return false
        }
        if (resolveKey(r.targetId) != resolveKey(sessionId)) return false
        pendingForwardRespond = null
        Log.d(TAG, "consumePendingForwardRespond: target=${r.targetId}")
        return true
    }
}
