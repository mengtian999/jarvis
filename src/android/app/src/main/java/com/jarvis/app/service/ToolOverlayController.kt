 ...
 * shows the Jarvis launcher logo, the running tool's label, and a short
 * status — same data the FGS notification reads
 * (`SessionActivityTracker.currentToolName` / `currentToolStatus`).
 *
 * T-bg-overlay-polish: the per-tool icon was replaced with the Jarvis
 ...
 *
 *   - Jarvis foreground OR no tool running OR user toggle OFF OR no perm → hidden
 *   - otherwise → shown, content updated on each tool-status change
 *
 * Position persists in [com.jarvis.app.data.repository.BackgroundSettingsRepository]
 * across process restarts; first run defaults to the bottom-left corner
 * (10 dp from each edge). Tap dismisses to bring Jarvis back to the
...
class ToolOverlayController(private val context: Context) {
    ...
    // `jarvis://session/<id>` deep-link to land back in the right chat;
    ...
    private fun updateContent(
        ...
        // through verbatim would render a noisy "Jarvis / Idle" capsule that
        ...
        try {
            ...
            val launchIntent = Intent(
                ...
                if (!sid.isNullOrBlank()) {
                    data = Uri.parse("jarvis://session/$sid")
                }
     ...
     * "Jarvis is using " prefix — the overlay capsule is tight, so we just
