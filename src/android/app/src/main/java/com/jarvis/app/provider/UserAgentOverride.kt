 ...
 * The default carries the Jarvis version so request logs upstream can be
...
/**
 * [T-android-default-ua] Branded User-Agent used by every Jarvis-originated
 * outbound request that doesn't have a SDK-specific UA requirement.
 *
 * Format mirrors iOS exactly:
 *   `Jarvis/<version> (Android <release>; <model>)`
 *
 * e.g. `Jarvis/0.14-preview (Android 13; Pixel 4a)` — same shape as iOS's
 * `Jarvis/1.10 (iOS 26.5; iPhone)`.
 *
...
object MinisUserAgent {
    val DEFAULT: String by lazy {
        ...
        "Jarvis/${BuildConfig.VERSION_NAME} (Android $release; $model)"
    }
}
