...
class DebugServer(
     ...
     *   adb shell run-as com.jarvis.app cat files/debug_server_token
    ...
    fun start() {
        ...
        acceptJob = scope.launch {
            try {
                ...
                Log.i(TAG, "Remote (non-loopback) clients must send X-Jarvis-Token: $authToken")
        ...
        socket.use { s ->
            try {
                ...
                while (true) {
                    ...
                    // [T-android-debugserver-auth] Token via X-Jarvis-Token or
                ...
                if (!isAuthorized(isLoopback, providedToken, authToken)) {
                    ...
                    sendResponse(writer, 401, rpcHandler.errorJSON(-32000, "Unauthorized — send X-Jarvis-Token (see `adb shell run-as com.jarvis.app cat files/debug_server_token`)"))
                    return
                }
                ...
                if (method == "GET") {
                    ...
                    when (path) {
                        ...
                        "/skill/examples/python", "/skill/examples/jarvis_rpc_android.py" -> {
                            sendSkillAsset(writer, "examples/jarvis_rpc_android.py", "text/x-python; charset=utf-8")
                            return
                        }
    ...
    private fun sendSkill(writer: PrintWriter, wantsHuman: Boolean) {
        ...
        val py = readSkillAsset("examples/jarvis_rpc_android.py") ?: ""
    ...
    private fun sendResponse(
        ...
        writer.print("Access-Control-Allow-Headers: Content-Type, X-Jarvis-Token, Authorization\r\n")
        writer.print("\r\n")
        writer.print(body)
        writer.flush()
    }
