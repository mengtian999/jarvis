package com.jarvis.app.backup.remote
 ...
 * `.jarvis-parts/<backupId>/` with a resume journal, because rclone has no
 ...
 * and invisible to the iOS restore picker, which lists `.jarvisbak` files only.
 ...
 * self-contained `.jarvisbak` a user can grab with any client, and an
 * interrupted upload simply re-runs. A failure is surfaced per-destination by
 * the caller, so it is visible, not silent.
 *
 * **New uploads never write `.jarvis-parts` again.** Reading it back is kept as
...
class RcloneChunkedUpload(private val context: Context) {
    ...
    fun upload(
        ...
        // mistaken for a backup: the restore list matches `.jarvisbak` exactly.
     ...
     * are hidden from the restore list (the filter requires a `.jarvisbak`
     ...
     * `.partial` scratch objects, never a user's `.jarvisbak` and never the
     * historical `.jarvis-parts` directory.
     */
     ...
     *
     * A `.jarvis-parts/<id>/` upload is a DIRECTORY, and `deletefile` on it
    ...
    data class RemotePackage(
        ...
        val modified: Long?,
        /**
         * >1 only for a `.jarvis-parts` upload written by an older build.
     ...
     *
     * Dotfiles are skipped — that hides `.jarvis-parts` scratch and the
    ...
    /**
     * Everything restorable in [remote]: whole `.jarvisbak` packages, plus any
     * chunked upload a PREVIOUS build left behind.
     *
     * In-flight scratch files never show up here: they are named
     * `<package>.jarvisbak.partial`, and the filter below requires the name to
     * END in `.jarvisbak`.
     */
    fun listPackages(remote: RcloneRemoteStore.Remote): List<RemotePackage> {
        ...
        // BACKWARD COMPATIBILITY ONLY. Nothing writes `.jarvis-parts` any more,
    ...
    fun download(
        ...
        val scratch = File(context.cacheDir, "jarvis-dl-${pkg.displayName}").apply { mkdirs() }
        try {
    ...
    companion object {
         ...
         */
        const val PARTS_DIR = ".jarvis-parts"
