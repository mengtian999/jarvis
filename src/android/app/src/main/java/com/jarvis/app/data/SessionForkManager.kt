...
class SessionForkManager(
    ...
    suspend fun duplicateSession(sessionId: String): String? {
        ...
        val new = chatRepository.createSession(
            ...
            title = dupTitle,
            // [T-role-session-bound] Inherit the source session's bound role
            // so the duplicate keeps the same persona. fork never switches
            // role (that would re-introduce in-session personality split) —
            // "copy this role's session" stays this role.
        ...
        for (msg in messages) {
            val newMsg = chatRepository.appendMessage(
                ...
                // [T-role-message-level] Preserve each message's generating
                // role so the copy's history renders the same roles as the
                // source (assistant rows keep their roleId; user rows stay null).
                roleId = msg.roleId,
            )
     ...
     * `<filesDir>/jarvis-global/memory/<fileName>`. Mirrors iOS
    ...
    fun copyMemory(fileName: String, content: String): Boolean {
        ...
        val dir = File(filesDir, "jarvis-global/memory").apply { mkdirs() }
        return try {
