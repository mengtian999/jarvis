...
object ExecutionCoordinator {
    ...
    private suspend fun getOrCreateShell(sessionId: String): PersistentShell {
        ...
        if (existing != null && existing.isAlive) {
            Log.w(TAG, "[diag] reuse existing shell for sessionId=$sessionId attachmentsMount=${existing.debugBindMount("/var/jarvis/attachments")}")
            return existing
        }
        ...
        return globalLock.withLock {
            ...
            if (recheck != null && recheck.isAlive) {
                Log.w(TAG, "[diag] reuse existing shell (post-lock) for sessionId=$sessionId attachmentsMount=${recheck.debugBindMount("/var/jarvis/attachments")}")
            ...
            Log.w(TAG, "[diag] new shell created sessionId=$sessionId attachmentsMount=${bindMounts["/var/jarvis/attachments"]}")
            shell
        }
    }
    ...
    private fun buildSessionBindMounts(sessionId: String): Map<String, String> {
        ...
        val prevAttachments = PRootKernel.bindMounts["/var/jarvis/attachments"]

        // Session-specific directories
        val sessionBase = File(filesDir, "jarvis-sessions/$sessionId")
        listOf("attachments", "offloads", "workspace", "browser").forEach { subdir ->
            val hostDir = File(sessionBase, subdir).also { it.mkdirs() }
            val linuxPath = "/var/jarvis/$subdir"
        ...
        Log.w(TAG, "[diag] buildSessionBindMounts sessionId=$sessionId " +
            "attachments: prev=$prevAttachments new=${mounts["/var/jarvis/attachments"]}")
        ...
        // shell that runs jarvis-mcp-cli — /var/jarvis/mcp-servers/servers.json
        // then resolves to the empty rootfs placeholder and `jarvis-mcp-cli list`
        ...
        // below.
        val globalBase = File(filesDir, "jarvis-global")
        ...
        // is invisible to the shell — `ls /var/jarvis/mounts/<name>/` then shows
        ...
        PRootKernel.mountedFoldersStore?.entries?.value?.forEach { entry ->
            ...
            val linuxPath = "/var/jarvis/mounts/${entry.name}"
