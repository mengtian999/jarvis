package com.jarvis.app.scheduled
...
import com.jarvis.app.debug.HeadlessChatRunner
import com.jarvis.app.logging.AppLogger
import com.jarvis.app.service.AgentForegroundService
...
object ScheduledAgentRunner {
     ...
     *   e.g. the jarvis-scheduled CLI, which runs in its own offload thread.
    ...
    private suspend fun resolveSessionId(app: MinisApp, task: ScheduledTask): String? {
        return when (val mode = task.targetMode) {
            ...
            ScheduledTargetMode.NewSession -> {
                ...
                val memoryOn = com.jarvis.app.data.MemoryGlobalPrefs.isGlobalEnabled(app)
                val session = app.chatRepository.createSession(
                    modelId = seedModelId,
                    title = title,
                    memoryEnabled = memoryOn,
                    // [T-role-session-bound] Scheduled tasks run under the
                    // global current role — same seed a plain draft gets.
                    roleId = com.jarvis.app.agent.RoleStore.currentRole().roleId,
                )
    ...
    private fun postCompletionNotification(
        ...
        val deepLink = Uri.parse("jarvis://session/$sessionId")
        ...
        val title = "Jarvis: ${task.label.ifBlank { "Scheduled task" }}"
