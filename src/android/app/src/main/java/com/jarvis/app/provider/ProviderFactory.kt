package com.jarvis.app.provider
...
import com.jarvis.app.provider.anthropic.AnthropicProvider
import com.jarvis.app.provider.gemini.GeminiProvider
import com.jarvis.app.provider.openai.OpenAIProvider
        ...
        val provider: LLMProvider = when (instance.providerType) {
            ...
            ProviderType.openAI, ProviderType.openAIResponses -> {
                ...
                val manualBearer = if (context != null &&
                    ...
                    com.jarvis.app.auth.OAuthManager.forInstance(context, instance)?.loadManualBearerToken()
                } else null
                ...
                if (instance.credentialType == ProviderCredential.oauth && manualBearer.isNullOrEmpty()
                    ...
                    OpenAIProvider(
                        oauthTokenProvider = { oauthManager.validAccessToken() ?: throw com.jarvis.app.data.model.LLMError.InvalidApiKey() },
                        model = model,
            ...
            ProviderType.xAI -> {
                ...
                if (instance.credentialType == ProviderCredential.oauth && manualBearer.isNullOrEmpty()
                    && context != null) {
                    val oauthManager = com.jarvis.app.auth.XAIOAuthManager(context, instance.id)
                    OpenAIProvider.oauthOpenAICompat(
                        oauthTokenProvider = {
                            oauthManager.validAccessToken()
                                ?: throw com.jarvis.app.data.model.LLMError.InvalidApiKey()
                        },
                        model = model,
                        basePath = base,
            ...
            ProviderType.kimiCode -> {
                ...
                val base = basePath ?: "${com.jarvis.app.auth.KimiDeviceFlow.CODING_API_BASE}/v1"
                val manualBearer = if (context != null &&
                    instance.credentialType == ProviderCredential.oauth) {
                    com.jarvis.app.auth.OAuthManager.forInstance(context, instance)?.loadManualBearerToken()
                } else null
                if (instance.credentialType == ProviderCredential.oauth && manualBearer.isNullOrEmpty()
                    && context != null) {
                    val oauthManager = com.jarvis.app.auth.KimiOAuthManager(context, instance.id)
