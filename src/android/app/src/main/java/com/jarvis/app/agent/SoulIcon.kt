...
object SoulIcon {
    ...
    /**
     * [T-android-soul-icon-config-images] Turn a `jarvis-config` value into
     * bitmap bytes.
     *
    ...
    fun classifySource(raw: String): Source {
        ...
        return when {
            v.startsWith("http://", true) || v.startsWith("https://", true) ->
                Source.Unsupported(
                    "remote URLs aren't supported on Android — download the file first, " +
                        "then pass a path like /var/jarvis/attachments/icon.png",
                )
            v.startsWith("jarvis://") -> {
                // jarvis://attachments/x.png -> /var/jarvis/attachments/x.png
                val rest = v.removePrefix("jarvis://").trimStart('/')
                if (rest.isEmpty()) Source.Unsupported("empty jarvis:// path")
                else Source.LinuxPath("/var/jarvis/$rest")
            }
            v.startsWith("data:") -> {
                val comma = v.indexOf(',')
            ...
            looksLikeBareBase64(v) ->
                ...
                "not an emoji, a data URI, base64, a jarvis:// resource or a /var/jarvis path",
            )
        }
    }
     ...
     * model-supplied `/var/jarvis/attachments/../../../databases/x` would walk
     * straight out of the sandbox.
     */
    val ALLOWED_LINUX_ROOTS = listOf(
        "/var/jarvis/attachments",
        "/var/jarvis/workspace",
        "/var/jarvis/offloads",
        "/var/jarvis/shared",
        "/var/jarvis/memory",
        "/var/jarvis/skills",
