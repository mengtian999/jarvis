...
import com.jarvis.app.data.db.FolderEntity
...
/**
 * Builds a `.jarvisbak` package on Android (docs/backup-restore-design.md §2,
...
class BackupExporter(
    ...
    data class Options(
        ...
        /**
         * [T-backup-roles-category] Role personas the user UNCHECKED in the
         * expandable Roles sub-list. Default empty = every role ships. The
         * default role ("default" / Jarvis) is never excludable — see
         * [exportRoles], which filters the id out defensively so a package
         * always restores with at least one usable persona.
         */
        val excludedRoleIds: Set<String> = emptySet(),
    )
    ...
    private suspend fun exportBody(options: Options, onProgress: ((String) -> Unit)?): Summary {
        ...
        try {
            ...
            BackupFileIndexWriter(File(staging, "files.index.jsonl")).use { fileIndex ->
                ...
                if (BackupCategory.ROLES in options.categories) {
                    onProgress?.invoke("Exporting roles…")
                    exportRoles(dataDir, options.excludedRoleIds)?.let { stats[BackupCategory.ROLES.key] = it }
                }
                ...
                BackupJsonlWriter(dataDir, "compact_markers").use { markers ->
                    BackupJsonlWriter(dataDir, "folders").use { folders ->
                        for (session in dao.listSessions()) {
                            ...
                            val dir = File(context.filesDir, "jarvis-sessions/${session.id}")
                            val r = trees.export(
    ...
    /**
     * §3.2 — the cross-session `/var/jarvis/shared` bucket. Host-side this is
     * `<filesDir>/jarvis-global/shared`, NOT anything inside the rootfs.
     */
    private fun exportSharedFiles(trees: BackupFileTreeExporter): BackupManifest.CategoryStat {
        val r = trees.export(
            File(context.filesDir, "jarvis-global/shared"), "shared", BackupCategory.SHARED_FILES
        )
    ...
    private fun exportSkills(trees: BackupFileTreeExporter): BackupManifest.CategoryStat {
        val root = File(context.filesDir, "jarvis-global/skills")
    ...
    private fun exportMemory(dataDir: File): BackupManifest.CategoryStat {
        val source = File(context.filesDir, "jarvis-global/memory")
        val dest = File(dataDir, "memory").apply { mkdirs() }
        var entries = 0
        var bytes = 0L
        if (source.isDirectory) {
            for (file in source.walkTopDown().filter { it.isFile }) {
                // [T-backup-roles-category] roles/ moved to its own ROLES
                // category; keeping it here too would put the same tree in the
                // package twice (and double-count its bytes).
                val rel = file.relativeTo(source).path.replace(File.separatorChar, '/')
                if (rel == "roles" || rel.startsWith("roles/")) continue
    ...
    /**
     * Role personas, copied verbatim into `data/roles/` — the whole roles/
     * directory including each role's avatar.png and the `current.json`
     * pointer, so a restore puts back exactly what was on disk.
     *
     * `entries` counts ROLES (directories holding a role.json), not files —
     * the unit a user thinks in, mirroring how SKILLS reports skill counts.
     * `files` carries the real file count for the count label, like SKILLS.
     *
     * [T-backup-roles-category] [excludedRoleIds] is the expandable sub-list's
     * unchecked set. The default role (id `"default"`, Jarvis) is filtered OUT
     * of that set here as the data-level guarantee that a package always
     * restores with at least one usable persona — the UI refuses to add it,
     * and this is the second line of defence. `current.json` always travels
     * (it lives at the roles/ root, not under a role dir); if it points at an
     * excluded role, the importer's orphan fallback handles it on restore.
     *
     * Returns null when the collection is absent or empty AFTER filtering, so
     * an empty roles list adds no category rather than an empty one.
     */
    private fun exportRoles(
        dataDir: File,
        excludedRoleIds: Set<String>,
    ): BackupManifest.CategoryStat? {
        val source = File(context.filesDir, "jarvis-global/memory/roles")
        if (!source.isDirectory) return null
        // The default role (id "default", Jarvis — see RoleStore.DEFAULT_ROLE_ID)
        // is never excludable: defence in depth alongside the UI's refusal to
        // put "default" in the excluded set. A package must always restore with
        // at least one usable persona.
        val effectiveExcluded = excludedRoleIds - "default"
        val roleDirs = source.listFiles()?.filter {
            it.isDirectory && File(it, "role.json").isFile && it.name !in effectiveExcluded
        } ?: return null
        if (roleDirs.isEmpty()) return null
        val includedDirNames = roleDirs.map { it.name }.toSet()
        val dest = File(dataDir, "roles").apply { mkdirs() }
        var files = 0
        var bytes = 0L
        for (file in source.walkTopDown().filter { it.isFile }) {
            val rel = file.relativeTo(source).path.replace(File.separatorChar, '/')
            // Files at the roles/ root (current.json) always travel; files
            // under roles/<roleId>/ only when that role is included.
            val isRootFile = '/' !in rel
            val ownerDir = rel.substringBefore('/', "")
            if (!isRootFile && ownerDir !in includedDirNames) continue
            val out = File(dest, rel).apply { parentFile?.mkdirs() }
            file.copyTo(out, overwrite = true)
            files += 1
            bytes += file.length()
    ...
    private fun exportMcpServers(dataDir: File): BackupManifest.CategoryStat? {
        val source = File(context.filesDir, "jarvis-global/mcp-servers/servers.json")
    ...
    private val app: com.jarvis.app.MinisApp?
        get() = context.applicationContext as? com.jarvis.app.MinisApp
    ...
    private suspend fun exportProviders(
        ...
        val config = repo?.config?.value ?: com.jarvis.app.data.model.ProviderConfig()
        val url = File(dataDir, "provider_config.json")
        url.writeText(
            BackupFormat.json.encodeToString(
                com.jarvis.app.data.model.ProviderConfig.serializer(), config
            )
        )
    ...
    private suspend fun exportThinkingRules(dataDir: File): Int {
        val dao = com.jarvis.app.data.db.ProviderDatabase
    ...
    private fun archive(staging: File, backupId: String): File {
        ...
        // perfectly valid `.jarvisbak` name, which then shows up in the restore
    ...
    companion object {
         ...
         * directories, NOT inside `jarvis-global/shared` — that path is
         * bind-mounted into the guest at `/var/jarvis/shared`, so a package
