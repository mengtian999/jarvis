...
object MarkdownParser {
    ...
    private fun splitTableRow(line: String): List<String> {
        ...
        // [T-jarvis-url-fullwidth-pipe-android] Fast path: the only reason to
        // protect a `|` from the cell split is a markdown link whose URL
        // contains a raw ASCII pipe, e.g. `[f](jarvis://ns/a|b.png)` — the model
        // (or a hand-authored row) can emit an un-encoded `|` inside the URL,
        // and a naive `split('|')` would chop the link in two. App-generated
        // jarvis:// URLs already percent-encode `|` to `%7C`, so this is purely
