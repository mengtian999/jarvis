...
object DebugMethodRegistry {
    ...
    private val DEBUG_ONLY_METHODS: List<MethodSpec> = listOf(
        ...
        MethodSpec(
            ...
            description = "DEBUG-only: invoke ModelUseOffloadHandler directly with the given argv. Parallels debug.shizuku.exec — lets harnesses trigger `jarvis-model-use run/list/search` without an in-shell prompt.",
            params = listOf(
                ParamSpec("args", "[string]", required = false, description = "argv past `jarvis-model-use` (e.g. [\"run\", \"--model\", \"gpt-5.3-codex\"])."),
        ...
        MethodSpec(
            ...
            description = "DEBUG-only: invoke SessionsOffloadHandler directly with the given argv. Parallels debug.modelUse.exec — lets harnesses trigger `jarvis-sessions-cli list/search/messages` (incl. --full) without an in-shell prompt.",
            params = listOf(
                ParamSpec("args", "[string]", required = false, description = "argv past `jarvis-sessions-cli` (e.g. [\"messages\", \"--id\", \"<session_id>\", \"--full\"])."),
                ParamSpec("command", "string", required = false, description = "Whitespace-separated alternative to args."),
            ),
            returns = "{exitCode, output, argv}",
            example = ex(
        ...
        MethodSpec(
            name = "debug.jarvisConfig.exec",
            description = "DEBUG-only: drive jarvis-config through the REAL ConfigBridge (same code path as the in-shell CLI), so a harness can exercise every collection, the confirmation gate and the audit log without an in-shell prompt. Subcommands: set, get, topics, topic-help, audit-list.",
            params = listOf(
    ...
    private val BASE_METHODS: List<MethodSpec> = listOf(
        ...
        MethodSpec(
            ...
            params = listOf(
                ...
                ParamSpec("label", "string", required = false, default = "jarvis-debug", description = "ClipData label."),
            ),
        ...
        MethodSpec(
            ...
            params = listOf(
                ParamSpec("path", "string", required = true, description = "Linux path under the proot rootfs (e.g. /tmp/test.sh, /var/jarvis/skills/example.md)."),
        ...
        MethodSpec(
            ...
            description = "Toggle whether a model entry is exposed to the in-shell jarvis-model-use agent.",
            params = listOf(
                ParamSpec("entryId", "string", required = true, description = "Target entry UUID."),
        ...
        MethodSpec(
            ...
            description = "Toggle whether a model group is exposed to the in-shell jarvis-model-use agent.",
            params = listOf(
                ParamSpec("groupId", "string", required = true, description = "Target group UUID."),
                ParamSpec("inLoop", "bool", required = true, description = "true to add, false to remove."),
            ),
