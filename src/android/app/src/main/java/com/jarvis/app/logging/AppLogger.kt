package com.jarvis.app.logging
...
object AppLogger {
    ...
    private fun startCapture() {
        ...
        // post-crash one.
        appContext?.let { com.jarvis.app.diagnostics.EnvironmentBanner.log(it) }
    }
     ...
     * itself produced (tag prefix `Jarvis.`) are skipped — [log] already wrote
    ...
    private fun writeLogcatLine(rawLine: String) {
        ...
        if (slashIdx >= 0 && parenIdx > slashIdx) {
            ...
            if (tag.startsWith("Jarvis.") || tag == "AppLogger") return
        }
        try {
            val now = Date()
