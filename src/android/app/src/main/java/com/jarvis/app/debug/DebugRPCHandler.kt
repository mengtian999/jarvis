...
class DebugRPCHandler(private val context: Context) {
    ...
    private suspend fun dispatch(method: String, params: JSONObject): Any {
        return when (method) {
            ...
            // harnesses can verify jarvis-sessions-cli (list / search /
            ...
            // [T-jarvis-config-provider-add] DEBUG-only invocation of the
            // ConfigOffloadHandler — parallels debug.modelUse.exec so test
            // harnesses can exercise jarvis-config (get / set / set-batch /
    ...
    private fun handleAppInfo(): JSONObject {
        ...
        return JSONObject().apply {
            ...
            put("diskUsage", JSONObject().apply {
                ...
                put("sessions", dirSize(File(filesDir, "jarvis-sessions")))
                put("global", dirSize(File(filesDir, "jarvis-global")))
            })
        }
    }
     ...
     * at arbitrary paths. Use `jarvis-sessions` (default) to enumerate every
    ...
    private fun handleRawLS(params: JSONObject): Any {
        val subPath = params.optString("path", "jarvis-sessions")
    ...
    private fun crashSummary(f: File): String = try {
        ...
        val frame = body.firstOrNull { it.trimStart().startsWith("at com.jarvis") }
    ...
    private suspend fun handleSetClipboard(params: JSONObject): JSONObject {
        val text = params.optString("text")
        if (text.isEmpty()) throw RPCException(-32602, "Invalid params: 'text' is required")
        val label = params.optString("label", "jarvis-debug")
     ...
     * (`jarvis-browser-use`, `jarvis-open`, …) without going through the agent.
     *
     * Params:
    ...
    private suspend fun handleShellExecute(params: JSONObject): JSONObject {
        ...
        // `jarvis-open` via this RPC trigger the same in-app preview flow as
        ...
        val result = try {
            ExecutionCoordinator.execute(
                ...
                lineCallback = { rawLine ->
                    val (_, urls) = com.jarvis.app.terminal.MinisUrlMarker.extract(rawLine)
        ...
        for (raw in capturedUrls) {
            com.jarvis.app.terminal.MinisOpenUrlBroker.offer(raw)
        }
        return JSONObject()
    ...
    private suspend fun handleUpdateCheck(): JSONObject {
        return when (val r = com.jarvis.app.data.UpdateChecker.check()) {
            is com.jarvis.app.data.UpdateChecker.CheckResult.UpdateAvailable -> JSONObject()
            ...
            com.jarvis.app.data.UpdateChecker.CheckResult.UpToDate ->
                JSONObject().put("status", "up_to_date")
            com.jarvis.app.data.UpdateChecker.CheckResult.NoReleaseAvailable ->
                JSONObject().put("status", "no_release")
            is com.jarvis.app.data.UpdateChecker.CheckResult.NoApkAsset ->
                JSONObject().put("status", "no_apk_asset").put("tag_name", r.tagName)
            com.jarvis.app.data.UpdateChecker.CheckResult.Forbidden ->
                JSONObject().put("status", "forbidden")
            com.jarvis.app.data.UpdateChecker.CheckResult.NetworkUnreachable ->
                JSONObject().put("status", "network_unreachable")
            is com.jarvis.app.data.UpdateChecker.CheckResult.Error ->
    ...
    private suspend fun handleUpdateDownload(params: JSONObject): JSONObject {
        ...
        return when (val r = com.jarvis.app.data.UpdateChecker.download(context, url)) {
            is com.jarvis.app.data.UpdateChecker.DownloadResult.Success -> JSONObject()
                .put("status", "ok")
                .put("path", r.file.absolutePath)
                .put("size", r.file.length())
            is com.jarvis.app.data.UpdateChecker.DownloadResult.Error -> JSONObject()
                .put("status", "error")
    ...
    private fun handleUpdateInstall(params: JSONObject): JSONObject {
        ...
        val canInstall = com.jarvis.app.data.UpdateChecker.canInstall(context)
        return try {
            com.jarvis.app.data.UpdateChecker.installApk(context, file)
    ...
    private fun handleWriteFile(params: JSONObject): JSONObject {
        ...
        val hostFile = PRootKernel.resolveHostPath(path) ?: run {
            val rootfsDir = com.jarvis.app.sandbox.RootfsManager.getInstance(context).rootfsDir
    ...
    private fun handlePermissionsList(): JSONObject {
        ...
        for (tool in com.jarvis.app.offload.OffloadPermissionManager.toolRegistry) {
            arr.put(JSONObject()
                ...
                .put("currentLevel", com.jarvis.app.offload.OffloadPermissionManager.getLevel(tool.toolName).name)
    ...
    /**
     * T344: Direct invocation of [com.jarvis.app.sandbox.offload.ShizukuOffloadHandler]
    ...
    private fun handleShizukuExec(params: JSONObject): JSONObject {
        ...
        val handler = com.jarvis.app.sandbox.offload.ShizukuOffloadHandler(context)
        ...
        val request = com.jarvis.app.sandbox.NativeOffloadRequest(
            pid = -1,
    ...
    /**
     * Direct invocation of [com.jarvis.app.sandbox.offload.ModelUseOffloadHandler]
     * for e2e harnesses. Mirrors [handleShizukuExec]; lets callers exercise the
     * `jarvis-model-use` CLI without going through a real Alpine shell prompt.
     * DEBUG-only.
     */
    private fun handleModelUseExec(params: JSONObject): JSONObject {
        ...
        val finalArgv: List<String> = if (params.has("input")) {
            ...
            val hostFile = com.jarvis.app.sandbox.PRootKernel.resolveHostPath(linuxPath)
        ...
        val app = context.applicationContext as com.jarvis.app.MinisApp
        val handler = com.jarvis.app.sandbox.offload.ModelUseOffloadHandler(context, app.providerRepository)
        val request = com.jarvis.app.sandbox.NativeOffloadRequest(
            pid = -1,
            argv = listOf("jarvis-model-use") + finalArgv,
            env = emptyMap(),
            cwd = "/",
            sessionId = null,
        )
     ...
     * [com.jarvis.app.sandbox.offload.SessionsOffloadHandler] for e2e
     * harnesses. Mirrors [handleModelUseExec]; lets callers exercise the
     * `jarvis-sessions-cli` CLI (list / search / messages, incl. --full)
    ...
    private fun handleSessionsExec(params: JSONObject): JSONObject {
        ...
        val app = context.applicationContext as com.jarvis.app.MinisApp
        val handler = com.jarvis.app.sandbox.offload.SessionsOffloadHandler(app.chatRepository)
        val request = com.jarvis.app.sandbox.NativeOffloadRequest(
            pid = -1,
            argv = listOf("jarvis-sessions-cli") + argvTail,
            env = emptyMap(),
            cwd = "/",
            sessionId = null,
        )
    ...
    /**
     * [T-jarvis-config-provider-add] DEBUG-only jarvis-config invocation
    ...
    private fun handleMinisConfigExec(params: JSONObject): JSONObject {
        ...
        return when (sub) {
            "set" -> {
                ...
                AppLogger.info(
                    "DebugRPC",
                    "debug.jarvisConfig.exec set items=${items.length()} skipConfirmation=$skip",
                )
                // Hop to the main thread because performWriteBatch is a
                // suspend fun that uses Dispatchers.Main internally.
                kotlinx.coroutines.runBlocking {
                    com.jarvis.app.config.ConfigBridge.performWriteBatch(
                        items = items,
                        caption = "debug.jarvisConfig.exec",
                        actorRaw = "debug-rpc",
                        sessionId = null,
            ...
            "get" -> {
                ...
                AppLogger.info("DebugRPC", "debug.jarvisConfig.exec get path=$path")
                com.jarvis.app.config.ConfigBridge.readField(
                    path = path,
                    filter = filter,
                    page = page,
                    pageSize = pageSize,
            ...
            // writable paths in advance; `topics` is `jarvis-config --help`'s
            // index and `topic-help` is `jarvis-config <topic> --help`.
            "topics" -> JSONObject().apply {
                put("ok", true)
                put("topics", com.jarvis.app.config.ConfigBridge.allTopics())
            }
            "topic-help" -> {
                ...
                val known = com.jarvis.app.config.ConfigBridge.allTopics()
                    .let { arr -> (0 until arr.length()).any { arr.optString(it) == topic } }
                if (!known) {
                    throw RPCException(-32602, "Unknown topic '$topic' — call subcommand=topics")
                }
                val fields = com.jarvis.app.config.ConfigBridge.fieldsForTopic(topic)
                JSONObject().apply {
                    put("ok", true)
                    put("topic", topic)
            ...
            "audit-list" -> {
                ...
                com.jarvis.app.config.ConfigBridge.auditList(limit, scope)
            }
