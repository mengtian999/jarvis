...
import com.jarvis.app.offload.MinisNotificationListenerService
import com.jarvis.app.offload.OffloadPermissionManager
import com.jarvis.app.offload.ScheduledNotificationReceiver
import com.jarvis.app.offload.ScheduledNotificationStore
...
class NotificationOffloadHandler(private val context: Context) : NativeOffloadHandler {
    ...
    private fun handleSend(args: OffloadArgs): NativeOffloadResult {
                ...
                if (r == OffloadPermissionManager.AndroidPermissionResult.DENIED) {
                    ...
                    r = OffloadPermissionManager.requestSettingsGate(
                        OffloadPermissionManager.SettingsGateRequest(
                            ...
                            message = "Jarvis needs notification permission to send notifications. Open Settings to allow it.",
        ...
        val oemHint = if (OsCompat.isHuawei || OsCompat.isXiaomi) {
            "On ${OsCompat.oemLabel()}, banner notifications may be disabled by default — the user can enable them in Settings → Notifications for Jarvis."
        } else null
        val data = JSONObject()
            .put("id", id)
            .put("title", title)
            ...
            val result = runBlocking {
                OffloadPermissionManager.requestSettingsGate(
                    OffloadPermissionManager.SettingsGateRequest(
                        ...
                        message = "Jarvis needs Notification access to read the status-bar notifications. Open Settings and enable \"Jarvis\" under Notification access.",
    ...
    companion object {
        ...
        private const val CHANNEL_ID = "jarvis_agent_notifications"
...
Notes:
  ...
  - list needs Notification access (Settings → Apps → Special app
    access → Notification access → Jarvis). First list call opens it.
"""
    }
}
