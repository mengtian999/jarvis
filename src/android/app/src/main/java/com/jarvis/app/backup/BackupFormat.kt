...
/**
 * On-the-wire types for the `.jarvisbak` backup package.
 *
...
object BackupFormat {
    ...
    const val CURRENT = "jarvisbak/1"

    /** File extension registered to the app for "open to import". */
    const val FILE_EXTENSION = "jarvisbak"

    /**
     * MIME type for a `.jarvisbak` package.
     *
     * Matches the `application/x-jarvisbak` iOS declares for the
     * `com.jarvis.app.jarvisbak` UTI (src/ios/Info.plist), so both platforms
     ...
     */
    const val MIME_TYPE = "application/x-jarvisbak"
...
enum class BackupCategory(val key: String) {
    ...
    /**
     * Role personas (`roles/<roleId>/role.json` + `avatar.png`, plus the
     * `current.json` pointer). [T-backup-roles-category] Roles used to ride
     * along inside MEMORY's whole-tree copy, which coupled the persona data to
     * an unrelated toggle and hid it from the manifest — and on iOS it dropped
     * out of packages entirely, since MEMORY there copies only top-level
     * `*.md` files. A first-class category fixes both. Wire key must stay
     * `roles` to match the iOS `BackupCategory` raw value.
     */
    ROLES("roles"),
    PROVIDERS("providers"),
    ...
    val carriesFileTree: Boolean
        get() = when (this) {
            ...
            // ROLES copies verbatim like MEMORY — role dirs are tiny (a JSON
            // body plus a 64px avatar), so the §3.4 cap has nothing to do.
            MEMORY, ROLES, PROVIDERS, MCP_SERVERS, VOICE_CORRECTIONS,
