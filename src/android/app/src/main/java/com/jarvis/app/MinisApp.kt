...
import com.jarvis.app.data.db.DatabaseVersionGuard
import com.jarvis.app.data.repository.BackgroundSettingsRepository
...
import com.jarvis.app.notification.BackgroundTaskNotifier
import com.jarvis.app.logging.AppLogger
import com.jarvis.app.network.NetworkMonitor
import com.jarvis.app.offload.OffloadPermissionManager
import com.jarvis.app.provider.ModelsDevApi
import com.jarvis.app.sandbox.ExecutionCoordinator
import com.jarvis.app.sandbox.MountedFolderCoordinator
import com.jarvis.app.sandbox.NativeOffloadServer
import com.jarvis.app.sandbox.PRootKernel
import com.jarvis.app.sandbox.RootfsManager
import com.jarvis.app.sandbox.offload.AccessibilityOffloadHandler
import com.jarvis.app.sandbox.offload.AlarmOffloadHandler
import com.jarvis.app.sandbox.offload.BrowserUseOffloadHandler
import com.jarvis.app.sandbox.offload.CalendarOffloadHandler
import com.jarvis.app.sandbox.offload.ClipboardOffloadHandler
import com.jarvis.app.sandbox.offload.ContactsOffloadHandler
import com.jarvis.app.sandbox.offload.DeviceOffloadHandler
import com.jarvis.app.sandbox.offload.LocationOffloadHandler
import com.jarvis.app.sandbox.offload.ModelUseOffloadHandler
import com.jarvis.app.sandbox.offload.SessionsOffloadHandler
import com.jarvis.app.sandbox.offload.ShizukuOffloadHandler
import com.jarvis.app.sandbox.offload.NotificationOffloadHandler
import com.jarvis.app.sandbox.offload.OpenOffloadHandler
import com.jarvis.app.sandbox.offload.PhotosOffloadHandler
import com.jarvis.app.sandbox.offload.PlayerOffloadHandler
import com.jarvis.app.sandbox.offload.SpeakOffloadHandler
import com.jarvis.app.sandbox.offload.SpeechOffloadHandler
import com.jarvis.app.sandbox.offload.WeatherOffloadHandler
import com.jarvis.app.service.SessionActivityTracker
import com.jarvis.app.ui.MinisImageFetcher
...
class MinisApp : Application(), ImageLoaderFactory {
    ...
    /**
     * Application-scoped BrowserTabPool for shell-invoked `jarvis-browser-use`.
    ...
    val sharedBrowserTabPool: BrowserTabPool by lazy {
        BrowserTabPool(this).also { it.setSession("jarvis-browser-use") }
    }
    ...
    override fun onCreate() {
        ...
        com.jarvis.app.data.FastModePrefs.prime(this)
        ...
        // Activity context.
        com.jarvis.app.data.AutoCompactPrefs.prime(this)
        ...
        // libs first execute.
        try {
            com.jarvis.app.crash.NativeCrashHandler.install(
            ...
            Thread.setDefaultUncaughtExceptionHandler { thread, throwable ->
                try {
                    ...
                    if (isFgsTimeout) {
                        ...
                        runCatching {
                            val intent = Intent(this, com.jarvis.app.service.AgentForegroundService::class.java)
                            stopService(intent)
                        }
                    }
                } catch (t: Throwable) {
        ...
        com.jarvis.app.crash.CrashFrequencyDetector.checkAtLaunch(this)
        ...
        // [com.jarvis.app.diagnostics.LaunchCycleBeacon].
        try {
            com.jarvis.app.diagnostics.LaunchCycleBeacon.recordLaunch(this)
        } catch (t: Throwable) {
        ...
        com.jarvis.app.diagnostics.HangDetector.start(this)
        ...
        memoryRepository = MemoryRepository(java.io.File(filesDir, "jarvis-global/memory"))
        ...
        com.jarvis.app.agent.SoulStore.ensureExists(this)
        com.jarvis.app.agent.SoulStore.refreshCache(this)

        // [T-role-multi] Prime the multi-role system on startup: injects the
        // application context, ensures the default Jarvis role exists,
        // restores the persisted current role id and warms the reactive
        // current-role StateFlow consumed by the chat UI.
        com.jarvis.app.agent.RoleStore.prime(this)

        // T-config: jarvis-config CLI surface — registry / audit log /
        ...
        com.jarvis.app.config.MinisConfigPermissionStore.init(this)
        com.jarvis.app.config.audit.ConfigAuditLog.init(this)
        com.jarvis.app.config.ConfigRegistry.init(
        ...
        com.jarvis.app.data.EnvVarPrivacyStore.init(this)
        com.jarvis.app.data.EnvVarRedactor.envVarRepository = envVarRepository
        ...
        // Register global /var/jarvis/{memory,skills,shared} bind mounts up-front
        ...
        // `shell_execute` already has `/var/jarvis/mounts/<name>/` visible.
        ...
        // /var/jarvis/mounts/<name> placeholder dirs that PRoot's `-b` needs.
        ...
        NativeOffloadServer.register("jarvis-model-use", ModelUseOffloadHandler(this, providerRepository))
        // T-config: jarvis-config — agent-facing settings management
        ...
        NativeOffloadServer.register(
            "jarvis-config",
            com.jarvis.app.sandbox.offload.ConfigOffloadHandler(),
        )
        NativeOffloadServer.register("jarvis-browser-use", BrowserUseOffloadHandler(this))
        // T188: jarvis-sessions-cli — agent-side query of chat history.
        // Registers next to the other minis-* tools so PRootKernel.
        // installHandlerStubs() picks it up on the next rootfs boot
        // (writes a 17-byte exit-0 stub at /usr/local/bin/jarvis-sessions-cli
        // so PATH lookup succeeds; PRoot intercepts the execve before
        // the stub runs and routes to this handler).
        NativeOffloadServer.register("jarvis-sessions-cli", SessionsOffloadHandler(chatRepository))
        // [T-android-scheduled-tasks-full] jarvis-scheduled — create/list/run
        // timed AI tasks (new chat / follow-up / re-run), mirroring the in-app
        // Scheduled Tasks editor and the iOS Shortcuts intent set.
        NativeOffloadServer.register(
            "jarvis-scheduled",
            com.jarvis.app.sandbox.offload.ScheduledTaskOffloadHandler(this),
        )
        ...
        com.jarvis.app.offload.ShizukuManager.init(this)

        // T-android-jarvis-debug-cli: shell-side CLI wrapper around the in-app
        // DebugServer (127.0.0.1:5321) JSON-RPC. DEBUG-only — Release builds
        // ship neither the DebugServer nor this handler, so the
        // `/usr/local/bin/jarvis-debug` stub is also absent (PRootKernel.
        // installHandlerStubs enumerates currently-registered handlers).
        if (BuildConfig.DEBUG) {
            NativeOffloadServer.register(
                "jarvis-debug",
                com.jarvis.app.sandbox.offload.DebugOffloadHandler(this),
            )
        }
        ...
        com.jarvis.app.service.SessionBadgeStore.init(this)
        ...
        kotlinx.coroutines.CoroutineScope(kotlinx.coroutines.Dispatchers.IO).launch {
            ...
            com.jarvis.app.service.SessionBadgeStore.reconcileInterruptedSessions(interrupted - active)
        }
        ...
        // notifier into the (Context-free) gate, so a jarvis-config approval that
        ...
        val configConfirmNotifier = com.jarvis.app.notification.ConfigConfirmNotifier(
            context = this,
            backgroundSettings = backgroundSettingsRepository,
            isAppForeground = ::isAppForeground,
        )
        com.jarvis.app.config.confirm.ConfigConfirmationGate.backgroundNotifier = {
            configConfirmNotifier.notifyIfBackgrounded(it)
        }
        com.jarvis.app.config.confirm.ConfigConfirmationGate.cancelNotification = {
        ...
        registerActivityLifecycleCallbacks(object : ActivityLifecycleCallbacks {
            ...
            override fun onActivityResumed(activity: Activity) {
                ...
                com.jarvis.app.crash.CrashFrequencyDetector.maybeShowOnActivity(activity)
            ...
            override fun onActivityStopped(activity: Activity) {
                ...
                if (foregroundActivityCount == 0) {
                    ...
                    com.jarvis.app.config.confirm.ConfigConfirmationGate.notifyPending()
                }
            }
        ...
        com.jarvis.app.speech.SpeechRecognitionManager.init(this)
        ...
        if (BuildConfig.DEBUG) {
            try {
                com.jarvis.app.debug.DebugServer(this).start()
            } catch (e: Exception) {
        ...
        // system Clock app. Pre-T266 builds wrote alarms into Jarvis's own
        // SharedPreferences + AlarmManager; T266 retired that path but old
        // installs still have ghost entries that fire only inside Jarvis.
     ...
     * jarvis_alarms_prefs through SET_ALARM / SET_TIMER, then clear the
    ...
    private fun migrateGhostAlarms() {
        val prefs = getSharedPreferences("jarvis_alarms_prefs", Context.MODE_PRIVATE)
    ...
    /**
     * Coil global ImageLoader — registers [MinisImageFetcher] so `jarvis://`
     * URIs in Markdown images (e.g. `![alt](jarvis://attachments/x.png)`)
     * resolve to local files under /var/jarvis/.
     */
    override fun newImageLoader(): ImageLoader =
        ImageLoader.Builder(this)
            .components {
                ...
                // jarvis://attachments/foo.jpg invalidate Coil's cached bitmap.
    ...
    override fun onTrimMemory(level: Int) {
        ...
        runCatching { com.jarvis.app.ui.chat.KatexWebViewPool.evictAll() }
            .onFailure { Log.w("MinisApp", "KatexWebViewPool.evictAll failed: ${it.message}") }
        runCatching { com.jarvis.app.ui.markdown.KaTeXRendererCache.evictAll() }
        ...
        if (level >= TRIM_MEMORY_COMPLETE) {
            ...
            runCatching { com.jarvis.app.ui.chat.KatexWebViewPool.releaseWebView() }
    ...
    override fun onTerminate() {
        ...
        try {
            com.jarvis.app.diagnostics.LaunchCycleBeacon.recordCleanExit(this)
        } catch (t: Throwable) {
