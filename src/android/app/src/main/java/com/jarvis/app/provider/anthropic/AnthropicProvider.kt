...
class AnthropicProvider(
    ...
    internal fun resolveSystemPrompt(userPrompt: String?): JSONArray? {
        val claudeCodePrefix = com.jarvis.app.auth.ClaudeOAuthManager.ANTHROPIC_OAUTH_IDENTIFIER_PROMPT
        if (isOAuth) {
    ...
    private fun buildRequest(bodyStr: String, body: JSONObject): Request {
        ...
        // the branded Jarvis UA on the regular apiKey path, but on the OAuth
        ...
        builder.applyUserAgentOverride(
            customUserAgent,
            defaultUserAgent = if (isOAuth) null else com.jarvis.app.provider.MinisUserAgent.DEFAULT,
        )
