...
object MountedFolderCoordinator {
    ...
    private const val MOUNTS_PREFIX = "/var/jarvis/mounts/"

    /**
     * Throws [ReadOnlyMountException] when [linuxPath] is inside an
     * effectively read-only mount. Paths outside `/var/jarvis/mounts/` and
    ...
    /**
     * True when [linuxPath] is `/var/jarvis/mounts/<name>/...` AND the
    ...
    fun isLinuxPathUnderReadOnlyMount(linuxPath: String, store: MountedFoldersStore): Boolean {
        ...
        // `/var/jarvis/mounts/foobar` as inside a mount named `foo`.
