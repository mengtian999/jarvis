...
import com.jarvis.app.config.audit.ConfigAuditActor
import com.jarvis.app.config.audit.ConfigAuditEntry
import com.jarvis.app.config.audit.ConfigAuditLog
import com.jarvis.app.config.audit.ConfigAuditStatus
import com.jarvis.app.config.confirm.ConfigConfirmationGate
import com.jarvis.app.config.confirm.ConfirmOutcome
import com.jarvis.app.config.confirm.PendingConfigChange
import com.jarvis.app.config.confirm.PendingConfigChangeItem
 ...
 * in [com.jarvis.app.sandbox.NativeOffloadResult]. The handler
...
object ConfigBridge {
    ...
    /** Standardised "jarvis-config disabled" envelope. Includes user_message. */
    fun disabledErrorEnvelope(): JSONObject = JSONObject().apply {
        put("ok", false)
        put("error", "permission_denied")
        put("reason", "jarvis-config is disabled in Settings → Permissions.")
        put(
            "user_message",
            "I tried to change a setting but jarvis-config is currently disabled. " +
                "You can enable it at [Settings → Permissions](jarvis://settings/permissions), " +
    ...
    private fun readFieldRaw(path: String): JSONObject {
        ...
        if (field.access == ConfigAccess.HIDDEN) return JSONObject().apply {
            put("ok", false)
            put("error", "permission_denied")
            put("reason", "'$path' is intentionally not exposed to jarvis-config.")
        }
        ...
        return try {
            ...
            // [T-jarvis-config-provider-add] Forward PermissionDenied verbatim
    ...
    private fun paginationHint(
        ...
        if (page > totalPages) {
            ...
            return "Page $page is out of range (only $totalPages $pgWord available). " +
                "Try: jarvis-config get $path$filterFragment --page $totalPages"
        }
        if (totalPages <= 1) {
        ...
        if (page < totalPages) {
            return "Showing page $page of $totalPages ($pageCount of $total items). " +
                "To get more, use: jarvis-config get $path$filterFragment --page ${page + 1} --page-size $pageSize"
        }
    ...
    suspend fun performWriteBatch(
        ...
        for (i in 0 until items.length()) {
            ...
            if ((isAppend || isRemove) && ConfigRegistry.get().collection(resolvePath) != null) {
                ...
                // [T-jarvis-config-provider-add] Redact credential values
            ...
            if (field.access != ConfigAccess.READWRITE) {
                val reason = if (field.access == ConfigAccess.HIDDEN) {
                    "'$rawPath' is intentionally not exposed to jarvis-config."
                } else {
            ...
            // [T-jarvis-config-provider-add] Mask credential writes the
            ...
            is ConfirmOutcome.Approved -> {
                ...
                for ((idx, row) in outcome.items.withIndex()) {
                    ...
                    try {
                        ...
                        if (r.collection != null && r.collectionVerb != null) {
                            ...
                            // [T-jarvis-config-provider-add] If this is a
                ...
                if (applied.length() == 0 && earlyError != null) {
                    ...
                    JSONObject().apply {
                        ...
                        put("audit_url", "jarvis://settings/logs?tab=config-audit")
                        put("user_message", "Settings updated. Review or revert at [Logs → Config Changes](jarvis://settings/logs?tab=config-audit).")
                    }
                }
            }
        }
    ...
    private data class Resolved(
        ...
        val newValue: ConfigValue,
        // [T-jarvis-config-provider-add] When non-null, audit / display
    ...
    fun auditRevert(
        ...
        val res = runBlocking {
            performWriteBatch(
                items = items,
                caption = "jarvis-config audit revert ${entry.id.take(8)}",
                actorRaw = actorRaw,
                sessionId = sessionId,
