...
/**
 * Coil Fetcher that resolves `jarvis://` URIs to local files.
 *
 * Usage: Register with ImageLoader.Builder().components {
 *     add(MinisImageFetcher.Factory())
 * }
 *
 * jarvis://attachments/foo.jpg → /var/jarvis/attachments/foo.jpg → host path
 */
class MinisImageFetcher(
    ...
    override suspend fun fetch(): FetchResult {
        // Strip jarvis:// prefix, drop any ?query, and percent-decode so
        // Chinese/emoji/space filenames resolve to the actual on-disk file.
        val stripped = uri.removePrefix("jarvis://").substringBefore('?')
    ...
    class Factory : Fetcher.Factory<String> {
        override fun create(data: String, options: Options, imageLoader: ImageLoader): Fetcher? {
            if (!data.startsWith("jarvis://")) return null
     ...
     * StringMapper converts an `AsyncImage(model = "jarvis://…")` String into
     ...
     * `jarvis://` file is overwritten in-place (e.g. Grok regenerating an
     ...
     *
     * The key composes `<jarvis-uri>?mt=<lastModified>`. The fetcher above
