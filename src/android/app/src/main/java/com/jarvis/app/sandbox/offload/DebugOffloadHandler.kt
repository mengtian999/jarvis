...
/**
 * jarvis-debug — DEBUG-ONLY CLI wrapper for the local DebugServer JSON-RPC
...
class DebugOffloadHandler(@Suppress("UNUSED_PARAMETER") context: Context) : NativeOffloadHandler {
    ...
    override fun handle(request: NativeOffloadRequest): NativeOffloadResult {
        ...
        // Per-subcommand --help: `jarvis-debug ls --help` prints the same
        ...
        val rpc: RpcCall = try {
            ...
            return NativeOffloadResult(2, "jarvis-debug $sub: ${e.message}\n")
        }
    ...
    private fun buildRpcCall(sub: String, args: OffloadArgs): RpcCall {
        ...
        return when (sub) {
            ...
            // `jarvis-debug call <method> [--params '<json>']`
            "call" -> {
            ...
            else -> throw BadArgsException("unknown subcommand '$sub' (try `jarvis-debug --help`)")
        }
    }
    ...
    companion object {
        ...
        private const val HELP = """jarvis-debug — CLI for the in-app DebugServer JSON-RPC (debug builds only)

Usage:
  jarvis-debug <subcommand> [options]
...
Android-only (DEBUG_ONLY_METHODS in DebugMethodRegistry):
  ...
  model-use <argv...>                Invoke jarvis-model-use (debug.modelUse.exec)
...
EXAMPLES:
  jarvis-debug discover
  jarvis-debug appInfo
  jarvis-debug ls /sdcard --recursive --maxDepth 2
  jarvis-debug read /etc/passwd --offset 0 --limit 100
  jarvis-debug exec uname -a
  jarvis-debug call debug.logs.list --params '{}'
