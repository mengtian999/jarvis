 ...
 *
 * Routing order:
 *  1. Recognized jarvis:// deep-link action  → DeepLink (delegated to MainActivity via Intent.ACTION_VIEW)
 *  2. jarvis://<sandbox path>, file://, or absolute /var/jarvis|/root path → SandboxFile
...
object ChatLinkResolver {
    ...
    fun resolve(rawUrl: String, sessionId: String? = null, context: Context? = null): ChatLinkAction {
        ...
        // 1. jarvis:// deep links — only branch out when the URL maps to a known action,
        ...
        if (com.jarvis.app.ui.browser.BrowserExternalSchemeHandler.shouldHandleExternally(trimmed)) {
     ...
     * Accepts:
     *   jarvis://attachments/foo.png        → /var/jarvis/attachments/foo.png
     *   jarvis:///var/jarvis/workspace/x.csv → /var/jarvis/workspace/x.csv (absolute)
     *   file:///path/to/file               → /path/to/file
     *   /var/jarvis/workspace/x.csv         → resolved via bind mount
    ...
    private fun resolveSandboxFile(
        ...
        return when (scheme) {
            "jarvis" -> {
                ...
                // `jarvis://` URLs don't use fragments, so stripping at '#'
                // would truncate filenames like `foo #China.mp4`.
                val stripped = raw.removePrefix("jarvis://").substringBefore('?')
                // [T-android-jarvis-url-double-encoding] Try each decode
                ...
                minisPathCandidates(stripped)
                    .asSequence()
                    .map { candidate ->
                        val linuxPath = if (candidate.startsWith("/")) candidate else "/var/jarvis/$candidate"
                        lookup(linuxPath)
                    }
                    ...
                    ?: lookup(
                        minisPathCandidates(stripped).first().let {
                            if (it.startsWith("/")) it else "/var/jarvis/$it"
                        },
                    )
            }
            "file" -> {
    ...
    /**
     * [T-android-jarvis-url-double-encoding] Decode candidates for the path part
     * of a `jarvis://` URL, in priority order. Ported from iOS
     ...
     * and a web preview of a `jarvis://` URL renders nothing at all. There is
