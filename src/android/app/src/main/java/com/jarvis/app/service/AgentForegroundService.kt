...
class AgentForegroundService : Service() {
    ...
    companion object {
        ...
        private const val ACTION_STOP = "com.jarvis.app.STOP_AGENT_SERVICE"
    ...
    private fun buildNotification(sessionCount: Int, toolStatus: String): Notification {
        ...
        val mainIntent = Intent(this, Class.forName("com.jarvis.app.MainActivity")).apply {
        ...
        // Title:   "Jarvis is using <Tool>"  (or session-count summary when idle/between turns)
        ...
        // through to the generic "Jarvis is running" title while the icon fell
    ...
    private fun toolDisplayLabel(toolName: String): String = when (toolName) {
        "shell_execute" -> "Jarvis is using Shell"
        "file_read" -> "Jarvis is reading File"
        "file_write" -> "Jarvis is using Editor"
        "file_edit" -> "Jarvis is editing File"
        "browser_use" -> "Jarvis is using Browser"
        "read_image" -> "Jarvis is reading Image"
        "memory_write", "memory_get" -> "Jarvis is using Memory"
        "web_search" -> "Jarvis is using Search"
        else -> "Jarvis is using $toolName"
    }
