...
import com.jarvis.app.data.db.ChatDao
...
import com.jarvis.app.data.model.ModelAttributionSnapshot
...
class ChatRepository(internal val dao: ChatDao) {
    ...
    suspend fun createSession(
        ...
        // [T-role-session-bound] Persona bound to the new session —
        // determines the personality for subsequent replies. Immutable
        // in-session. Callers pass the active role id (global current role
        // for a fresh draft, or an explicit initialRoleId when opening a
        // role-specific new chat). Null = legacy/abnormal; new sessions
        // should always seed a role.
        roleId: String? = null,
    ): ChatSessionEntity {
     ...
     * from [com.jarvis.app.data.model.ContentPart]: "toolUse" / "toolResult"
     ...
     * [com.jarvis.app.ui.chat.ChatViewModel.rerunFromToolBlock]'s block-
    ...
    suspend fun appendMessage(
        ...
        // [T-role-message-level] Persona that generated this assistant
        // message. Only assistant rows carry a non-null roleId; callers
        // persisting user/system messages leave it null. Mirrors the
        // message-level role binding so history renders each role correctly
        // even after the global current role changes.
        roleId: String? = null,
    ...
    // ───────────────── T188: jarvis-sessions-cli backend ─────────────────
    //
    ...
    /**
     * Backs `jarvis-sessions-cli list`. Returns sessions ordered by
    ...
    /**
     * Backs `jarvis-sessions-cli search`. Over-fetches `limit * 3` rows
    ...
    /**
     * Backs `jarvis-sessions-cli messages --id ... --offset --limit`.
    ...
    suspend fun loadMessagePage(
        ...
        // documented 600; `jarvis-sessions-cli messages --full` passes
    ...
    /**
     * Paginated raw [MessageEntity] page — used by [com.jarvis.app.share.ChatExporter]
...
/** T188: shape of a session row surfaced to `jarvis-sessions-cli list`. */
...
/** T188: a single matching message returned by `jarvis-sessions-cli search`. */
