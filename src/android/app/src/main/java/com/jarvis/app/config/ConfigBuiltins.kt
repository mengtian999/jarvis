...
import com.jarvis.app.config.collections.EnvVarsCollection
import com.jarvis.app.service.DynamicIslandSupport
import com.jarvis.app.config.collections.GroupsCollection
import com.jarvis.app.config.collections.ModelsCollection
import com.jarvis.app.config.collections.ProvidersCollection
import com.jarvis.app.config.fields.ClosureField
import com.jarvis.app.config.fields.PrefsBoolField
import com.jarvis.app.config.fields.PrefsDoubleField
import com.jarvis.app.config.fields.PrefsEnumField
import com.jarvis.app.config.fields.PrefsIntCodedEnumField
import com.jarvis.app.config.fields.PrefsIntField
import com.jarvis.app.config.fields.PrefsLongField
import com.jarvis.app.config.fields.PrefsStringField
import com.jarvis.app.config.fields.ReadOnlyField
...
import com.jarvis.app.ui.chat.ChatViewModelStore
...
internal object ConfigBuiltins {
    ...
    // MemoryGlobalPrefs: SharedPreferences `jarvis_memory_prefs`, key
    ...
    private fun registerMemory(r: ConfigRegistry, context: Context) {
        val prefs = context.getSharedPreferences("jarvis_memory_prefs", Context.MODE_PRIVATE)
        r.register(
            PrefsBoolField(
                path = "memory.enabled",
    ...
    private fun registerSelfMeta(r: ConfigRegistry) {
        r.register(
            ClosureField(
                path = "permissions.jarvisConfig.enabled",
                displayName = "Allow jarvis-config",
     ...
     *  `entry:<uuid>` / `group:<id>` form the iOS jarvis-config surface uses.
    ...
    private fun registerAppearance(r: ConfigRegistry, context: Context) {
        ...
        r.register(
            PrefsBoolField(
                ...
                prefs = prefs,
                key = com.jarvis.app.ui.settings.KEY_SHOW_CHAT_TITLE,
                defaultValue = true,
            )
        )
        r.register(
    ...
    private fun registerChat(r: ConfigRegistry, context: Context) {
        ...
        // live here, not in `jarvis_settings`. Hoisted above the registrations
        // so every field can bind to the same store the UI reads.
        val appearancePrefs = context.getSharedPreferences(
            com.jarvis.app.ui.settings.PREF_APPEARANCE,
        ...
        r.register(
            PrefsIntCodedEnumField(
                ...
                // `jarvis_settings/return_key_behavior`, which nothing reads,
                // so `jarvis-config set chat.returnKey send` reported success
                // while the chat box kept inserting newlines.
                prefs = appearancePrefs,
                key = com.jarvis.app.ui.settings.KEY_RETURN_KEY_BEHAVIOR,
        ...
        r.register(
            PrefsBoolField(
                ...
                prefs = appearancePrefs,
                key = com.jarvis.app.ui.settings.KEY_KEEP_SCREEN_AWAKE,
                defaultValue = false,
            )
        )
        ...
        // reads/writes, so flipping via jarvis-config flows back into the
        ...
        r.register(
            PrefsBoolField(
                ...
                prefs = appearancePrefs,
                key = com.jarvis.app.ui.settings.KEY_TOOL_PREVIEW,
                defaultValue = true,
            )
        )
        ...
        r.register(
            PrefsBoolField(
                ...
                prefs = appearancePrefs,
                key = com.jarvis.app.ui.settings.KEY_AUTO_FOCUS_AFTER_REPLY,
                defaultValue = true,
            )
        )
        r.register(fontScaleField(
            ...
            prefs = appearancePrefs,
            key = com.jarvis.app.ui.settings.KEY_FONT_CHAT_INPUT,
        ))
        r.register(fontScaleField(
            ...
            prefs = appearancePrefs,
            key = com.jarvis.app.ui.settings.KEY_FONT_MESSAGE,
        ))
    }
    ...
    private fun registerBackground(r: ConfigRegistry, context: Context) {
        ...
        // "unsupported" footer unless the device is capable, so jarvis-config
    ...
    private fun registerLogs(r: ConfigRegistry, context: Context) {
        ...
        // jarvis-config did nothing. Default mirrors AppLogger's own default
        ...
        r.register(
            ClosureField(
                ...
                revertable = true,
                reader = {
                    ConfigValue.Bool(com.jarvis.app.logging.AppLogger.isEnabled(context))
                },
                writer = { v ->
                    val b = (v as? ConfigValue.Bool)?.value
                        ?: throw ConfigError.TypeMismatch("bool")
                    com.jarvis.app.logging.AppLogger.setEnabled(context, b)
                },
            )
        )
    }
    ...
    private fun registerProviderCollections(
        ...
        r.register(com.jarvis.app.config.collections.ThinkingRulesCollection(providerRepo))

        val isoFormatter = SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss'Z'", Locale.US).apply {
            timeZone = TimeZone.getTimeZone("UTC")
        }

        // Aggregate read-only summary so `jarvis-config get providers`
        ...
        r.register(
            ReadOnlyField(
                ...
                description = "Read-only list of configured providers (id, label, type, credential type, enabled, base URL). Credentials are not included. Use `jarvis-config add providers <json>` to create a new provider (payload mirrors a provider-export `config` block, with optional `apiKey` as literal or `\$\$ENV_VAR`).",
        ...
        // Aggregate read-only summary so `jarvis-config get models`
        ...
        r.register(
            ReadOnlyField(
                ...
                reader = {
                    val cfg = providerRepo.config.value
                    val providersById = HashMap<String, com.jarvis.app.data.model.ProviderInstance>(cfg.instances.size)
        ...
        // Aggregate read-only summary so `jarvis-config get envvars`
        ...
        r.register(
            ClosureField(
                ...
                risk = ConfigRisk.SENSITIVE,
                revertable = true,
                reader = { ConfigValue.Bool(com.jarvis.app.data.EnvVarPrivacyStore.isEnabled) },
                writer = { v ->
                    val b = (v as? ConfigValue.Bool)?.value
                        ?: throw ConfigError.TypeMismatch("bool")
                    com.jarvis.app.data.EnvVarPrivacyStore.setEnabled(b)
                },
            )
        )

        // Aggregate read-only summary so `jarvis-config get groups`
        ...
        r.register(
            ReadOnlyField(
                ...
                reader = {
                    val cfg = providerRepo.config.value
                    val entriesById =
                        HashMap<String, com.jarvis.app.data.model.ModelEntry>(cfg.modelEntries.size)
                    for (e in cfg.modelEntries) entriesById[e.id] = e
                    val providersById =
                        HashMap<String, com.jarvis.app.data.model.ProviderInstance>(cfg.instances.size)
                    for (inst in cfg.instances) providersById[inst.id] = inst
                    ConfigValue.Arr(
    ...
    private fun registerDefaults(r: ConfigRegistry, repo: ProviderRepository) {
        ...
        r.register(
            ClosureField(
                ...
                description = "Model entries available via jarvis-model-use. Replace with full list to set; use .append/.remove for single-element ops.",
                ...
                reader = {
                    ...
                    // rows). Surfacing them confuses jarvis-model-use and the
        ...
        r.register(
            ClosureField(
                ...
                description = "Whole groups exposed via jarvis-model-use.",
                valueSchema = ConfigSchema.Array(ConfigSchema.Str()),
                risk = ConfigRisk.SENSITIVE,
                revertable = true,
                reader = {
    ...
    private fun resolveSoulIconImage(context: Context, raw: String): String {
        val icon = com.jarvis.app.agent.SoulIcon
        val bytes: ByteArray = when (val src = icon.classifySource(raw)) {
            is com.jarvis.app.agent.SoulIcon.Source.Unsupported ->
                throw ConfigError.InvalidValue(src.reason)

            is com.jarvis.app.agent.SoulIcon.Source.Bytes -> src.data

            is com.jarvis.app.agent.SoulIcon.Source.LinuxPath -> {
                ...
                val kernel = com.jarvis.app.sandbox.PRootKernel
        ...
        return when (val r = icon.encode(bitmap)) {
            is com.jarvis.app.agent.SoulIcon.EncodeResult.Success -> r.dataUri
            is com.jarvis.app.agent.SoulIcon.EncodeResult.Failure -> when (r.reason) {
                com.jarvis.app.agent.SoulIcon.Rejection.TOO_LARGE ->
                    throw ConfigError.InvalidValue("that image is too large to store inline")
                com.jarvis.app.agent.SoulIcon.Rejection.UNREADABLE ->
    ...
    private fun registerSoul(r: ConfigRegistry, context: Context) {
        ...
        // English ≤ 500 words); see [com.jarvis.app.agent.SoulStore.isOverLimit].
        ...
        // UI don't race with jarvis-config writes. Parse falls back to
        // the canonical default content if the file was somehow deleted
        // between launches — same behavior as the Settings page.
        fun loadCurrent(): com.jarvis.app.agent.SoulFile =
            com.jarvis.app.agent.SoulStore.load(context)
                ?: com.jarvis.app.agent.SoulMDParser.parse(
                    com.jarvis.app.agent.SoulStore.DEFAULT_CONTENT
                )

        fun saveCurrent(file: com.jarvis.app.agent.SoulFile) {
            com.jarvis.app.agent.SoulStore.save(context, file)
        }
        ...
        r.register(
            ClosureField(
                ...
                description = "The agent's name as shown in chat bubble headers and as the system-prompt identity. Trimmed; empty falls back to \"Jarvis\".",
                ...
                writer = { v ->
                    ...
                    val name = raw.trim().ifBlank {
                        com.jarvis.app.agent.SoulMetadata.DEFAULT.name
                    }
                    val cur = loadCurrent()
        ...
        // (jarvis:// or a /var/jarvis path) is only an import source: it is
        ...
        // "<image>" so `jarvis-config get` cannot flood the context with
        ...
        r.register(
            ClosureField(
                ...
                // The description IS the API doc — `jarvis-config topic-help
                ...
                description = "The Soul's identity icon — also called its avatar, its persona image, or its " +
                    ...
                    "  • jarvis:// resource, e.g. jarvis://attachments/icon.png\n" +
                    "  • a path inside the minis directories, e.g. /var/jarvis/attachments/icon.png\n" +
                    ...
                    "square, downscaled to ${com.jarvis.app.agent.SoulIcon.STORED_PIXELS}×" +
                    "${com.jarvis.app.agent.SoulIcon.STORED_PIXELS}, re-encoded as PNG and stored inline.\n" +
                    "\n" +
                    ...
                    "  jarvis-config set soul.icon '\"⚡\"'\n" +
                    "  jarvis-config set soul.icon '\"jarvis://attachments/icon.png\"'\n" +
                    "  jarvis-config set soul.icon '\"\"'   # back to the default sparkle",
                ...
                reader = {
                    ...
                    ConfigValue.Str(
                        if (com.jarvis.app.agent.SoulIcon.isDataUri(raw)) "<image>" else raw,
                    )
                },
                writer = { v ->
                    val raw = (v as? ConfigValue.Str)?.value
                    ...
                    val next = when {
                        ...
                        com.jarvis.app.agent.SoulIcon.graphemeClusters(trimmed).size == 1 &&
                            com.jarvis.app.agent.SoulIcon.isEmojiGlyph(trimmed) -> trimmed
            ...
            ClosureField(
                ...
                reader = {
                    ...
                    ConfigValue.Str(cur.metadata.lang.ifBlank {
                        com.jarvis.app.agent.SoulMetadata.DEFAULT.lang
                    })
                },
                writer = { v ->
                    val raw = (v as? ConfigValue.Str)?.value
        ...
        r.register(
            ClosureField(
                ...
                description = "Personality / voice instructions injected as a block in the system prompt. Length cap is language-aware: Chinese ≤ ${com.jarvis.app.agent.SoulStore.CHINESE_CHAR_LIMIT} chars OR English ≤ ${com.jarvis.app.agent.SoulStore.ENGLISH_WORD_LIMIT} words (rule picked by the majority language). The writer also rejects prompt-injection patterns (\"ignore previous instructions\" etc.) — keep this to genuine character / tone guidance.",
                ...
                writer = { v ->
                    val raw = (v as? ConfigValue.Str)?.value
                        ?: throw ConfigError.TypeMismatch("string")
                    if (com.jarvis.app.agent.SystemPromptBuilder
                    ...
                    val check = com.jarvis.app.agent.SoulStore.isOverLimit(raw)
                    when (check) {
                        is com.jarvis.app.agent.SoulBodyLimitCheck.Ok -> Unit
                        is com.jarvis.app.agent.SoulBodyLimitCheck.OverLimitChinese ->
                            throw ConfigError.InvalidValue(
                                "Over limit: ${check.chars} Chinese chars exceeds cap of ${check.cap}. Either trim to ≤ ${check.cap} chars, or rewrite mostly in English to use the ≤ ${com.jarvis.app.agent.SoulStore.ENGLISH_WORD_LIMIT}-word cap."
                            )
                        is com.jarvis.app.agent.SoulBodyLimitCheck.OverLimitEnglish ->
                            throw ConfigError.InvalidValue(
                                "Over limit: ${check.words} English words exceeds cap of ${check.cap}. Either trim to ≤ ${check.cap} words, or rewrite mostly in Chinese to use the ≤ ${com.jarvis.app.agent.SoulStore.CHINESE_CHAR_LIMIT}-char cap."
                            )
                    }
                    val cur = loadCurrent()
