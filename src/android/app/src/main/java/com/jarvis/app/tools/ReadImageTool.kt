...
object ReadImageTool {
    ...
    fun definition(): AgentToolDefinition = AgentToolDefinition(
        ...
        parameters = mapOf(
            ...
            "path" to AgentToolParam("string", "Linux path (e.g. /var/jarvis/attachments/chart.png) or jarvis:// URL (e.g. jarvis://attachments/chart.png)"),
     ...
     * (`/var/jarvis/{attachments,workspace,offloads,browser}/...`) resolve
    ...
    fun execute(argsJson: String, sessionId: String? = null, context: Context? = null): ToolExecutionResult {
        return try {
            ...
            val path = if (rawPath.startsWith("jarvis://")) {
                "/var/jarvis/" + java.net.URLDecoder.decode(rawPath.removePrefix("jarvis://"), "UTF-8")
            } else rawPath
