#!/usr/bin/env python3
"""jarvis-mcp-cli — MCP client CLI for the Minis agent (iSH / PRoot).
...
Diagnostics go to /var/jarvis/mcp-servers/mcp-cli.log, never to stdout.
...
published in /tmp/jarvis-mcp-daemon.port. info / add / remove / enable / disable
...
LOG_PATH = "/var/jarvis/mcp-servers/mcp-cli.log"

PID_FILE = "/tmp/jarvis-mcp-daemon.pid"
PORT_FILE = "/tmp/jarvis-mcp-daemon.port"  # daemon publishes its 127.0.0.1 port here
LOCK_FILE = "/tmp/jarvis-mcp-daemon.lock"  # cold-start fork guard (one winner forks)
...
def maybe_start_daemon():
    ...
    try:
        ...
        import logging
        daemon_log = "/var/jarvis/mcp-servers/mcp-daemon.log"
        try:
...
USAGE = """jarvis-mcp-cli — MCP (Model Context Protocol) client for the Minis agent.

Usage: jarvis-mcp-cli <command> [args] [--pretty]
...
Files:
  Servers:  /var/jarvis/mcp-servers/servers.json   (mcpServers object, Claude-Desktop compatible)
  Log:      /var/jarvis/mcp-servers/mcp-cli.log

Examples:
  jarvis-mcp-cli list --pretty
  jarvis-mcp-cli tools notion
  jarvis-mcp-cli call notion search --input '{"q":"x"}'
  jarvis-mcp-cli add --name notion --url https://mcp.notion.so/mcp --header "Authorization: Bearer $NOTION_TOKEN"
  jarvis-mcp-cli add --name github --command npx --args "-y @modelcontextprotocol/server-github" --env "GITHUB_TOKEN=$GITHUB_TOKEN"
  jarvis-mcp-cli add --name atlassian --command uvx --args "mcp-atlassian" --startup-timeout 120
  jarvis-mcp-cli add --name gworkspace --url https://my-gws-mcp.example.com/mcp \
...
def main():
    ...
    try:
        if cmd == "list":
            ...
            sys.stderr.write("Run 'jarvis-mcp-cli --help' for usage.\n")
