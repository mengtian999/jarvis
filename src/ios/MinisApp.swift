...
struct MinisApp: App {
    ...
    /// [review S14] Drives the root-level restore sheet for a `.jarvisbak`
    ...
    init() {
        ...
        // first render instead of the `.default` stub ("Jarvis"). Without this the
            ...
            ZStack {
                ...
                .onReceive(SessionLockStore.shared.$appIsLocked) { locked in
                    ...
                    if BackupOpenRouter.handle(url) {
                        // .jarvisbak → restore flow, not the attachment pipeline.
                ...
                // [review S14] Restore flow for a `.jarvisbak` opened from
                ...
                .sheet(item: Binding(
                    ...
                    NavigationStack {
                        ...
                        // loaded. Someone who just tapped a .jarvisbak is mid
                ...
                // for `jarvis://open?session=…&path=…`). Mounted at the
                ...
                .onAppear {
                    ...
                    // which is bind-mounted into the guest at /var/jarvis/shared
                    ...
                    // Create /var/jarvis/mounts/<name> symlinks in the fakefs now
    ...
    /// `jarvis://open?session=…&path=…` deep link (openminis.app launcher
