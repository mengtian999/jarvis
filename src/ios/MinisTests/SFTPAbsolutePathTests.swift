...
///     `http://host/backup-….jarvisbak` instead of `…/backups/…`, which is the
...
final class SFTPAbsolutePathTests: XCTestCase {
    ...
    func testSFTPJoinKeepsLeadingSlash() {
        XCTAssertEqual(remote(backend: "sftp", path: "/srv/backup").join("pkg.jarvisbak"),
                       "/srv/backup/pkg.jarvisbak")
    }
