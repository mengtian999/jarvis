...
/// the CLI-level bounded backoff for `Jarvis.LLMError.transientError` (Gemini
...
final class ModelUseTransientRetryTests: XCTestCase {
    ...
    private func transient(_ n: Int) -> Jarvis.LLMError {
    ...
    func testTransientErrorExhaustsRetriesThenThrows() async {
        ...
        } catch let Jarvis.LLMError.transientError(message) {
    ...
    func testProviderErrorFailsFastWithoutRetry() async {
        ...
        do {
            _ = try await ModelUseOffloadBridge.withTransientRetry(
                ...
                attempts += 1
                throw Jarvis.LLMError.providerError(message: "400 INVALID_ARGUMENT")
            }
            XCTFail("expected providerError")
        } catch let Jarvis.LLMError.providerError(message) {
    ...
    /// Non-Jarvis.LLMError failures are also untouched (no retry, error passes
    /// through unchanged).
    ...
    func testCanRetryFalseSurfacesTransientErrorImmediately() async {
        ...
        } catch Jarvis.LLMError.transientError {
            // expected
        } catch {
            XCTFail("unexpected error type: \(error)")
        }
