...
final class CorrectionAdmissionTests: XCTestCase {
    ...
    func testLatinPhoneticSimilarity() {
        ...
        XCTAssertFalse(f("jarvis", "minutes"))   // 2 vs 3 — a rewrite, correctly dropped
    ...
    func testLinuxMinisAdmitted() {
        ...
        XCTAssertEqual(judge("linux", "jarvis"), .latinPhonetic)
    ...
    func testRealDataAdmissionMatrix() {
        let shouldCollect: [(String, String)] = [
            ...
            ("linux", "jarvis"),   // Signal 4 — the GH report; nothing else catches it
        ]
