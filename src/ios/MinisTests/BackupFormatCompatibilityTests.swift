...
/// Regression tests for `.jarvisbak` format acceptance and manifest tolerance
...
final class BackupFormatCompatibilityTests: XCTestCase {
    ...
    func testFormatAcceptanceMatrix() {
        ...
        XCTAssertTrue(BackupImporter.isFormatSupported("jarvisbak/1"))
        // Same major with a minor component — §2.2 rule 2 requires this to
        // import. The pre-fix code REFUSED it (Int("1.1") == nil).
        XCTAssertTrue(BackupImporter.isFormatSupported("jarvisbak/1.1"))
        XCTAssertTrue(BackupImporter.isFormatSupported("jarvisbak/1.0.3"))
        ...
        XCTAssertFalse(BackupImporter.isFormatSupported("jarvisbak/2"))
        XCTAssertFalse(BackupImporter.isFormatSupported("jarvisbak/2.0"))
    }

    func testFormatAcceptanceRejectsMalformedStrings() {
        XCTAssertFalse(BackupImporter.isFormatSupported(""))
        XCTAssertFalse(BackupImporter.isFormatSupported("jarvisbak"))
        XCTAssertFalse(BackupImporter.isFormatSupported("jarvisbak/"))
        XCTAssertFalse(BackupImporter.isFormatSupported("jarvisbak/x"))
        XCTAssertFalse(BackupImporter.isFormatSupported("/1"))
        // Case matters: the prefix is compared literally.
        XCTAssertFalse(BackupImporter.isFormatSupported("jarvisbak/1"))
    }
    ...
    func testUnknownFieldsAreIgnored() throws {
        let m = try decode("""
        {"format":"jarvisbak/1","device_name":"iPhone",
         "future_field":{"nested":[1,2,3]},"another":"x"}
        """)
        XCTAssertEqual(m.deviceName, "iPhone")
        XCTAssertEqual(m.format, "jarvisbak/1")
    }
    ...
    func testSnakeCaseKeysAreTheWireFormat() throws {
        let m = try decode("""
        {"format":"jarvisbak/1","device_name":"A","backup_id":"B",
    ...
    func testKDFAlgAndSaltAreRequired() {
        let base = """
        {"format":"jarvisbak/1","encryption":{"scheme":"jarvisbak-enc/1",
         "verifier":"dg==","kdf":{%@}}}
        """
