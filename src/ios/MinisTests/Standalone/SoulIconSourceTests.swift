...
// accepts images (data URI, bare base64, jarvis:// resource, local path,
...
func looksLikeImageSource(_ raw: String) -> Bool {
    ...
    if s.hasPrefix("data:") || s.hasPrefix("jarvis://") { return true }
...
for (label, s, want) in [
    ...
    ("minis attachments", "jarvis://attachments/icon.png", true),
    ("minis workspace", "jarvis://workspace/icon.png", true),
    ("https", "https://example.com/icon.png", true),
    ("http", "http://example.com/icon.png", true),
    ("absolute path", "/var/jarvis/attachments/icon.png", true),
...
for needle in ["data:image/png;base64,iVBORw0KGgo",
               "jarvis://attachments/icon.png",
               "jarvis://workspace/icon.png",
               "/var/jarvis/attachments/icon.png",
...
check("help gives a runnable quoted example",
      builtinsSource.contains("minis-config set soul.icon '\\\"jarvis://attachments/icon.png\\\"'"))
