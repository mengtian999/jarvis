// Tests for [T-backup-package-name-device] — `.jarvisbak` filenames are now
// `<device>-<yyyyMMdd>-<sortable-id>.jarvisbak` so that packages from several
...
func packageFileName(backupId: String, at date: Date, deviceName: String,
    ...
    return "\(device)-\(stamp)-\(sortableID(backupId: backupId, at: date))\(suffix).jarvisbak"
}
...
do {
    ...
    check("ends with the extension", name.hasSuffix(".jarvisbak"))
    ...
    let fields = simple.replacingOccurrences(of: ".jarvisbak", with: "").split(separator: "-")
...
do {
    ...
    check("marker sits before the extension", enc.hasSuffix("-encrypted.jarvisbak"))
    ...
    check("still ends in .jarvisbak", enc.hasSuffix(".jarvisbak"))
    ...
    let fields = packageFileName(backupId: "abc", at: t, deviceName: "iPhone",
                                 encrypted: true)
        .replacingOccurrences(of: ".jarvisbak", with: "").split(separator: "-")
