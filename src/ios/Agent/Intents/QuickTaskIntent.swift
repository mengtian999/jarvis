...
/// e.g. "Jarvis analyze sleep" (or the same phrase pattern in the user's locale).
...
struct QuickTaskIntent: AppIntent {
    ...
    static var description = IntentDescription("Runs a predefined Jarvis task like sleep analysis, weather check, or morning briefing.")
    ...
    func perform() async throws -> some IntentResult & ReturnsValue<SendPromptResult> & ProvidesDialog {
        ...
        if sendCompletionNotification {
            ShortcutNotification.post(
                ...
                title: AppLocalized("Jarvis: \(taskName)"),
        ...
        if waitForResult {
            ...
            if sendCompletionNotification {
                ShortcutNotification.post(
                    ...
                    title: AppLocalized("Jarvis: \(taskName) Done"),
        ...
        Task { @MainActor in
            ...
            if capturedSendCompletionNotification {
                ShortcutNotification.post(
                    ...
                    title: AppLocalized("Jarvis: \(capturedTaskName) Done"),
