...
struct ChatSession: Identifiable, Codable, Hashable {
    ...
    /// [T-role-session-bound] The persona bound to this session at creation.
    /// Immutable in-session (switching personas = new session or forwarding).
    /// Nil = legacy/abnormal; new sessions should always seed a role. Synced
    /// across devices via iCloud (NOT device-local). Orphan refs (role
    /// deleted) render via RoleStore.loadDefault() — same strategy as
    /// folderId. Defaulted so pre-refactor construction sites keep compiling.
    ...
    static func == (lhs: ChatSession, rhs: ChatSession) -> Bool {
        lhs.id == rhs.id
            ...
            // [T-role-session-bound] Same rationale as folderId: a role edit
            // (avatar/name) changes the row's rendered identity without
            // bumping updatedAt, so compare the id too.
...
struct MediaRef: Codable, Hashable {
    ...
    /// `/var/jarvis/attachments/uploads/<name>`, `/var/jarvis/browser/<sid>/...`).
...
struct RawMessage: Identifiable, Codable, Hashable {
    ...
    /// message. Distinct from `role` (speaker kind: user/assistant/system).
    /// Only assistant rows carry a non-nil roleId; user/system rows are nil.
    /// Persisted to messages.role_id and synced via iCloud (NOT device-local
    /// like errorInfo). Orphan refs (role deleted) render via
    /// RoleStore.loadDefault() — same strategy as folderId.
    var roleId: String? = nil
...
actor ChatStore {
    ...
    init() {
        ...
        self.minisBaseURL = baseURL.appendingPathComponent("jarvis", isDirectory: true)
    ...
    private func createTables() {
        ...
        // [T-role-session-bound] Persona bound to the session (see
        // ChatSession.roleId). NULL = legacy/abnormal. Synced via iCloud.
        // Not a declared FK — same orphan-tolerance rationale as folder_id
        // (role records can arrive out of order; deleted roles leave refs
        // that render via the default-role fallback).
        addColumnIfMissing(table: "sessions", column: "role_id", definition: "TEXT")
        // [T-role-message-level] Per-message generating persona for assistant
        // rows (NULL for user/system). Synced via iCloud. See RawMessage.roleId.
        addColumnIfMissing(table: "messages", column: "role_id", definition: "TEXT")
    ...
    @discardableResult
    func createSession(modelId: String, title: String? = nil, source: String? = nil, roleId: String? = nil) -> ChatSession {
        invalidateSessionListCache()
        let now = Date()
        let session = ChatSession(
            ...
            updatedAt: now,
            source: source,
            // [T-role-session-bound] Seed the bound persona into the new row
            // so reloads + sync resolve it. Callers pass the global current
            // role for a plain new chat, or an explicit role for
            // role-scoped creation (fork / forward / top-bar entry).
            roleId: roleId
        )
        ...
        let sql = "INSERT INTO sessions (id, title, model_id, created_at, updated_at, source, memory_enabled, role_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
        var stmt: OpaquePointer?
        if sqlite3_prepare_v2(db, sql, -1, &stmt, nil) == SQLITE_OK {
            ...
            // [T-role-session-bound] Persist the bound persona (nil = legacy).
            bindOptionalText(stmt, index: 8, value: session.roleId)
    ...
    func listSessions() -> [ChatSession] {
        ...
        let sql = """
            SELECT s.id, s.title, s.model_id, s.created_at, s.updated_at, s.category,
                   ...
                   s.folder_id,
                   -- [T-role-session-bound] bound persona, appended after
                   -- folder_id under the same index discipline.
                   s.role_id
        ...
        if sqlite3_prepare_v2(db, sql, -1, &stmt, nil) == SQLITE_OK {
            while sqlite3_step(stmt) == SQLITE_ROW {
                ...
                // [T-role-session-bound] bound persona (column 15).
                let sessRoleId = Self.colTextOpt(stmt, 15)
                ...
                sessions.append(ChatSession(
                    ...
                    folderId: folderId, roleId: sessRoleId
                ))
            }
        }
        sqlite3_finalize(stmt)
    ...
    func getSession(_ id: String) -> ChatSession? {
        let sql = "SELECT id, title, model_id, created_at, updated_at, category, source, pinned_at, folder_id, role_id FROM sessions WHERE id = ?"
        var stmt: OpaquePointer?
        ...
        if sqlite3_prepare_v2(db, sql, -1, &stmt, nil) == SQLITE_OK {
            ...
            if sqlite3_step(stmt) == SQLITE_ROW {
                ...
                // [T-role-session-bound] bound persona of the session.
                let sessRoleId = sqlite3_column_text(stmt, 9).map { String(cString: $0) }

                session = ChatSession(
                    id: id, title: title, category: category, modelId: modelId,
                    createdAt: createdAt, updatedAt: updatedAt, source: source,
                    pinnedAt: pinnedAt, folderId: folderId, roleId: sessRoleId
                )
            }
        }
        sqlite3_finalize(stmt)
    ...
    func appendMessages(_ messages: [RawMessage]) {
        ...
        let sql = """
            INSERT INTO messages (id, session_id, role, parts_json, created_at, token_usage, sort_order, reasoning_content, stream_interrupt_count, updated_at, error_info, part_flags, role_id)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ...
        for message in messages {
            ...
            if prepRC == SQLITE_OK {
                ...
                // [T-role-message-level] Persist the generating persona
                // (assistant rows only; user/system stay nil by convention).
                bindOptionalText(stmt, index: 13, value: message.roleId)
    ...
    func loadMessages(sessionId: String) -> [RawMessage] {
        ...
        let sql = """
            SELECT id, session_id, role, parts_json, created_at, token_usage, reasoning_content, stream_interrupt_count, sort_order, error_info, role_id
        ...
        if prepareResult == SQLITE_OK {
            ...
            while sqlite3_step(stmt) == SQLITE_ROW {
                ...
                // [T-role-message-level] generating persona (assistant rows).
                let msgRoleId = sqlite3_column_text(stmt, 10).map { String(cString: $0) }
                var msg = RawMessage(
    ...
    func toChatMessage(mediaResolver: (MediaRef) -> URL, showThinking: Bool = true) -> ChatMessage {
        ...
        for part in parts {
            ...
            case .text(let s):
                ...
                // (`[attached image: /var/jarvis/...]` next to inline image data, and
        ...
        // [T-role-message-level] generating persona (assistant rows only).
        msg.roleId = roleId
...
extension ChatStore {
    ...
    func mergeRemoteSession(_ session: ChatSession, fromDeviceId: String, memoryEnabled: Bool = true, modelBinding: String? = nil, remotePinnedAtRaw: Date? = nil, remoteFolderId: String? = nil, remoteHasFolderField: Bool = false, remoteRoleId: String? = nil, remoteHasRoleField: Bool = false) {
        invalidateSessionListCache()
        ...
        if let localTs = localUpdatedAt {
            ...
            if remoteUpdated > localTs {
                ...
                sqlite3_finalize(stmt)
                // [T-role-session-bound] Role binding rides in a dedicated
                // statement rather than a third SQL variant: the role is
                // immutable in-session, so this only ever seeds legacy rows
                // receiving their first role-carrying record. Skipped when the
                // peer's record predates the field (remoteHasRoleField=false)
                // — an old build's omission must not wipe a local binding.
                if remoteHasRoleField {
                    let rSql = "UPDATE sessions SET role_id = ? WHERE id = ?"
                    var rStmt: OpaquePointer?
                    if sqlite3_prepare_v2(db, rSql, -1, &rStmt, nil) == SQLITE_OK {
                        bindOptionalText(rStmt, index: 1, value: remoteRoleId)
                        sqlite3_bind_text(rStmt, 2, (session.id as NSString).utf8String, -1, nil)
                        sqlite3_step(rStmt)
                    }
            ...
            let sql = "INSERT INTO sessions (id, title, category, model_id, created_at, updated_at, remote_origin_device_id, memory_enabled, model_binding, pinned_at, folder_id, role_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
            var stmt: OpaquePointer?
            if sqlite3_prepare_v2(db, sql, -1, &stmt, nil) == SQLITE_OK {
                sqlite3_bind_text(stmt, 1, (session.id as NSString).utf8String, -1, nil)
                bindOptionalText(stmt, index: 2, value: session.title)
                ...
                // [T-role-session-bound] Same orphan tolerance as folder_id:
                // an unknown roleId renders via the default-role fallback.
                bindOptionalText(stmt, index: 12, value: remoteHasRoleField ? remoteRoleId : nil)
                sqlite3_step(stmt)
            }
            sqlite3_finalize(stmt)
    ...
    func mergeRemoteMessage(
        ...
        updatedAt: Date? = nil,
        // [T-role-message-level] generating persona (assistant rows). nil =
        // old peer record / non-assistant row.
        roleId: String? = nil
        ...
        if let localTs = localUpdatedAt {
            ...
            let updateSql = """
                UPDATE messages SET parts_json = ?, token_usage = ?, reasoning_content = ?,
                    stream_interrupt_count = ?, updated_at = ?, part_flags = ?, role_id = ?
            ...
            if sqlite3_prepare_v2(db, updateSql, -1, &stmt, nil) == SQLITE_OK {
                ...
                // [T-role-message-level] LWW carries the persona too.
                bindOptionalText(stmt, index: 7, value: roleId)
            ...
            let partFlags = Self.partFlags(fromPartsJSON: partsJson)
            let insertSql = """
                INSERT INTO messages (id, session_id, role, parts_json, created_at, token_usage, sort_order, reasoning_content, stream_interrupt_count, updated_at, part_flags, role_id)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ...
            if sqlite3_prepare_v2(db, insertSql, -1, &stmt, nil) == SQLITE_OK {
                ...
                // [T-role-message-level] generating persona (assistant rows).
                bindOptionalText(stmt, index: 12, value: roleId)
    ...
    func loadSingleMessage(id: String) -> RawMessage? {
        let sql = """
            SELECT id, session_id, role, parts_json, created_at, token_usage, reasoning_content, stream_interrupt_count, sort_order, role_id
        ...
        if sqlite3_prepare_v2(db, sql, -1, &stmt, nil) == SQLITE_OK {
            ...
            if sqlite3_step(stmt) == SQLITE_ROW {
                ...
                // [T-role-message-level] generating persona (assistant rows).
                raw.roleId = sqlite3_column_text(stmt, 9).map { String(cString: $0) }
                message = raw
            }
        }
        sqlite3_finalize(stmt)
