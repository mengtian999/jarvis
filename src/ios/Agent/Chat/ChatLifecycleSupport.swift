...
final class ViewModelCache {
    ...
    // MARK: - [T-role-forward] Auto-respond request
    //
    // One-shot "auto-respond after forward" request. The source session's
    // forwardMessagesToRole appends the forwarded material directly to the
    // target's DB rows, then stashes this marker; the target's AIChatView
    // drains it on appear and starts the agent loop over the just-loaded
    // history without the user tapping send. TTL + target binding mirror the
    // Move-to pendingTransfer discipline: a stale or mis-addressed marker is
    // dropped rather than firing an unexpected reply.

    private struct PendingForwardRespond {
        let targetId: String
        let stashedAt = Date()
    }
    private var pendingForwardRespond: PendingForwardRespond?
    private static let forwardRespondTTL: TimeInterval = 300

    func stashPendingForwardRespond(targetId: String) {
        pendingForwardRespond = PendingForwardRespond(targetId: targetId)
        logger.info("[ForwardRespond] stash target=\(targetId.prefix(8))")
    }

    /// Drain the forward-respond slot exactly once, only for [sessionId].
    func consumePendingForwardRespond(sessionId: String) -> Bool {
        guard let r = pendingForwardRespond else { return false }
        pendingForwardRespond = nil
        guard Date().timeIntervalSince(r.stashedAt) <= Self.forwardRespondTTL,
              r.targetId == sessionId else {
            logger.info("[ForwardRespond] drop stale/mismatched marker (target=\(r.targetId.prefix(8)))")
            return false
        }
        logger.info("[ForwardRespond] consume target=\(sessionId.prefix(8))")
        return true
    }
