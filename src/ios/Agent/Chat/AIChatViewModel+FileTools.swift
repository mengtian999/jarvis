...
extension AIChatViewModel {
    ...
    /// Snapshot all files under /var/jarvis/ with their modification dates.
    ...
    /// Resolve a jarvis:// URL string to a host filesystem URL.
    ...
    private func resolveMinisURL(_ urlString: String) -> URL? {
        guard let url = URL(string: urlString), url.scheme == "jarvis",
    ...
    /// Convert a Linux path under /var/jarvis/ to a jarvis:// URL, or nil if not under /var/jarvis/.
    func linuxPathToMinisURL(_ path: String) -> String? {
        let prefix = "/var/jarvis/"
    ...
    /// For /var/jarvis/ paths, queries ISHExecutionCoordinator's active bind mount table
    /// to get the actual host URL, avoiding races when mounts switch between concurrent sessions.
    /// Falls back to resolveHostPath for non-/var/jarvis/ paths (e.g. /tmp, /root).
    func resolvePathForDirectRead(_ linuxPath: String) async -> URL? {
        if linuxPath.hasPrefix("/var/jarvis/") || linuxPath == "/var/jarvis" {
    ...
    /// Resolve a jarvis:// URL or /var/jarvis/ Linux path for direct reads.
    /// Unifies both URL schemes into a single async resolution path.
    func resolveMinisPath(_ pathOrURL: String) async -> URL? {
        if pathOrURL.hasPrefix("jarvis://") {
            // Convert jarvis:// URL to Linux path, then resolve via mount table
            ...
            let linuxPath = "/var/jarvis/\(host)" + (subPath.isEmpty ? "" : "/\(subPath)")
    ...
    func executeFileWrite(from json: String) async throws -> FileToolResult {
        ...
        // If the target is under /var/jarvis/mounts/ (user-mounted external
        ...
        if path.hasPrefix("/var/jarvis/mounts/"), let sid = self.sessionId {
            #if DEBUG
            ...
            #if DEBUG
            if path.hasPrefix("/var/jarvis/mounts/") {
                print("[FileWrite] ensureMounted SKIPPED path=\(path) — sessionId is nil")
            }
            #endif
        }

        // Write-specific resolution for /var/jarvis/mounts/ targets:
        ...
        // Defense-in-depth: if the Linux path is under /var/jarvis/mounts/ but
        // the resolved hostURL landed anywhere other than the real external
        // folder (the fakefs `data/var/jarvis/mounts/` shadow, or a per-session
        ...
        if path.hasPrefix("/var/jarvis/mounts/") {
            ...
            let fakefsMountsPrefix = RootfsManager.shared.dataPath
                .appendingPathComponent("var/jarvis/mounts").standardizedFileURL.path
