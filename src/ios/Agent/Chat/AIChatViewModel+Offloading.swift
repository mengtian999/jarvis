...
extension AIChatViewModel {
    ...
    /// Decide whether a file path (Linux view) or jarvis:// URL is backed by
    /// persistent session storage under `/var/jarvis/*`. Paths in those
    ...
    private static func isPersistentMinisPath(_ path: String) -> Bool {
        // Linux view: bind-mounted persistent directories under /var/jarvis
        if path.hasPrefix("/var/jarvis/workspace/") { return true }
        if path.hasPrefix("/var/jarvis/browser/") { return true }
        if path.hasPrefix("/var/jarvis/attachments/") { return true }
        if path.hasPrefix("/var/jarvis/offloads/") { return true }
        // jarvis:// URL view maps 1:1 to the directories above
        if path.hasPrefix("jarvis://workspace/") { return true }
        if path.hasPrefix("jarvis://browser/") { return true }
        if path.hasPrefix("jarvis://attachments/") { return true }
        if path.hasPrefix("jarvis://offloads/") { return true }
        return false
    }
    ...
    private static func originalImagePath(
        ...
        // browser_use (any action) appends `minis_url: jarvis://...` to the
        // result content when it persists a screenshot or fetched file to
        // /var/jarvis/browser/. Extract that URL.
        if toolName == "browser_use" || toolName.isEmpty {
            if let urlRange = content.range(of: #"jarvis://\S+"#, options: .regularExpression) {
    ...
    /// original path lives under `/var/jarvis/*` (workspace, browser,
        ...
        for mi in agentHistory.indices {
            ...
            for pi in parts.indices {
                ...
                case .toolResult(let id, let name, let content, let isError, let imgData, let mime, let pageURL, _):
                    ...
                    // original path is under /var/jarvis/* (persistent).
    ...
    func offloadContextIfNeeded(model: LLMModel, lastContextTokens: Int, force: Bool = false) {
        ...
        if offloadedCount > 0 {
            ...
            if let files = try? FileManager.default.contentsOfDirectory(
                ...
                logger.info("  /var/jarvis/offloads/tools/ — \(files.count) files (showing top \(top20.count)):")
                for file in top20 {
    ...
    /// Bind-mount a session's minis directories into iSH-visible /var/jarvis/.
    ...
    /// reinstall; contents migrate). All /var/jarvis symlinks carry absolute
    ...
    private func ensureMinisSymlinks(for sid: String) {
        ...
        // Ensure /var/jarvis/ exists in data/
        let jarvisDataDir = dataPath.appendingPathComponent("var/jarvis")
        try? fm.createDirectory(at: jarvisDataDir, withIntermediateDirectories: true)
    ...
    /// Create/refresh `/var/jarvis/mounts/<name>` symlinks for every entry in
    ...
    /// Reconcile `/var/jarvis/mounts/<name>` against `snapshot`. Runs entirely
    ...
    nonisolated private static func performMountedFolderSymlinkRefresh(
        ...
        let fm = FileManager.default
        let dataPath = RootfsManager.shared.dataPath
        let mountsDir = dataPath.appendingPathComponent("var/jarvis/mounts", isDirectory: true)
    ...
    /// Returns a `jarvis://attachments/<filename>` URL string, or nil on failure.
    func saveAttachment(_ data: Data, filename: String) -> String? {
        ...
        return "jarvis://attachments/\(encoded)"
    }
