...
struct AttachmentMeta: Identifiable, Equatable {
    let id = UUID()
    /// Linux path, e.g. `/var/jarvis/attachments/uploads/photo.jpg`
    let path: String
    let size: Int
    let modified: Date

    /// Derive a `jarvis://` URL from the Linux path.
    var minisURL: String {
        guard path.hasPrefix("/var/jarvis/") else { return path }
        let rel = String(path.dropFirst("/var/jarvis/".count))
        return "jarvis://\(rel)"
    }
...
final class ChatMessage: Identifiable, ObservableObject {
    ...
    /// message (RawMessage.roleId on reload, or the VM's sessionRole for live
    /// streaming messages). Only assistant messages carry a non-nil value;
    /// the message header resolves it via RoleStore.resolveRole → per-message
    /// rendering that stays pinned across global role switches.
