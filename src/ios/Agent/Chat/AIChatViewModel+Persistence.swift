...
extension AIChatViewModel {
    ...
    func loadSession() async {
        ...
        // [T-role-session-bound] Hydrate the session's bound persona from the
        // persisted row. Orphan refs (role deleted on this device) fall back
        // to the default role — never crash on a stale id. Direct disk read
        // (RoleStore.load), not the snapshot: this is a one-shot per session
        // open and must not race the lazily-warmed snapshot.
        if let stored = await ChatStore.shared.getSession(sessionId) {
            sessionRole = stored.roleId.flatMap { RoleStore.load(roleId: $0) } ?? RoleStore.loadDefault()
        }
        ...
        guard !rawMessages.isEmpty else {
            ...
            // Still mount minis even for empty sessions so /var/jarvis works in terminal
    ...
    func ensureSessionReturningId() async -> String {
        ...
        // [T-role-session-bound] Seed the draft's persona exactly once, at
        // first-send time: an explicit initialRoleId ("以此角色新建会话") wins
        // (unresolvable/deleted → default, matching the render-time orphan
        // policy); a plain draft takes the global current role the user
        // picked in Settings. The seeded value is immutable for the rest of
        // the session's life.
        if !draftRoleSeeded {
            draftRoleSeeded = true
            if let rid = initialRoleId {
                sessionRole = RoleStore.load(roleId: rid) ?? RoleStore.loadDefault()
            } else {
                sessionRole = RoleStore.currentRole()
            }
        }
        let model = selectedModel
        // [T-role-session-bound] Persist the bound persona into the new row so
        // reloads + sync resolve it.
        let session = await ChatStore.shared.createSession(modelId: model.id, source: sessionSource, roleId: sessionRole.roleId)
    ...
    // MARK: - [T-role-forward] Cross-session message forwarding
    //
    ...
    // flattened to user messages (roleId=nil) so the target persona reads
    // them as "the user pasted some material". The source session is never
    // mutated.

    /// Forward [messageIds] (DB row ids) into [targetSessionId], or into a NEW
    /// session bound to [targetRoleId] when nil.
    ///
    /// - Returns: the target session id, or nil when nothing was forwardable.
    ///
    /// [autoRespond] stashes a `ViewModelCache.PendingForwardRespond` marker so
    /// the target chat auto-starts its agent loop when opened. Skipped when the
    /// target session is already processing — never fight a running loop.
    func forwardMessagesToRole(messageIds: [String], targetRoleId: String, targetSessionId: String?, autoRespond: Bool = true) async -> String? {
        guard let sourceId = sessionId, !messageIds.isEmpty else { return nil }
        let store = ChatStore.shared

        let selected = await store.loadMessages(sessionId: sourceId)
            .filter { messageIds.contains($0.id) }
            .sorted { $0.sortOrder < $1.sortOrder }
        guard !selected.isEmpty else { return nil }

        // Pickers only list live roles, but the role may have been deleted
        // behind the sheet — fall back to the default role id so the row
        // still satisfies "new sessions always seed a role".
        ...
        if let existing {
            ...
            targetId = await store.createSession(
                ...
                title: Self.forwardPreviewTitle(selected),
                roleId: effectiveRoleId
            ).id
        }

        for msg in selected {
            let text = Self.forwardExtractText(msg.parts)
            guard !text.isEmpty else { continue }
            // Flattened user row: role .user, roleId nil (not produced by the
            // target persona). Source rows are untouched (copy semantics).
            let raw = RawMessage(
                id: UUID().uuidString, sessionId: targetId, role: .user,
                parts: [.text(text)], createdAt: Date()
            )
        ...
        if autoRespond {
            ...
            if !targetProcessing {
                if existing != nil {
                    // Drop any cached idle VM so the next open reloads from DB
                    // WITH the forwarded rows.
                    ViewModelCache.shared.remove(sessionId: targetId)
                }
                ViewModelCache.shared.stashPendingForwardRespond(targetId: targetId)
            }
        }
        AppLogger(category: "Forward").info("[Forward] \(selected.count) msg(s) → session=\(targetId.prefix(8)) role=\(effectiveRoleId) existing=\(existing != nil) autoRespond=\(autoRespond)")
        return targetId
    }

    /// [T-role-forward] UI-level single-message forward (context-menu MVP):
    /// extracts the forwardable text straight from the ChatMessage (its UI id
    /// is an independent UUID with no DB mapping, so the DB-id API above
    /// doesn't apply). Assistant text blocks are joined; tool/thinking blocks
    /// are skipped silently.
    func forwardChatMessageToRole(_ message: ChatMessage, targetRoleId: String, targetSessionId: String?, autoRespond: Bool = true) async -> String? {
        guard let sourceId = sessionId else { return nil }
        let text: String
        ...
        AppLogger(category: "Forward").info("[ForwardUI] msg → session=\(targetId.prefix(8)) role=\(effectiveRoleId) source=\(sourceId.prefix(8))")
        return targetId
    }

    /// [T-role-forward] Extract the forwardable text of a message's parts:
    /// concatenated text parts. MVP forwards text only; images get a trailing
    /// "[图片未转发]" note. Internal toolUse/toolResult parts are not
    /// user-visible material and are skipped silently.
    static func forwardExtractText(_ parts: [ContentPart]) -> String {
        var texts: [String] = []
        var hadMedia = false
        for part in parts {
            switch part {
            case .text(let s):
                if !s.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty { texts.append(s) }
            case .mediaRef:
                hadMedia = true
    ...
    private static func forwardPreviewTitle(_ selected: [RawMessage]) -> String {
        let firstText = selected
            .map { forwardExtractText($0.parts) }
            .first { !$0.isEmpty } ?? "Forwarded Chat"
        let collapsed = firstText.components(separatedBy: .whitespacesAndNewlines)
            .filter { !$0.isEmpty }
            .joined(separator: " ")
        return String(collapsed.prefix(50)).isEmpty ? "Forwarded Chat" : String(collapsed.prefix(50))
    }

    /// [T-role-forward] Auto-start the agent loop over the already-persisted
    /// forwarded history WITHOUT appending a user bubble. Called by
    /// AIChatView when it drains a `ViewModelCache.PendingForwardRespond`
    /// marker after a forward. Mirrors resume()'s loop-launch scaffolding;
    /// runAgentLoop() appends its own assistant placeholder (stamped with the
    /// session role).
    func respondToForwardedHistory() {
        guard !isProcessing, !isCompacting else { return }
        guard resolveCurrentEntry() != nil else {
            errorMessage = String(localized: "No model configured. Add a provider in Settings.")
            return
        }
        errorMessage = nil
        userDidCancel = false
        canResume = false
        isProcessing = true
        logger.info("[ForwardRespond] start sid=\(sessionId?.prefix(8) ?? "nil")")
        ensureKernelBooted()
        ...
        currentTask = Task { [weak self] in
            ...
            do {
                ...
                logger.info("Agent loop cancelled (forward-respond)")
                self.handleUserCancelledCleanup()
            } catch {
                let rawDesc = String(describing: error)
                logger.error("Agent loop error (forward-respond): \(rawDesc)")
                if self.userDidCancel {
                    self.handleUserCancelledCleanup()
                } else if let last = self.messages.last(where: { $0.role == .assistant }) {
                    last.error = Self.friendlyErrorMessage((error as? LocalizedError)?.errorDescription ?? rawDesc)
                    Task { await self.persistErrorInfo(last.error!) }
                } else {
                    self.errorMessage = Self.friendlyErrorMessage(rawDesc)
                }
            }
    ...
    func buildRawMessage(_ msg: AgentMessage, tokenUsage: TokenUsage? = nil, snapshots: [String: (toolName: String, snapshot: ToolSnapshot)] = [:], thoughtSignatures: [String: String] = [:], reasoningContent: String? = nil, streamInterruptCount: Int = 0, toolStatuses: [String: String] = [:], modelEntryId: String? = nil) async -> RawMessage? {
        ...
        var raw = RawMessage(
            ...
            // [T-role-message-level] Only assistant rows record the generating
            // persona (user/tool_result rows stay nil) so history renders each
            // role correctly even after the global current role changes.
            roleId: msg.role == .user ? nil : sessionRole.roleId
        )
