...
struct FileMentionEntry: Identifiable, Equatable, Hashable {
    ...
    /// iSH-visible path (e.g. `/var/jarvis/workspace/foo/bar.py`). Inserted into input.
    ...
    /// Path shown in the menu (strips the `/var/jarvis/` prefix for density).
...
final class FileMentionIndex: ObservableObject {
    ...
    private struct ScanRoot {
        ...
        let hostURL: URL
        /// Linux-visible path prefix (e.g. `/var/jarvis/workspace`).
    ...
    /// substring. Lets `@mounts` match `/var/jarvis/mounts/<name>/...` even
    ...
    /// Is this the immediate child of a scope root (e.g. `/var/jarvis/skills/foo`,
    /// `/var/jarvis/mounts/bar`, `/var/jarvis/shared/baz`)? These top-level
