...
final class AIChatViewModel: ObservableObject, SpeechControlling {
    ...
    // MARK: - [T-role-session-bound] Session persona
    //
    // The persona bound to this session. Immutable in-session (switching
    // personas = new session or forwarding). Seeded:
    //   - draft: initialRoleId when the draft was opened role-scoped, else
    //     the global current role at first send
    //   - existing session: the persisted session.roleId (orphan → default)
    // baseSystemPrompt reads it (RoleSystemPromptBuilder), and persisted
    // assistant rows carry sessionRole.roleId (message-level rendering).
    @Published private(set) var sessionRole: RoleFile = RoleStore.loadDefault()
    /// Explicit role binding for a not-yet-persisted draft ("以此角色新建会话").
    /// Set by the draft-creation site (ContentView / forward flow) BEFORE the
    /// first send; consumed by ensureSessionReturningId. nil = plain draft →
    /// global current role.
    var initialRoleId: String? = nil
    /// Guards the one-time draft role seed (initialRoleId ?? global current).
    private var draftRoleSeeded = false
    ...
    /// [T-role-message-level] Assistant placeholder factory: stamps the
    /// session's bound role so live message headers render the right persona
    /// while streaming (persisted rows get the same id via buildRawMessage).
    func makeAssistantPlaceholder() -> ChatMessage {
        let m = ChatMessage(role: .assistant, content: "", blocks: [])
        m.roleId = sessionRole.roleId
        return m
    }
    ...
    init() {
        ...
        syncChangeObserver = NotificationCenter.default.addObserver(
            ...
            // "Jarvis is thinking" indicator. Flip a flag instead; the
    ...
    private static func shortFrame(_ raw: String) -> String {
        ...
        // "3   Jarvis   0x0000000104abcd12 $s5Minis... mangled ... + 40"
    ...
    /// Jarvis-2026-06-01-134710.ips: user switched sessions while both vms had
    ...
    /// Matches: ![...](jarvis://attachments/file.mp3?auto_play=true)
    ...
    static func triggerAutoPlayAudio(in text: String) {
        ...
        // Resolve jarvis:// to file URL
        let fileURL: URL?
        if cleanURL.scheme == "jarvis" {
    ...
    /// Resolve jarvis:// URL to local file URL (standalone, no UI dependency).
    private static func resolveMinisFileURLForAutoPlay(url: URL) -> URL? {
        guard url.scheme == "jarvis", let host = url.host else { return nil }
        ...
        // /var/jarvis data dir ONLY for the global, non-session-scoped
        // namespaces (memory/skills/shared). For a session-scoped host like
        // `attachments`, /var/jarvis/<host> binds to whichever session is
        ...
        if globalDirs.contains(host) {
            ...
            for subPath in subPaths {
                let linuxPath = "/var/jarvis/\(host)/\(subPath)"
    ...
    private var baseSystemPrompt: String {
        // [T-role-session-bound] Layer 1 is rendered by
        // RoleSystemPromptBuilder, parametric on the SESSION's bound persona
        // (immutable in-session) — NOT the global current role, so a Settings
        // role switch mid-session never re-personas an open chat. The builder
        // owns the identity sentence + Personality/style blocks and scrubs
        // prompt injections from the role body.
        RoleSystemPromptBuilder.identitySection(for: sessionRole)
            ...
            // Jarvis has no in-app scheduler (see 'Scheduled tasks'), so the only
            ...
            + "IMPORTANT: jarvis:// URLs are app-internal — they are NOT web URLs. Do NOT pass jarvis:// action URLs (open_terminal, views, settings) to browser_use — those are app deep links, use Markdown links in chat instead. "
            + "However, jarvis:// resource URLs CAN be opened in browser_use with navigate. All directories under /var/jarvis/ are accessible: workspace, attachments, offloads, shared, etc. "
            + "The built-in browser fully supports jarvis:// — HTML pages and all sub-resources (JS, CSS, images, fonts, etc.) referenced via jarvis:// absolute URLs or relative paths resolve correctly within the current session. "
            + "When building multi-file web projects, use file_write to create files in the same directory (e.g. /var/jarvis/workspace/myapp/), "
            + "then reference sub-resources with relative paths in HTML (e.g. <link href=\"style.css\">, <script src=\"app.js\">, <img src=\"logo.png\">). "
            + "The browser resolves relative paths against the jarvis:// base URL automatically. "
            + "Cross-directory references also work with absolute jarvis:// URLs (e.g. <img src=\"jarvis://attachments/photo.png\"> from a workspace HTML page). "
            + "Navigate to the entry HTML to preview, e.g. jarvis://workspace/myapp/index.html.\n"
            ...
            + "IMPORTANT: jarvis:// URLs MUST be percent-encoded. Non-ASCII characters (Chinese, emoji, spaces, etc.) in filenames will break Markdown rendering if not encoded. "
            + "Use the minis_url from tool results directly — it is already encoded. "
            + "If you construct a jarvis:// URL manually, percent-encode the filename (e.g. %E4%B8%AD%E6%96%87 for non-ASCII characters).\n"
            ...
            + "Audio auto-play: append ?auto_play=true to an audio jarvis:// URL to auto-play when rendered (e.g. ![audio](jarvis://attachments/song.mp3?auto_play=true)). Use sparingly — only when the user explicitly asks to hear audio immediately.\n"
            ...
            + "Use Markdown links for all jarvis:// files — the user can tap to preview them directly in chat.\n\n"
            ...
            + "apple-alarm sets alarms and timers via AlarmKit (iOS 26+). Alarms can only be viewed in the Jarvis home screen (alarm icon in the top-right toolbar) or by opening jarvis://views/alarm. "
            + "After setting an alarm, tell the user it is visible on the Jarvis home screen and they can tap the alarm icon or open jarvis://views/alarm to manage it.\n"
            ...
            + "minis-open <url-or-path>: Opens a resource inside Jarvis without leaving the chat. Accepts http/https URLs (→ built-in WebKit preview) and chat-resource file paths under /var/jarvis/** (→ built-in file preview, routed by extension: images to the image viewer, .md to markdown preview, .html to HTML preview, .pdf/office docs to QuickLook, audio/video to the media player, else share sheet). Examples: `minis-open https://example.com`, `minis-open /var/jarvis/workspace/report.md`, `minis-open /var/jarvis/attachments/chart.png`. Prefer this over `apple-open` for anything that can be previewed in-app so the user doesn't lose conversation context. Use `apple-open` for non-web schemes (tel:, mailto:, maps://, settings, etc.) or when the user explicitly wants the system handler.\n"
            ...
            + "Interactive terminal: jarvis://open_terminal opens a terminal for tasks that require interactive stdin (passwords, ssh, TUI apps like htop/vi). "
            ...
            + "[Set ENV_NAME](jarvis://settings/environments?create_key=ENV_NAME&create_value=&create_note=Used%20by%20XYZ) — "
    ...
    /// user gets two overlapping notifications for one task: "Jarvis Task
    ...
    /// Currently active session ID — accessible from anywhere for jarvis:// URL resolution.
    ...
    func send() {
        ...
        if skipCompactCheck {
            ...
            case .exhausted:
                if sessionSource == "shortcut" {
                    ...
                    let errMsg = makeAssistantPlaceholder()
        ...
        currentTask = Task { [weak self] in
            ...
            for (i, attachment) in pendingAttachments.enumerated() {
                ...
                let linuxPath = "/var/jarvis/attachments/uploads/\(safeName)"
    ...
    private func injectQueuedPromptsAsNewTurn(
        ...
        messages.append(makeAssistantPlaceholder())
    ...
    private func handleUserCancelledCleanup() {
        ...
        if userDidCancel {
            ...
            // render as a bare "Jarvis" header bubble with the typing
