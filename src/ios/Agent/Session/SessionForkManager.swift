...
final class SessionForkManager {
    ...
    func forkSession(remoteSessionId: String, remoteDeviceId: String) async -> ChatSession? {
        ...
        // [T-role-session-bound] A fork keeps the source persona (roles are
        // immutable in-session; a fork is a continuation of the same thread).
        let newSession = await store.createSession(modelId: remoteSession.modelId, title: forkTitle, roleId: remoteSession.roleId)
    ...
    func duplicateSession(sessionId: String) async -> ChatSession? {
        ...
        let newSession = await store.createSession(modelId: session.modelId, title: dupTitle, roleId: session.roleId)
