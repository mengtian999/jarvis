...
enum SoulIconSource {
        ...
        var errorDescription: String? {
            ...
            case .outsideAllowedDirs(let p):
                return "\(p) is outside the directories this tool may read. "
                     + "Use a jarvis:// URL (e.g. jarvis://attachments/icon.png) "
    ...
    static func looksLikeImageSource(_ raw: String) -> Bool {
        ...
        if s.hasPrefix("jarvis://") { return true }
    ...
    /// `jarvis://` goes through the app's own resolver, which is what enforces
...
enum SystemPromptBuilder {
    ...
    /// expectations ("Jarvis, capable AI assistant, iSH Linux shell")
...
//
// [T-role-message-level] AssistantSoulName was removed: the assistant message
// header now renders the per-message persona via AssistantMessageHeader
// (RoleStore.swift), and the chat top bar / typing indicator read the
// session's bound role. SoulStore.cachedMetadata remains in use only as the
// legacy fallback name for role-less rendering paths.
