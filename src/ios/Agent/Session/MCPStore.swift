...
//  /var/jarvis/mcp-servers — the SAME file the in-guest `minis-mcp-cli`
...
struct MCPServerConfig: Codable, Identifiable, Hashable {
    ...
    /// first MCP `initialize` (Jarvis config, not MCP protocol). Enforcement
...
final class MCPStore: ObservableObject {
    ...
    /// Host path of the guest's /var/jarvis/mcp-servers/servers.json. Lives in
    ...
    nonisolated private static var legacyServersFileURL: URL {
        FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("alpine-rootfs/data/var/jarvis/mcp-servers", isDirectory: true)
    ...
    private struct ServerEntry: Codable {
        ...
        var args: [String]?
        var env: [String: String]?
        var startupTimeoutSeconds: Int?   // Jarvis STDIO startup timeout; round-tripped verbatim
    }
    ...
    /// it initializes includes /var/jarvis/mcp-servers.
