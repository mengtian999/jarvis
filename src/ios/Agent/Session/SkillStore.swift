...
final class SkillStore: ObservableObject {
    ...
    private var rootfsSkillsDir: URL {
        ...
        return documents.appendingPathComponent("alpine-rootfs/data/var/jarvis/skills")
    }
...
"""
    ...
    func importFromArchive(at url: URL) throws -> Skill {
        ...
        for entry in entries {
            ...
            let linuxPath = "/var/jarvis/skills/\(skill.id)/\(relativePath)"
    ...
    /// via the iSH shell (`mkdir /var/jarvis/skills/<id>` + write SKILL.md),
    ...
    /// directory name is the id (NOT a re-slugified name) so the `jarvis://` link
    ...
    func skillPromptFragment(for sessionId: String) -> String? {
        ...
        for skill in selected {
            ...
            xml += "    <path>/var/jarvis/skills/\(skill.id)/SKILL.md</path>\n"
            xml += "  </skill>\n"
        }
        xml += "</available_skills>"

        var fragment = "Skills:\n"
        fragment += "Reusable instruction sets stored at /var/jarvis/skills/<name>/SKILL.md. Read the SKILL.md file to load full instructions before using a skill.\n\n"
        fragment += xml
        ...
        if hasMore {
            ...
            fragment += "\n\n\(omitted.count) more skills not shown above: \(undisclosedNames). List /var/jarvis/skills/ or grep to search all."
        }
        ...
        if sqlite3_prepare_v2(db, sql, -1, &stmt, nil) == SQLITE_OK {
            ...
            while sqlite3_step(stmt) == SQLITE_ROW {
                ...
                if let content = try? String(contentsOf: skillFile, encoding: .utf8) {
                    ...
                    // /var/jarvis/skills/<name>/SKILL.md from the iSH shell, and
    ...
    private func syncToRootfs(skill: Skill, content: String) {
        ...
        let linuxSkillDir = "/var/jarvis/skills/\(skill.id)"
    ...
    private func downloadGitHubDirectory(
        ...
        for item in items {
            ...
            let linuxPath = "/var/jarvis/skills/\(skill.id)/\(relPath)"
            ensureParentDirsInMetaDB(for: linuxPath)
            ensureFakefsMetadata(for: linuxPath, isDirectory: false)
        }
    }
    ...
    func importSkillFromSyncWithAsset(
        ...
        do {
            ...
            for rel in localBundled where !zipPaths.contains(rel) {
                ...
                let linuxPath = "/var/jarvis/skills/\(skillId)/\(rel)"
