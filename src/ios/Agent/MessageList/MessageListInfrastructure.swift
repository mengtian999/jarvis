...
enum MessageListItem: Hashable {
    ...
    /// The "sparkles Jarvis" label row at the top of an assistant turn.
...
class SelfSizingCell: UICollectionViewCell {
    ...
    override func prepareForReuse() {
        ...
        // .performTransition(). No Jarvis frames on the stack: iOS 26/27's Liquid
...
final class NoAnimationCollectionView: UICollectionView {
    ...
    override func layoutSubviews() {
        ...
        if isTracking || isDecelerating {
            ...
            // streaming one (the "Jarvis" header) drift — the header↔body spacing
...
final class CellStateBridgeV2: ObservableObject {
    ...
    /// [T-role-forward] "转发到角色会话" from the message context menu. Carries
    /// the UI message id; AIChatView maps it to DB row ids and presents the
    /// role/target picker sheet.
    @Published var onForward: ((UUID) -> Void)?
    ...
    /// selection-menu "Read Selection" action) via the Jarvis TTS stack.
