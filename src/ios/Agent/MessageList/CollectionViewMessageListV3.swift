...
struct CollectionViewMessageListV3: UIViewControllerRepresentable {
    ...
    /// [T-role-forward] Long-press "Forward to role" per UI message id.
    var onForwardMessage: ((UUID) -> Void)?
    ...
    func updateUIViewController(_ vc: MessageListViewController, context: Context) {
        ...
        coord.onForwardMessage = onForwardMessage
...
/// Header: "✦ Jarvis" label at the top of each assistant turn.
...
private struct BridgedAssistantHeaderV3: View {
    ...
    var body: some View {
        HStack(spacing: 6) {
            ...
            Text(soulMeta.name.isEmpty ? "Jarvis" : soulMeta.name)
...
private struct BridgedAssistantFooterV3: View {
    ...
    @ObservedObject var bridge: CellStateBridgeV2
    var maxWidth: CGFloat
    /// [T-role-session-bound] Injected via .environmentObject(vm) by the cell
    /// factory — supplies the session's bound persona name to the typing
    /// indicator (never the global current role).
    @EnvironmentObject var vm: AIChatViewModel
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if showsTypingIndicator {
                TypingIndicator(sessionRoleName: vm.sessionRole.metadata.name)
            }
...
extension CollectionViewMessageListV3 {
    ...
    final class Coordinator: NSObject, UICollectionViewDelegate, UIScrollViewDelegate, UICollectionViewDataSourcePrefetching {
        ...
        var onForceSync: (() -> Void)?
        var onScreenshotImage: ((UIImage) -> Void)?
        /// [T-role-forward] per-UI-message forward request → AIChatView sheet.
        var onForwardMessage: ((UUID) -> Void)?
        ...
        private func updateBridge(_ bridge: CellStateBridgeV2, message: ChatMessage, in messages: [ChatMessage],
            ...
            // selection menu — speaks the selected snippet via the Jarvis TTS
            ...
            let forward = onForwardMessage
            bridge.onForceSync = forceSync
            // [T-role-forward] Available on user + assistant rows (any
            // forwardable content); suppressed on compacted/system rows and
            // while the loop runs (rows keep mutating).
            bridge.onForward = (message.isCompactedHistory || message.role == .compactDivider || message.role == .systemInfo || vm.isProcessing)
                ? nil
                : { forward?(message.id) }
        ...
        private func applySnapshot(messages rawMessages: [ChatMessage], caller: String = "?") {
            ...
            if let layout = viewController?.messageListLayout {
                ...
                for (i, item) in newItems.enumerated() {
                    ...
                    switch item {
                    case .assistantHeader:
                        // Header is always a fixed "sparkles Jarvis" label row (measured: 28pt)
            ...
            if let cv = viewController?.collectionView {
                ...
                // TALLER height it measured while "Jarvis is thinking…" was
