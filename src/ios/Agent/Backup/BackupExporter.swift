...
/// Builds a `.jarvisbak` package (docs/backup-restore-design.md §2, §9 stage 1).
///
...
actor BackupExporter {
    ...
    struct Options {
        ...
        /// [T-backup-roles-category] Role ids the user UNCHECKED in the
        ...
        var excludedRoleIds: Set<String> = []
        ...
        init(categories: Set<BackupCategory> = Set(BackupCategory.backupable),
            ...
            self.excludedRoleIds = excludedRoleIds
        }
    }
    ...
    private func exportBody(options: Options,
        ...
        // a bare `backup-f9c34c.jarvisbak` with no date, leaving a folder of
        ...
        try await run(.roles, AppLocalized("Exporting roles…")) {
            try exportRoles(dataDir: dataDir, excluded: options.excludedRoleIds)
        }
    ...
    ///
    /// [T-backup-roles-category] Roles were absent from iOS packages entirely
    /// (see the `roles` case in BackupCategory): MEMORY's export only handled
    /// top-level `*.md` files. `entries` counts ROLES (directories holding a
    /// role.json), not files, mirroring the SKILLS reporting unit. Returns nil
    /// for an empty or absent collection, so it adds no category rather than
    /// an empty one (same rule as MCP servers).
    private func exportRoles(dataDir: URL, excluded: Set<String>) throws
        -> BackupManifest.CategoryStat? {
        let src = RoleStore.rolesDir
        // The default role (id "default", Jarvis) is never excludable: defence
        // in depth alongside the UI's refusal to put "default" in the set.
        let effectiveExcluded = excluded.subtracting(["default"])
        let includedNames = ((try? fm.contentsOfDirectory(atPath: src.path)) ?? [])
            .filter { name in
                !effectiveExcluded.contains(name) &&
                fm.fileExists(atPath: src.appendingPathComponent(name)
                    .appendingPathComponent("role.json").path)
            }
        if includedNames.isEmpty { return nil }
        let dst = dataDir.appendingPathComponent("roles", isDirectory: true)
        try fm.createDirectory(at: dst, withIntermediateDirectories: true)

        var files = 0
        var bytes: Int64 = 0
        if let enumerator = fm.enumerator(
            at: src, includingPropertiesForKeys: [.isRegularFileKey, .fileSizeKey]) {
            for case let url as URL in enumerator {
                let values = try? url.resourceValues(forKeys: [.isRegularFileKey, .fileSizeKey])
                guard values?.isRegularFile == true else { continue }
                let rel = String(url.path.dropFirst(src.path.count))
                    .trimmingCharacters(in: CharacterSet(charactersIn: "/"))
                // Files at the roles/ root (current.json) always travel; files
                // under roles/<roleId>/ only when that role is included.
                let ownerDir = rel.split(separator: "/").first.map(String.init) ?? ""
                let isRootFile = rel.firstIndex(of: "/") == nil
                if !isRootFile && !includedNames.contains(ownerDir) { continue }
        ...
        return BackupManifest.CategoryStat(entries: includedNames.count, bytes: bytes,
                                          encrypted: false, files: files)
    }
    ...
    /// `<device>-<yyyyMMdd>-<sortable-id>.jarvisbak`, e.g.
    /// `Ethans-iPhone-20260823-mf3k9q2phz.jarvisbak`. Three deliberate changes
    /// from the old `backup-20260823-1259-f69c00.jarvisbak`:
    ///
    ...
    ///    `backup-` prefix is dropped to pay for it — `.jarvisbak` already says
    ///    what the file is.
    ...
    /// `Ethans-iPhone-20260823-mf3k9q2phz-encrypted.jarvisbak`. Whether a
