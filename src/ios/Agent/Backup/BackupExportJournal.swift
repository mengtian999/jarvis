...
enum BackupExportJournal {
    ...
    ///
    /// Deliberately still named `jarvisbak-<backupId>`: `archive()` zips this
    ...
    static func stagingRoot(backupId: String) -> URL {
        supportDir.appendingPathComponent("jarvisbak-\(backupId)", isDirectory: true)
    }
    ...
    static func sweepAbandoned() {
        let fm = FileManager.default
        let keep = interrupted().map { "jarvisbak-\($0.backupId)" }

        var removed = 0
        for name in (try? fm.contentsOfDirectory(atPath: supportDir.path)) ?? [] {
            guard name.hasPrefix("jarvisbak-"), name != keep else { continue }
        ...
        //   jarvisbak-        pre-resume export staging; import/peek work dirs
        ...
        //
        // Plus ANY `*.jarvisbak` in tmp/, which is what actually catches the
        ...
        let prefixes = ["jarvisbak-", "backup-", "restore-", "server-restore-"]
