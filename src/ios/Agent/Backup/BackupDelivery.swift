...
/// Hands a finished `.jarvisbak` package to the user
...
enum BackupDelivery {
    ...
    /// The registered UTI for `.jarvisbak` (Info.plist `UTExportedTypeDeclarations`).
    ///
    ...
    ///      will want.
    static let contentTypeIdentifier = "com.openminis.app.jarvisbak"
    ...
    ///   1. `shared/` is bind-mounted into the guest at `/var/jarvis/shared`
    ...
    /// cloud folder in Files and authorised it in Jarvis, a backup can be
    ...
    /// TRUNCATED file carrying a perfectly valid `.jarvisbak` name, which then
