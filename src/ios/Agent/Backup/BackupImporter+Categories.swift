...
extension BackupImporter {
    ...
    func snapshotForRollback(category: BackupCategory, staging: URL) throws -> BackupRollbackSnapshot {
        ...
        case .roles: liveDirs = [RoleStore.rolesDir]
    ...
    func importCategory(_ category: BackupCategory,
        ...
        case .roles: return try await importRoles(root: root)
    ...
    /// Role personas → `roles/` under the memory directory.
    ///
    /// [T-backup-roles-category] Roles were absent from iOS packages entirely:
    /// [importMemory] above handles only top-level `*.md` files, so the roles/
    /// subtree never rode along. The whole `data/roles` tree is copied back
    /// file-by-file; identical content is a skip, not an import — same
    /// reasoning as importMemory's hash check (ZIP timestamps are meaningless,
    /// and these files are tiny so hashing is cheap).
    ///
    /// Also accepts the legacy Android shape: packages written before ROLES
    /// was a category carried the tree inside `data/memory/roles/`, which
    /// importMemory ignores.
    private func importRoles(root: URL) async throws -> CategoryReport {
        var report = CategoryReport(category: BackupCategory.roles.rawValue)
        let fm = FileManager.default
        let dst = RoleStore.rolesDir

        func copyTree(from src: URL) throws {
            guard fm.fileExists(atPath: src.path) else { return }
            try fm.createDirectory(at: dst, withIntermediateDirectories: true)
            guard let enumerator = fm.enumerator(
                at: src, includingPropertiesForKeys: [.isRegularFileKey]) else { return }
            for case let url as URL in enumerator {
                ...
                try? fm.createDirectory(at: to.deletingLastPathComponent(),
                                        withIntermediateDirectories: true)
                if let localData = try? Data(contentsOf: to),
                   let pkgData = try? Data(contentsOf: url),
                   localData == pkgData {
                    report.skipped += 1
                    continue
                }
        ...
        try copyTree(from: root.appendingPathComponent("data/roles", isDirectory: true))
        if report.imported == 0 {
            // [T-backup-roles-legacy-memory] The pre-category shape.
            try copyTree(from: root.appendingPathComponent("data/memory/roles",
                                                           isDirectory: true))
        }

        // The collection snapshot is cached in memory and the current-role
        // pointer lives in the restored current.json — same reason importMemory
        // ends with SoulStore.refreshCache(). Post .rolesChanged so message
        // headers and the role list re-render without a relaunch.
        await MainActor.run {
            RoleStore.refreshRolesSnapshot()
    ...
    func containmentRoot(for category: BackupCategory) -> URL {
        ...
        case .roles: return RoleStore.rolesDir
