...
enum BackupFormat {
    ...
    /// best-effort parse.
    static let current = "jarvisbak/1"

    /// File extension registered to the app for "open to import".
    static let fileExtension = "jarvisbak"
...
enum BackupCategory: String, Codable, CaseIterable, Sendable {
    ...
    ///
    /// [T-backup-roles-category] Roles were never in an iOS package before:
    /// MEMORY's export copies only the top-level `*.md` files, so the roles/
    /// subtree silently dropped out of every backup. A first-class category
    /// fixes that; the raw value is the shared wire key, matching Android's
    ...
    var carriesFileTree: Bool {
        ...
        // ROLES copies verbatim like MEMORY — role dirs are tiny (a JSON body
        // plus a 64px avatar), so the §3.4 cap has nothing to do.
        case .memory, .roles, .providers, .mcpServers, .voiceCorrections,
