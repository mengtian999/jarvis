...
/// server always holds a clean, self-contained `.jarvisbak` a user can grab
...
enum RcloneTransfer {
    ...
    private static func sweepAbandonedPartials(fs: String,
        ...
        for e in list {
            ...
            // `.pkg.jarvisbak.partial` still ends in `.partial` and is still
    ...
    /// Every `.jarvisbak` in `remote`'s backup directory, newest first.
    ///
    /// In-flight scratch files never show up as restorable backups: they are
    /// named `<package>.jarvisbak.partial`, whose suffix is `.partial`, and the
    /// filter below requires the name to END in `.jarvisbak`.
    ...
    /// `.jarvisbak` files directly inside it.
    ///
