...
enum BackupCrypto {
    ...
    static let scheme = "jarvisbak-enc/1"
    ...
    /// `JBK1` — lets a reader identify an encrypted member before trying to
    /// parse it as JSON.
    static let magic = Data([0x4A, 0x42, 0x4B, 0x31])
    ...
    final class Keys {
        ...
        fileprivate init(kek: SymmetricKey) {
            ...
            dataKey = sub("jarvisbak/data")
            secretsKey = sub("jarvisbak/secrets")
            macKey = sub("jarvisbak/mac")
            verifierKey = sub("jarvisbak/verify")
        }

        /// `HMAC(verifier_key, "jarvisbak-v1")` truncated to 16 bytes (§5.2).
        ...
        var verifier: String {
            ...
            mac.update(data: Data("jarvisbak-v1".utf8))
