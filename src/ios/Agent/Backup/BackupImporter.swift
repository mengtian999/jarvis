...
/// Restores a `.jarvisbak` package (docs/backup-restore-design.md §8).
///
...
actor BackupImporter {
    ...
    private func importBody(from packageURL: URL,
        ...
        let work = fm.temporaryDirectory
            .appendingPathComponent("jarvisbak-import-\(UUID().uuidString)", isDirectory: true)
    ...
    func inspect(packageURL: URL) throws -> BackupManifest {
        let work = fm.temporaryDirectory
            .appendingPathComponent("jarvisbak-peek-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: work) }
        let root = try unpack(packageURL, to: work)
    ...
    /// reverse; the archive nests everything under `jarvisbak-<uuid>/`, so the
    ...
    /// REFUSED `jarvisbak/1.1` even though §2.2 rule 2 says same-major must
    ...
    private func orderedCategories(_ wanted: Set<BackupCategory>) -> [BackupCategory] {
        ...
        // Roles sits after memory: both write under the memory directory, and
        // the roles restore must not race the memory tree copy (Android's
        // ORDER list mirrors this).
        let order: [BackupCategory] = [.chats, .sharedFiles, .skills, .memory,
                                       .roles, .providers, .environmentVariables,
