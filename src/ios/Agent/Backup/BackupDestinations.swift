...
/// Jarvis, and iOS's FileProvider handles the protocol. From here it is an
/// ordinary directory.
@MainActor
enum BackupDestinations {
    ...
    /// folders in front of the user and invited them to scatter `.jarvisbak`
    ...
    /// probe (`isWritable`) and the user's own "don't let Jarvis write here"
    ...
    ///
    /// The local copy in Jarvis ▸ Backups is written before this runs and is
    ...
    /// A `.jarvisbak` found inside a mounted destination.
