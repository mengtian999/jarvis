...
actor ISHExecutionCoordinator {
    ...
    /// Maps linux mount paths (e.g. "/var/jarvis/memory") to their host
    ...
    /// Resolve a Linux path under /var/jarvis/ to its current host URL.
    ...
    private func performMount(_ sid: String) {
        ...
        for (idx, (persistDir, linuxDir)) in subdirs.enumerated() {
            ...
            if let hostDir = resolveHostPath(linuxDir) {
                ...
                if lstatOk && (lstatBuf.st_mode & S_IFMT) == S_IFLNK {
                    ...
                    // Safety: only remove paths under data/var/jarvis/ to avoid accidentally
                    // deleting other fakefs directories (e.g. data/dev/) due to path bugs.
                    let dataVarJarvis = RootfsManager.shared.dataPath.appendingPathComponent("var/jarvis").path
                    guard hostDir.path.hasPrefix(dataVarJarvis) else {
                        logger.error("MOUNT [\(idx)] SAFETY: refusing removeItem outside var/jarvis: \(hostDir.path)")
                        break
                    }
        ...
        // them just like /var/jarvis/shared.
    ...
    private func mountExternalFoldersLocked() {
        ...
        // Ensure the parent /var/jarvis/mounts dir exists in meta.db so fakefs
        // can list it.
