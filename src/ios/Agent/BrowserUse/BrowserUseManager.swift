...
final class BrowserUseManager: NSObject, ObservableObject {
    ...
    /// Downloads are saved into that session's /var/jarvis/workspace/ directory.
    ...
    /// Shared scheme handler for `jarvis://` URLs across all browser tabs
    ...
    private static func makeWebView(profile: UserAgentProfile,
        ...
        config.setURLSchemeHandler(Self.sharedMinisSchemeHandler, forURLScheme: "jarvis")

        // Bridge JS window.print() to the native iOS print dialog. Injected on
        // every page (jarvis:// and external http/https) at document start so a
    ...
    private func presentPrintDialog() {
        ...
        printInfo.jobName = pageTitle.isEmpty ? "Jarvis" : pageTitle
    ...
    private func navigate(to urlString: String?) async throws -> BrowserActionResult {
        ...
        // Only allow HTTP(S) and jarvis:// schemes for direct navigation
        let scheme = url.scheme?.lowercased() ?? ""
        if !["http", "https", "jarvis"].contains(scheme) {
            return .error("Cannot navigate to \(scheme):// URLs. Only http://, https://, and jarvis:// are supported.")
        }
    ...
    private func getCookies(keywords: [String]?, fuzzy: Bool) async -> BrowserActionResult {
        ...
        let offloadsDir = RootfsManager.shared.dataPath
            .appendingPathComponent("var/jarvis/offloads", isDirectory: true)
        ...
        let envGuestPath = "/var/jarvis/offloads/\(envFilename)"
...
extension BrowserUseManager: WKNavigationDelegate {
    ...
    nonisolated func webView(
        ...
        let allowedSchemes: Set<String> = ["http", "https", "about", "blob", "jarvis"]
            ...
            Task { @MainActor [weak self] in
                ...
                if let cont = self.navigationContinuation {
                    ...
                    cont.resume(throwing: URLError(.unsupportedURL, userInfo: [
                        NSLocalizedDescriptionKey: "Blocked navigation to \(scheme):// — only http(s) and jarvis:// are allowed in the browser."
                    ]))
                }
            }
            return
...
/// Handles `jarvis://` URLs in WKWebView by resolving them to local files.
/// Supports `jarvis://workspace/file.html`, `jarvis://shared/...`, etc.
...
final class BrowserDownloadCenter: ObservableObject {
    ...
    func began(sessionId: String, filename: String, progress: Progress,
        ...
        queueAgentEvent(sessionId: sessionId, filename: filename,
                        line: "\(filename): download started → saving to jarvis://workspace/\(filename)")
        return id
    }
    ...
    func finished(id: UUID, sizeText: String) {
        ...
        queueAgentEvent(sessionId: e.sessionId, filename: e.filename,
                        line: "\(e.filename): download COMPLETED (\(sizeText)) → jarvis://workspace/\(e.filename) (shell path: /var/jarvis/workspace/\(e.filename))")
    }
    ...
    func agentReport(for sessionId: String) -> String? {
        ...
        var lines: [String] = inflight.map { e in
            ...
            return "\(e.filename): downloading \(pct) (\(done) / \(total)) → saving to jarvis://workspace/\(e.filename)"
        }
        ...
        return "[browser_downloads] The browser is handling file download(s) NATIVELY "
            ...
            + "shell `ls -l /var/jarvis/workspace/`) instead of downloading in parallel."
    }
