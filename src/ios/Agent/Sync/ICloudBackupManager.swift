...
final class ICloudBackupManager: ObservableObject {
    ...
    private var sessionMinisURL: URL { minisBaseURL.appendingPathComponent("jarvis", isDirectory: true) }
    ...
    private func stageSessionFiles(to dir: URL) async throws {
        ...
        if fm.fileExists(atPath: sessionMinisURL.path) {
            try fm.copyItem(at: sessionMinisURL, to: dir.appendingPathComponent("jarvis"))
        }
    }
    ...
    private func restoreSessionFiles(from dir: URL) async throws {
        ...
        let minisSrc = dir.appendingPathComponent("jarvis")
