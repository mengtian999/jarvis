...
enum ChatStoreSyncHydrators {
    ...
    static func registerAll() async {
        ...
        h.register(
            recordType: "RoleV2",
            builder: { id in await buildRole(id: id) },
            merger: { record in await mergeRole(record: record) }
        )
        h.register(
            recordType: "RolesCollectionV2",
            builder: { _ in await buildRolesCollection() },
            merger: { record in await mergeRolesCollection(record: record) }
        )
    ...
    private static func mergeSession(record: PortableRecord) async {
        ...
        // [T-role-session-bound] Same presence-vs-value distinction: only a
        // peer that knows the roleId field may seed/clear the local binding.
        let roleFieldPresent = record.fields["roleId"] != nil
        let sessRoleId = optionalStringField(record, "roleId")
        ...
        await ChatStore.shared.mergeRemoteSession(
            ...
            remoteRoleId: sessRoleId,
            remoteHasRoleField: roleFieldPresent
        )
    }
    ...
    private static func mergeMessage(record: PortableRecord) async {
        ...
        await ChatStore.shared.mergeRemoteMessage(
            ...
            updatedAt: updatedAt,
            // [T-role-message-level] additive optional: old peers decode nil.
            roleId: optionalStringField(record, "roleId")
