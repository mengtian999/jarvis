...
actor SessionFileChangeTracker {
    ...
    /// file:
    ///
    ///   • Guest path:  `/var/jarvis/<subdir>/<rel>` (fakefs path-normalize
...
///
/// iSH stays agnostic about Jarvis sessions — this Swift bridge knows the
/// per-session host directory layout (`<minisBaseURL>/<sid>/<subdir>/...`)
/// and the bind-mount guest layout (`/var/jarvis/<subdir>/...`), and resolves
...
func installSessionFileChangeTracker(kernel: ISHKernel) {
    ...
    installLogger.info("[SessionFileTracker] installing fakefs change handler — iSH realfs writes/unlinks/renames under /var/jarvis/{workspace,attachments,browser,offloads} (or the equivalent host APFS path) will be queued for iCloud SessionFile sync")
    ...
    kernel.installFakefsChangeHandler { events in
        ...
        // path under `/var/jarvis/skills/` (or the equivalent host APFS
...
fileprivate enum FakefsSkillPathCheck {
    static func batchTouchesSkills(events: [ISHFakefsChangeEvent], minisBaseURL: URL) -> Bool {
        let guestPrefix = "/var/jarvis/skills"
...
final class SkillFilesystemNotifier {
    ...
    /// True when at least one fakefs event touching /var/jarvis/skills/
