...
struct SyncedSession: Syncable {
    ...
    var pinnedAt: Date?
    var folderId: String?
    /// [T-role-session-bound] Persona bound to the session. Synced field
    /// (cross-device consistent), added like folderId — old devices decode
    /// it as nil both ways, which mergeRemoteSession tolerates via
    /// remoteHasRoleField=false.
    var roleId: String?
    ...
    static let syncMetadata: SyncTypeMetadata<SyncedSession> = {
        ...
        return SyncTypeMetadata<SyncedSession>(
            ...
            fields: [
                ...
                // [T-role-session-bound] same additive pattern as folderId.
                F.optionalString("roleId",   \SyncedSession.roleId),
            ],
...
struct SyncedMessage: Syncable {
    ...
    var createdAt: Date
    var updatedAt: Date
    /// [T-role-message-level] Generating persona for assistant rows (nil for
    /// user/system and for records from pre-role builds). Additive optional
    /// field, same decode-as-nil tolerance as SyncedSession.roleId.
    var roleId: String?
    ...
    static let syncMetadata: SyncTypeMetadata<SyncedMessage> = {
        ...
        return SyncTypeMetadata<SyncedMessage>(
            ...
            fields: [
                ...
                // [T-role-message-level] additive optional field.
                F.optionalString("roleId",            \SyncedMessage.roleId),
...
/// [T-mcp-integration-ios] Whole-file sync of /var/jarvis/mcp-servers/servers.json
...
enum SyncedTypesBootstrap {
    static func registerAll() {
        ...
        // 角色同步类型
        r.register(SyncedRole.self)           // 单个角色配置
        r.register(SyncedRolesCollection.self) // 角色集合索引
    }
}
