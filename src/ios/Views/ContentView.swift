...
struct ContentView: View {
    ...
    // [T-role-session-bound] Role binding rides in the draft id exactly like
    // the model group: encoded at creation ("以此角色新建会话"), parsed back at
    // AIChatView construction, consumed by ensureSessionReturningId at first
    // send.
    private static let roleSeparator = "__role__"

    private static func makeNewSessionId(roleId: String) -> String {
        "\(newSessionPrefix)\(UUID().uuidString)\(roleSeparator)\(roleId)"
    }

    private static func extractRoleId(from id: String) -> String? {
        guard let range = id.range(of: roleSeparator) else { return nil }
        // Tolerate a __grp__ segment after the role segment (both orders).
        let raw = String(id[range.upperBound...])
        let roleId = raw.components(separatedBy: groupSeparator).first ?? raw
        return roleId.isEmpty ? nil : roleId
    }
    ...
    /// back to "Jarvis". Refreshed whenever SoulStore posts .soulMdChanged.
    @State private var soulName: String = SoulStore.cachedMetadata.name.isEmpty
        ? "Jarvis" : SoulStore.cachedMetadata.name
    /// Subtitle state shown under the "Jarvis" sidebar title. nil hides the
    ...
    var body: some View {
        ...
        // [T-role-session-bound] Drain the chat top-bar role guide requests:
        // "以此角色新建会话" opens a role-bound draft (same funnel as
        // newChatInFolder, but with a __role__-encoded draft id); "查看该角色
        // 的历史会话" sets the list's roleFilter. Values are one-shot — cleared
        // on consume.
        .onReceive(RoleNavigationBus.shared.$newChatWithRole) { roleId in
            guard let roleId else { return }
            RoleNavigationBus.shared.newChatWithRole = nil
            let draftId = Self.makeNewSessionId(roleId: roleId)
            if isWideLayout {
                openSession(draftId)
            } else {
                commitNavigationPath(NavigationPath([draftId]))
                currentStackSessionId = draftId
            }
        }
        .onReceive(RoleNavigationBus.shared.$showRoleHistory) { roleId in
            guard let roleId else { return }
            RoleNavigationBus.shared.showRoleHistory = nil
            roleFilter = roleId
            // Pop back to the list so the filtered result is visible.
            if isWideLayout {
                currentStackSessionId = nil
            } else if !navigationPath.isEmpty {
                commitNavigationPath(NavigationPath())
                currentStackSessionId = nil
            }
        }
        // [T-role-forward] Forward sheet confirmed → jump straight to the
        // target chat (same atomic path-replacement as move-to). The target's
        // AIChatView drains the auto-respond marker on appear.
        .onReceive(NotificationCenter.default.publisher(for: .forwardDidComplete)) { note in
            guard let target = note.userInfo?["targetSessionId"] as? String else { return }
            if isWideLayout {
                openSession(target)
            } else {
                commitNavigationPath(NavigationPath([target]))
                currentStackSessionId = target
            }
        }
        ...
        // WebApp deep link, a `.jarvisbak` opened from Files). iOS will not
        ...
        .onChange(of: selectedSessionId) { newValue in
            ...
            // Jarvis-2026-06-01-134710.ips). Suspend the outgoing vm here,
        ...
        .onChange(of: navigationPath) { _ in
            ...
            //
            // Crash 2026-08-10 19:23 (Jarvis 1.12(1), iOS 26.5.2, iPhone18,1 —
            ...
            sessionList(useNavigationLinks: true)
                .navigationDestination(for: String.self) { id in
                    ...
                    if id.hasPrefix("remote:") {
                        ...
                        AIChatView(sessionId: Self.isNewSessionId(id) ? nil : id, draftId: Self.isNewSessionId(id) ? id : nil, initialGroupId: Self.extractGroupId(from: id), initialRoleId: Self.extractRoleId(from: id))
                            .id(id)
                            .onAppear {
    ...
    private var detailView: some View {
        if let id = selectedSessionId {
            ...
            AIChatView(sessionId: effectiveId, draftId: isDraft ? id : nil, initialGroupId: Self.extractGroupId(from: id), initialRoleId: Self.extractRoleId(from: id))
                .id(id)
                .onAppear {
    ...
    /// [T-role-filter] roleFilter (set via "查看该角色的历史会话") ANDs with
    /// the search filter; nil = no role constraint.
    @State private var roleFilter: String? = nil

    private var filteredSessions: [ChatSession] {
        var list = sessions
        if let rid = roleFilter {
            list = list.filter { $0.roleId == rid }
        }
        guard let matchedIds = searchMatchedIds else { return list }
        return list.filter { matchedIds.contains($0.id) }
    }
    ...
    private func sessionList(useNavigationLinks: Bool) -> some View {
        ...
        // [T-role-filter] Active role filter chip — shown when the list was
        // opened via "查看此角色的历史会话". Tapping ✕ clears the filter and
        // restores the full list. safeAreaInset keeps it above the list
        // without touching either List's section structure.
        .safeAreaInset(edge: .top, spacing: 0) {
            if let rid = roleFilter {
                HStack(spacing: 6) {
                    RoleAvatarImage(roleId: rid, size: 18)
                    Text(RoleStore.roleById(rid)?.metadata.name ?? "✨")
                    ...
                    .accessibilityLabel(Text("Clear role filter"))
                }
    ...
    private func refreshMigrationSubtitle() {
        ...
        Task { @MainActor in
            ...
            if !byType.isEmpty {
                ...
                // "Jarvis" title sits at its normal size.
                if migrationSubtitle != nil { migrationSubtitle = nil }
            }
        }
    }
    ...
    /// Tiny indicator next to the "Jarvis" title showing the sync state.
    @ViewBuilder
    ...
    private var sidebarToolbarContent: some ToolbarContent {
        ToolbarItem(placement: .principal) {
            if isSelecting {
                ...
                // Title text comes from SOUL.md (falls back to "Jarvis"). The
...
private struct SessionRow: View, Equatable {
    ...
    private var providerIcon: some View {
        // [T-role-avatar] Leading icon = the session's bound role avatar
        // (roleId → snapshot → default-✨ fallback), replacing the category
        // glyph. RoleAvatarImage observes .rolesChanged so role edits/deletes
        // re-render rows live. Falls through to the category glyph only when
        // even the default avatar has no image bytes.
        if RoleStore.resolveRole(session.roleId).avatarData != nil {
            RoleAvatarImage(roleId: session.roleId, size: 28)
        } else {
...
private struct RemoteSessionRow: View {
    ...
    @ViewBuilder
    private var providerIcon: some View {
        // [T-role-avatar] Same role-avatar swap as SessionRow.
        if RoleStore.resolveRole(session.roleId).avatarData != nil {
            RoleAvatarImage(roleId: session.roleId, size: 28)
        } else {
            let icon = categoryIcon
...
private struct SettingsSheet: View {
    ...
    fileprivate static func makeBugReportURL() -> URL? {
        ...
        | Jarvis Version | \(appVersion) (build \(build)) |
        | Device Model | \(device) |
