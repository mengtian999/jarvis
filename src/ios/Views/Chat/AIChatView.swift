...
struct AIChatView: View {
    ...
    /// [T-role-session-bound] Persona to bind when creating a new session
    /// ("以此角色新建会话"). Encoded in the draft id like the group id;
    /// seeded into vm.initialRoleId → persisted at draft promotion.
    ...
    init(sessionId: String? = nil, draftId: String? = nil, remoteDeviceId: String? = nil, initialGroupId: String? = nil, initialRoleId: String? = nil) {
        self.sessionId = sessionId
        self.draftId = draftId
        self.remoteDeviceId = remoteDeviceId
        self.initialGroupId = initialGroupId
        self.initialRoleId = initialRoleId
    ...
    /// from SOUL.md (`name`), falls back to "Jarvis". Refreshed on .soulMdChanged.
    @State private var soulName: String = SoulStore.cachedMetadata.name.isEmpty
        ? "Jarvis" : SoulStore.cachedMetadata.name
    /// [T-role-session-bound] Top-bar role guide (avatar chip tap): read-only
    /// persona display with "new chat with this role / view role history".
    /// Never switches THIS session's persona.
    @State private var showRoleEntrySheet = false
    // [T-role-forward] Forward-to-role sheet state. Source = the long-pressed
    /// UI message (single-message MVP; Android has full multi-select).
    @State private var showForwardSheet = false
    @State private var forwardSourceMessageId: UUID?
    @State private var forwardPickRoleId: String? = nil
    @State private var forwardTargetSessionId: String? = nil
    @State private var forwardAutoRespond = true
    @State private var forwardTargetSessions: [ChatSession] = []

    /// [T-role-session-bound] Title fallback name = the SESSION's bound role
    /// (never the global current role — a Settings switch mid-session must
    /// not re-label an open chat). Empty role name degrades to the Soul name.
    private var sessionRoleFallbackName: String {
        let n = vm.sessionRole.metadata.name
        return n.isEmpty ? soulName : n
    }
    ...
    // jarvis:// link preview sheet state — hoisted from MinisOpenURLHandler so
    ...
    /// when `resolveMinisFileURL` returns nil for a tapped `jarvis://` link
    ...
    var body: some View {
        ...
        // [T-role-session-bound] Top-bar role guide. The session persona is
        // read-only: the only in-session affordance is jumping OUT to a new
        // chat bound to the same role or to that role's history. Switching
        // personas means a new session, by design. Navigation is executed by
        // ContentView via the RoleNavigationBus (this view can't reach the
        // parent NavigationStack).
        .sheet(isPresented: $showRoleEntrySheet) {
            RolePickerSheet(vm: vm)
        }
        // [T-role-forward] Role/target picker for the long-pressed message.
        // Step 1: target role. Step 2: destination (new chat / one of that
        // role's sessions) + auto-respond toggle. Confirm copies the message
        // into the target as a user row and (when enabled) auto-starts the
        // target persona's reply after navigation.
        .sheet(isPresented: $showForwardSheet) {
            ForwardToRoleSheet(
                vm: vm,
                sourceMessageId: forwardSourceMessageId,
                pickRoleId: $forwardPickRoleId,
            ...
            if let sid = vm.sessionId {
                NavigationStack {
                    FileBrowserView(
                        ...
                        rootLabel: "/var/jarvis/workspace",
        ...
        //  - jarvis://...: chat-resource file preview (image/markdown/html/
        ...
        .sheet(isPresented: $showFileBrowser) {
            NavigationStack {
                ...
                FileBrowserView(rootPath: base, initialPath: base.appendingPathComponent("var/jarvis"), rootLabel: "/")
            }
        }
        ...
        .onAppear {
            ...
            // [T-role-session-bound] Draft role binding ("以此角色新建会话");
            // consumed at first send (ensureSessionReturningId).
            vm.initialRoleId = initialRoleId
            ...
            // [T-role-forward] Drain an auto-respond request stashed by the
            // source session's forward flow: the forwarded rows are already in
            // the DB this loadSession just read — start the target persona's
            // reply without the user tapping send. Wait for the initial load
            // so the loop sees the forwarded history.
            if let sessionId, ViewModelCache.shared.consumePendingForwardRespond(sessionId: sessionId) {
                let targetVM = vm
                Task { @MainActor in
                    while !targetVM.hasCompletedInitialLoad && !Task.isCancelled {
                        try? await Task.sleep(nanoseconds: 100_000_000)
                    }
                    targetVM.respondToForwardedHistory()
                }
            }
    ...
    private var navTitleKey: NavTitleKey {
        ...
        return NavTitleKey(
            ...
            soulName: sessionRoleFallbackName,
    ...
    private var titleView: some View {
        ...
        // the SOUL.md `name` (or "Jarvis") for draft sessions or before a
        ...
        // (only descender-less titles like "Jarvis" hid it). -3 keeps a ~2pt
        ...
        return VStack(spacing: legacyLayout ? -3 : -2) {
            // [T-role-session-bound] Read-only role avatar chip ahead of the
            // title. Tap opens the role guide (new chat with this role / view
            // role history) — it never switches THIS session's persona. The
            // title Button keeps its title-edit behavior.
            HStack(spacing: 4) {
                Button {
                    showRoleEntrySheet = true
                } label: {
                    RoleAvatarImage(roleId: vm.sessionRole.roleId, size: 18)
                }
                .buttonStyle(.plain)
                .accessibilityLabel(Text("Session role"))
                Button {
                ...
                // size unchanged.
                Text(sessionTitle ?? sessionRoleFallbackName)
        ...
        Task {
            ...
            await MainActor.run {
                ...
                if next?.id != titlePillSession?.id
                    ...
                    // old "Jarvis" string slide before the real title swaps
    ...
    private var messagesArea: some View {
        ZStack {
            CollectionViewMessageListV3(
                ...
                // handler every `jarvis://` link in the transcript uses, so the
                // identity row and an agent-authored
                // `[Soul](jarvis://settings/soul)` link land identically —
                // no second navigation path to keep in sync.
                onOpenSoulSettings: {
                    guard let url = URL(string: "jarvis://settings/soul") else { return }
                    _ = handleMinisURLTap(url)
                },
                ...
                // [T-role-forward] Long-press "Forward to Role": stash the UI
                // message id and present the role/target picker sheet.
                onForwardMessage: { msgId in
                    forwardSourceMessageId = msgId
            ...
            // per-session vs cross-session /var/jarvis/ layout. Only shown
    ...
    /// Handle a jarvis:// or http(s):// URL tap forwarded from a cell-level
    ...
    /// Load a markdown image source (already canonicalized to jarvis:// or
    ...
    private func handleMinisURLTap(_ url: URL) -> OpenURLAction.Result {
        ...
        guard url.scheme == "jarvis" else { return .systemAction }

        // Terminal deep link: jarvis://open_terminal?init_command=...
        ...
        // View deep links: jarvis://views/alarm
        if url.host == "views" && url.path == "/alarm" {
            DeepLinkCoordinator.shared.showAlarmList = true
            return .handled
        }
        // Any jarvis://settings/<path> goes through DeepLinkRouter so all
...
/// `/var/jarvis/` as a 2-column folder grid so the user understands what's
/// available before typing.
///
/// Source-of-truth for the descriptions: AIChatViewModel.swift system-prompt
/// directory listing (`Shared directory /var/jarvis/ ...`). Keep them aligned
