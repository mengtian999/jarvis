...
final class MinisMediaCache {
    ...
    /// Downsampled UIImage cache keyed by absolute jarvis:// URL string.
    private let imageCache = NSCache<NSString, UIImage>()
    /// Video thumbnail cache keyed by file path.
    private let thumbnailCache = NSCache<NSString, UIImage>()
    /// Resolved host file URL cache keyed by jarvis:// URL string.
    ...
    /// was previously uncached, and for a `jarvis://mounts/<name>/…` source the
    ...
    /// after we write to a jarvis:// path ourselves, where waiting out the TTL
...
/// Resolve a jarvis:// URL with caching to avoid repeated FileManager lookups.
///
/// Cache key is scoped by the active session id because per-session paths
/// (e.g. `jarvis://attachments/foo.png`) resolve to different files in
...
///
/// For `jarvis://` sources we resolve to the host file URL and stat it.
...
func minisMediaCacheKey(for url: URL) -> String {
    ...
    guard url.scheme == "jarvis" else { return base }
...
/// Resolve a jarvis:// URL to a host filesystem URL.
...
func resolveMinisFileURL(url: URL) -> URL? {
    ...
    // User-mounted external folders: jarvis://mounts/<name>/<path>
    ...
    // jarvis://attachments/<file> is session-scoped: it must only resolve
...
/// Tappable file chip for jarvis:// links in Markdown.
...
struct AsyncImageTile: View {
    ...
    var body: some View {
        ...
        // the underlying jarvis:// file invalidates the displayed
    ...
    private func loadThumbnail() {
        ...
        // same jarvis:// path invalidates the cache automatically.
...
/// Environment action for handling jarvis:// and http(s):// URL taps.
///
...
// MARK: - jarvis:// Image File Preview (from URL handler)
...
/// already-resolved disk URL directly (no jarvis:// re-resolve).
