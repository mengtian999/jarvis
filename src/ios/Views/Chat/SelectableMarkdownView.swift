...
// MARK: - jarvis:// URL encoding [T-minis-url-fullwidth-pipe-ios]

/// Percent-encode the path of a `jarvis://` destination that contains raw,
/// unencoded bytes (e.g. a filename with U+FF5C `｜`, CJK, emoji the LLM emitted
/// unencoded inside a jarvis:// link), returning a parseable URL (or nil if it
...
fileprivate final class MarkdownNSRenderer {
    ...
    /// was 27 s of main-thread typesetter pin (ips Jarvis-2026-05-14-094312).
    ...
    private func renderBulletedList(isTight: Bool, items: [RawListItem], listDepth: Int, quoteDepth: Int) -> RenderedBlock {
        ...
        // device hit a fresh 0x8BADF00D watchdog (ips Jarvis-2026-05-14-
    ...
    /// minis_url (e.g. `jarvis://workspace/GitHub%E7%83%AD….md`). When the model
    ...
    private func normalizedLinkURL(from destination: String) -> URL? {
        ...
        // Only jarvis:// links are affected; everything else is untouched.
        guard destination.hasPrefix("jarvis://") else {
            return fallback
        }
        ...
        // emitted unencoded inside a jarvis:// link). The link would otherwise be
...
final class ImageAttachment: NSTextAttachment {
    ...
    /// Set when a jarvis:// file was not found; allows retry on next render.
    ...
    /// `./foo.png`, `subdir/bar.jpg`) become `jarvis://workspace/<path>`
    ...
    static func canonicalizeMarkdownImageSource(_ src: String) -> String {
        ...
        if let colon = src.firstIndex(of: ":") {
            ...
            if !maybeScheme.isEmpty && maybeScheme.allSatisfy({ $0.isLetter }) {
                // [T-minis-url-fullwidth-pipe-ios] Exception: a jarvis:// source the
                ...
                if src.hasPrefix("jarvis://"), URL(string: src) == nil,
        ...
        return "jarvis://workspace/\(encoded)"
    }
    ...
    func beginLoadingIfNeeded() {
        // Canonicalize scheme-less / relative sources to jarvis://workspace/…
        ...
        // of the same jarvis:// path misses and falls through to a fresh decode.
        ...
        let isMinisURL = URL(string: src).map { $0.scheme == "jarvis" } ?? false
        ...
        // recycle — render the placeholder and bail. jarvis:// is exempt: those
        ...
        Task.detached(priority: .userInitiated) {
            var fileURL: URL?
            let img: UIImage?
            if let url = URL(string: src), url.scheme == "jarvis" {
                imgLogger.info("[MinisImage][Load] resolving jarvis:// URL src=\(src) host=\(url.host ?? "nil") path=\(url.path)")
                if let resolved = resolveMinisFileURLForNativeText(url: url) {
                    imgLogger.info("[MinisImage][Load] jarvis:// resolved to localPath=\(resolved.path)")
                    fileURL = resolved
                ...
                // jarvis:// branch. HTTP(S) is deliberately NOT probed — a
            ...
            if let img {
                ...
                // jarvis:// is exempt — its not-yet-written-file case is handled
...
final class AudioAttachment: NSTextAttachment {
    ...
    private func resolveURL() {
        ...
        if let url = URL(string: cleanSource), url.scheme == "jarvis" {
...
final class SelectableMarkdownTextView: UITextView, UIGestureRecognizerDelegate {
    ...
    // MARK: - [T-selection-menu-minis-tts] Jarvis TTS menu actions

    /// Read the WHOLE reply from the start via the Jarvis TTS stack
    ...
    /// Speak an arbitrary text snippet via the Jarvis TTS stack (vm.speakText).
    ...
    override func buildMenu(with builder: UIMenuBuilder) {
        ...
        // / "Spell") — it reads with the OS voice, bypassing the Jarvis TTS
        ...
        // [T-selection-menu-minis-tts] Jarvis-owned read-aloud entries. "Read
    ...
    func updateAttachmentViews() {
        ...
        textStorage.enumerateAttribute(.attachment, in: fullRange, options: []) { value, range, _ in
            ...
            if let existingView = previousViewMap[attachId] {
                ...
                // case taps fell through to UIApplication.open and jarvis://
    ...
    override func layoutSubviews() {
        ...
        // Jarvis-2026-05-13-084827.ips: every layout pass re-triggers a full
...
struct SelectableMarkdownView: UIViewRepresentable {
    ...
    var onCopyScreenshot: (() -> Void)?
    /// [T-selection-menu-minis-tts] "Read from Start" (whole reply) via Jarvis TTS.
    var onReadAloud: (() -> Void)?
    /// [T-selection-menu-minis-tts] "Read Selection" (selected text) via Jarvis TTS.
    var onSpeakText: ((String) -> Void)?
    ...
    func sizeThatFits(_ proposal: ProposedViewSize, uiView: SelectableMarkdownTextView, context: Context) -> CGSize? {
        ...
        // message (user report, macOS, "Jarvis Feedback Review" — the shell
        ...
        // recorded in `Jarvis-2026-05-13-084827.ips` (depth=9 recursion at
    ...
    final class Coordinator: NSObject, UITextViewDelegate {
        ...
        /// `Jarvis-2026-05-13-084827.ips`: it rebuilds attachment subviews
...
// MARK: - Jarvis URL Resolution (bridged from AIChatView)

/// Resolves a `jarvis://` URL to a local file URL.
...
private func resolveMinisFileURLForNativeText(url: URL) -> URL? {
    ...
    // same tolerant resolution the other 4 jarvis:// resolvers already use; this
    ...
    // model-use-zimage-0.jpg would resolve to session A's image. jarvis:// is
    // session-scoped; not-found is the correct result for a cross-session ref.
    //
    // The rootfs `/var/jarvis/<host>` fallback is kept ONLY for the global,
    ...
    if globalHosts.contains(host) {
        for subPath in subPaths {
            let rootfsURL = RootfsManager.shared.dataPath
                .appendingPathComponent("var/jarvis", isDirectory: true)
                .appendingPathComponent(host, isDirectory: true)
                .appendingPathComponent(subPath)
