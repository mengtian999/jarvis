    ...
    var body: some View {
        ...
        .alert(
            ...
            Button(AppLocalized("Go Enable")) {
                ...
                // (= jarvis://settings/background?focus=…:true,…:true). Location
...
struct TypingIndicator: View {
    ...
    /// [T-role-session-bound] The SESSION's bound persona name — passed by the
    /// caller from AIChatViewModel.sessionRole so a global role switch in
    /// Settings mid-session never relabels an in-flight reply. nil (legacy
    /// call sites) falls back to the Soul name, the pre-refactor behavior.
    var sessionRoleName: String? = nil
    /// Live Soul name fallback — updates via `.soulMdChanged` Notification.
    ...
    private var displayName: String {
        let n = (sessionRoleName ?? soulName).trimmingCharacters(in: .whitespacesAndNewlines)
        return n.isEmpty ? "Jarvis" : n
    }
