...
struct ChatMessageRow: View {
    ...
    /// [T-role-session-bound] Session persona display name — drives the
    /// typing indicator ("<role> is thinking"). nil = legacy fallback to the
    /// Soul name.
    var sessionRoleName: String? = nil
    /// [T-role-forward] Long-press "Forward to role chat" — opens the
    /// role/target picker in AIChatView for THIS message.
    ...
    private func systemDividerRow(icon: String?, loading: Bool, compact: Bool) -> some View {
        HStack(spacing: 10) {
            ...
            // systemInfo line (e.g. pre-v3 "Downloaded … to /var/jarvis/… —
            // jarvis://…" rows persisted in history) overflowed BOTH screen
    ...
    private var userRow: some View {
        HStack {
            ...
            .contextMenu {
                ...
                // [T-role-forward] Single-message forward → role picker sheet.
                if let onForward {
                    Button {
                        onForward()
                    } label: {
                        Label(String(localized: "Forward to Role"), systemImage: "paperplane")
                    }
                }
    ...
    private var assistantRow: some View {
        VStack(alignment: .leading, spacing: 8) {
            // [T-role-message-level] Assistant label resolves the persona that
            // generated THIS message (message.roleId → snapshot → default), so
            // history keeps its per-message roles across global switches and
            // role deletions.
            AssistantMessageHeader(roleId: message.roleId)
                .padding(.top, 4)
            ...
            if isActiveMessage && message.shouldShowTypingIndicator {
                TypingIndicator(sessionRoleName: sessionRoleName)
            }
            ...
            Color.clear
                ...
                .contextMenu {
                    ...
                    EquatableMenuGate(key: AssistantMenuKey(
                        ...
                        // [T-role-forward] Single-message forward → role
                        // picker sheet.
                        if let onForward {
                            Button {
                                onForward()
