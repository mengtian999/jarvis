...
struct WebLoadError: Equatable {
    ...
    init?(error: Error, failedURL: URL? = nil) {
        ...
        case (NSURLErrorDomain, NSURLErrorCannotFindHost),
            ...
            message = host.map {
                AppLocalized("Jarvis can’t open the page because it can’t find the server “\($0)”.")
            } ?? AppLocalized("Jarvis can’t open the page because it can’t find the server.")
            systemImage = "wifi.exclamationmark"

        case (NSURLErrorDomain, NSURLErrorCannotConnectToHost):
            title = AppLocalized("Cannot Open Page")
            message = AppLocalized("Jarvis can’t open the page because it can’t connect to the server.")
            systemImage = "wifi.exclamationmark"
        ...
        case (NSURLErrorDomain, NSURLErrorSecureConnectionFailed),
            ...
            message = host.map {
                AppLocalized("Jarvis can’t verify the identity of the server “\($0)”.")
            } ?? AppLocalized("Jarvis can’t verify the identity of the server.")
