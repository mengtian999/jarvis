...
struct AssistantMenuKey: Equatable {
    ...
    let hasCompact: Bool
    let isActive: Bool
    /// [T-role-forward] "Forward to Role" availability — kept in the key so
    /// the gated menu rebuilds when the bridge wires/unwires the action.
    let hasForward: Bool = false
}
