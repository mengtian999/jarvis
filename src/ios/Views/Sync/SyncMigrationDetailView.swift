...
struct SyncMigrationDetailView: View {
                ...
                Section {
                    ...
                    if m.isSuspended {
                        HStack(spacing: 8) {
                            ...
                            Text("Paused — reopen Jarvis to continue")
                                .font(.caption)
                                .foregroundStyle(.secondary)
                        }
                    } else {
    ...
    private var iCloudZonesSection: some View {
        Section {
            ...
            Text("Every zone Jarvis has created in your iCloud private database. **V2** holds the current sync engine's data; **V1** holds legacy per-device backups from older builds. Deleting a zone is permanent and removes everything inside (records + assets). Use this to reclaim space after migration completes.")
                .font(.caption)
        }
    }
    ...
    private func zoneDescription(for row: ZoneRow) -> String {
        ...
        case .system:
            return AppLocalized("CloudKit built-in zone. Not used by Jarvis.")
        case .other:
    ...
    private func refreshZones(force: Bool) async {
        ...
        do {
            ...
            zonesList = zones
                ...
                // private database. Jarvis never writes to it and CK refuses
