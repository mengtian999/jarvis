...
struct ThinkingRulesSection: View {
    ...
    /// explains that thinking parameters here are maintained by Jarvis by design.
    private var unsupportedNoticeSection: some View {
        Section {
            HStack(alignment: .firstTextBaseline, spacing: 8) {
                Image(systemName: "info.circle")
                    .foregroundStyle(.secondary)
                Text("This provider uses an official protocol, so its thinking parameters are maintained by Jarvis. Custom rules apply to OpenAI-compatible providers on the Chat Completions API.")
                    .font(.footnote)
                    .foregroundStyle(.secondary)
            }
        } header: {
