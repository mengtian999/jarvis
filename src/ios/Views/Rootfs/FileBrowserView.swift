...
struct FileBrowserView: View {
    ...
    var body: some View {
        ...
        .sheet(item: $moveOrCopyItem) { item in
            let jarvisPath = viewModel.rootPath.appendingPathComponent("var/jarvis")
            let initial = FileManager.default.fileExists(atPath: jarvisPath.path) ? jarvisPath : nil
            NavigationStack {
                DirectoryPickerView(
...
private struct HTMLFilePreview: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> WKWebView {
        ...
        config.setURLSchemeHandler(BrowserUseManager.sharedMinisSchemeHandler, forURLScheme: "jarvis")
        ...
        FileItemRow(item: item, onTap: onTap, onExport: onExport)
            .contextMenu {
                Button {
                    ...
                    // absolute path (e.g. /var/jarvis/workspace/.../L3_0001.png)
            ...
            .sheet(isPresented: $showAddWebApp) {
                ...
                // without going through jarvis:// resolution.
...
class FileBrowserViewModel: ObservableObject {
    ...
    func loadItems() {
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            do {
                ...
                // (e.g. /var/jarvis/attachments -> Library/MinisChat/...)
    ...
    func deleteItem(_ item: FileItem) {
        ...
        do {
            ...
            if let linux = linuxPath(for: removedURL) {
                ...
                // trees (/var/jarvis/{shared,skills,memory}/...), the Files app has
    ...
    /// Maps a Linux path under `/var/jarvis/{shared,skills,memory}/...` to the
    ...
    private static let fileProviderExposedRoots: [String: NSFileProviderItemIdentifier] = [
        "/var/jarvis/shared":  NSFileProviderItemIdentifier("shared"),
        "/var/jarvis/skills":  NSFileProviderItemIdentifier("skills"),
        "/var/jarvis/memory":  NSFileProviderItemIdentifier("memory"),
    ]
    ...
    func displayPath(for url: URL) -> String {
        ...
        // directory we're browsing, e.g. `.../data/var/jarvis/browser`) + the
        ...
        // symlink-RESOLVED. For a bind-mounted subdir like `/var/jarvis/browser`
        ...
        // (`0419….jpg`) instead of `/var/jarvis/browser/0419….jpg`.
        ...
        if curStd == rawRootEarly || curStd.hasPrefix(rawRootEarly + "/") {
            let relDir = String(curStd.dropFirst(rawRootEarly.count))   // "" or "/var/jarvis/browser"
        ...
        // UN-resolved namespace: the listing keeps the logical `/var/jarvis/…`
    ...
    /// plus a rel of "/var/jarvis/x" yields "/var/jarvis/x" (not "//var/jarvis/x")
