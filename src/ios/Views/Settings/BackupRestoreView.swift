...
struct BackupRestoreView: View {
    ...
    /// opening a `.jarvisbak` file from Files rather than from this screen.
    ...
    private var otherSourcesSection: some View {
        Section {
            ...
            Text("Pick a .jarvisbak file from Files, iCloud Drive, or any connected storage — or add a server your backups were saved to.")
        }
    }
    ...
    private func detailText(_ c: BackupCategory, _ stat: BackupManifest.CategoryStat) -> String {
        ...
        case .mcpServers:
            ...
            return "\(servers) · \(size)"
        case .roles:
            let roles = countLabel(stat.entries,
                                   one: AppLocalized("1 role"),
                                   many: AppLocalized("%d roles"))
            return "\(roles) · \(size)"
        case .providers:
...
/// The `.jarvisbak` packages inside a single mounted destination folder.
///
    ...
    var body: some View {
        List {
            ...
            if loading {
                ...
                VStack(spacing: 10) {
                    Image(systemName: "externaldrive.badge.questionmark")
                        .font(.system(size: 40))
                        .foregroundStyle(.secondary)
                    Text("No .jarvisbak files in this folder.")
                        .font(.headline)
            ...
            ForEach(packages) { pkg in
                ...
                if !packages.isEmpty {
                    Text("Showing .jarvisbak files in this folder.")
                }
            }
        }
        ...
        VStack(spacing: 0) {
            ...
            List {
                Section {
                    ...
                    Text("Open folders to browse. Only .jarvisbak files are shown.")
                }
            }
            .refreshable { await reload() }
        }
