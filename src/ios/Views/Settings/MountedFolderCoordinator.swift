...
enum MountedFolderCoordinator {
    ...
    /// Checks both the fakefs symlink path (/var/jarvis/mounts/<name>/...) and
    ...
    /// Returns true if `linuxPath` (a `/var/jarvis/mounts/<name>/...` path from
    /// the iSH namespace) is under a mount entry that is effectively read-only.
    /// Compares by mount name under `/var/jarvis/mounts/`, so it's immune to the
