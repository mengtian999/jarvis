...
struct BackupSettingsView: View {
    ...
    /// [T-backup-roles-category] Role ids UNCHECKED in the expandable Roles
    /// sub-list. Empty = every role ships. The default role is never added.
    @AppStorage("backup.excludedRoleIds") private var excludedRoleIdsStore: String = ""
    @State private var excludedRoleIds: Set<String> = []
    ...
    var body: some View {
        Form {
            ...
            Section {
                ...
                ForEach(BackupCategory.backupable, id: \.self) { category in
                    // [T-backup-roles-category] Roles gets an expandable
                    // sub-list of the individual personas; every other
                    // category is a plain toggle.
                    if category == .roles {
                        rolesCategoryRow
                    } else {
            ...
            if let result {
                Section {
                    ...
                    if localCopyRemoved {
                        ...
                        Text("Saved in Jarvis ▸ Backups. Use Save to Files to copy it to iCloud Drive, a connected server, or another cloud provider.")
                    } else {
                        ...
                        Text("Saved in Jarvis ▸ Backups, and copied to \(ok) of \(deliveryResults.count) selected folder(s).")
                    }
                }
            }
        }
        ...
        // .jarvisbak from Files works from ANY screen — this view is the one
    ...
    /// [T-backup-roles-category] Restore the persisted unchecked-role set.
    func loadExcludedRoleIds() {
        guard !excludedRoleIdsStore.isEmpty else { return }
        excludedRoleIds = Set(excludedRoleIdsStore.split(separator: ",").map(String.init))
    }

    // MARK: - Roles selection

    /// The live role collection, snapshot-warmed (no per-render file IO).
    private var roles: [RoleFile] { RoleStore.rolesSnapshot }

    private var includedRoleCount: Int {
        roles.filter { !excludedRoleIds.contains($0.roleId) }.count
    }

    /// The Roles category row: the category toggle PLUS an expand chevron that
    /// opens the per-role picker. The chevron is a Button inside the Toggle
    /// label so it receives taps preferentially; the rest of the label toggles
    /// inclusion. The sub-list appears only while the category is on.
    private var rolesCategoryRow: some View {
        VStack(spacing: 0) {
            Toggle(isOn: binding(for: .roles)) {
                HStack(spacing: 12) {
                    BackupCategoryIcon(category: .roles)
                    VStack(alignment: .leading, spacing: 1) {
                        Text(displayName(.roles))
                        if selected.contains(.roles) {
                            Text("\(includedRoleCount) / \(roles.count)")
                                .font(.caption)
                                .foregroundStyle(.secondary)
                        }
                    }
                    ...
                    if selected.contains(.roles) {
                        Button {
                            rolesExpanded.toggle()
                        } label: {
                            Image(systemName: rolesExpanded ? "chevron.down" : "chevron.right")
                                .foregroundStyle(.secondary)
                        }
                        .buttonStyle(.plain)
                    }
            ...
            if selected.contains(.roles) && rolesExpanded {
                roleSelectionList
                    .padding(.leading, 40)
            }
        }
    ...
    /// The expandable per-role picker. "All roles" at the top, then one toggle
    /// per persona. The default role (Jarvis) is locked on — its toggle is
    /// disabled, because a package must always restore with at least one
    /// usable persona.
    private var roleSelectionList: some View {
        VStack(spacing: 0) {
            Toggle(isOn: Binding(
                get: { excludedRoleIds.isEmpty },
                set: { setAllRolesIncluded($0) }
            )) {
                Text(AppLocalized("All roles")).font(.subheadline)
            }
            ForEach(roles, id: \.roleId) { role in
                let isDefault = role.roleId == "default"
                Toggle(isOn: roleBinding(for: role.roleId, isDefault: isDefault)) {
                    HStack(spacing: 10) {
                        Image(systemName: "person.crop.circle")
                            .foregroundStyle(.secondary)
                        Text(role.metadata.name)
                        if isDefault {
                            Text(AppLocalized("Default"))
                                .font(.caption2)
                                .foregroundStyle(.secondary)
                        }
                    }
    ...
    /// Binding for one role's inclusion. The default role's setter is a no-op
    /// (it is always on); combined with `.disabled` on its toggle the row
    /// reads as locked.
    private func roleBinding(for roleId: String, isDefault: Bool) -> Binding<Bool> {
        Binding(
            get: { !excludedRoleIds.contains(roleId) },
            set: { on in
                guard !isDefault else { return }
                var next = excludedRoleIds
                if on { next.remove(roleId) } else { next.insert(roleId) }
                excludedRoleIds = next
                excludedRoleIdsStore = next.sorted().joined(separator: ",")
            })
    }

    /// Select-all / deselect-all. `on=true` clears the excluded set; `on=false`
    /// excludes every NON-default role (the default stays in).
    private func setAllRolesIncluded(_ on: Bool) {
        if on {
            excludedRoleIds = []
        } else {
            excludedRoleIds = Set(roles.map(\.roleId).filter { $0 != "default" })
        }
        excludedRoleIdsStore = excludedRoleIds.sorted().joined(separator: ",")
    }
    ...
    /// mounted in Files and authorised in Jarvis is just a directory, and
