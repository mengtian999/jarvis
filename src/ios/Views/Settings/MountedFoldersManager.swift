...
//  mounted into /var/jarvis/mounts/<name> via security-scoped bookmarks.
//
//  Lifecycle:
...
//  - ensureMinisSymlinks() creates /var/jarvis/mounts/<name> symlinks in fakefs
//
...
struct MountedFolderEntry: Codable, Identifiable, Equatable {
    ...
    let id: UUID
    /// User-chosen name; becomes the folder name under /var/jarvis/mounts/<name>.
    ...
    /// User intent: does the user *want* Jarvis (shell + AI + Browse Files) to
...
final class MountedFoldersManager {
    ...
    /// so it does NOT leak into iOS Files' "On My iPhone → Jarvis" view.
    ...
    /// Jarvis-internal soft lock on top of the OS-level `isWritable`. Calling
    ...
    /// Returns the entry matching a fakefs symlink path (`/var/jarvis/mounts/<name>/...`).
    ...
    private func removeMountSymlink(name: String) {
        ...
        let linkPath = dataPath
            .appendingPathComponent("var/jarvis/mounts", isDirectory: true)
