...
/// `jarvis://settings/environments?create_key=…` deep link if it wants
...
struct EnvVarsCollection: ConfigCollection {
    ...
    func add(_ payload: ConfigValue) throws -> String {
        throw ConfigError.permissionDenied(
            reason: "Use the deep link `jarvis://settings/environments?create_key=…` so the user enters the value directly"
        )
    }
