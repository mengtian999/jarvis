...
extension ConfigRegistry {
    ...
    private static func registerSoul(into r: ConfigRegistry) {
        ...
        //
        // An image may be given as a data URI, bare base64, a `jarvis://`
        ...
        r.register(ClosureField(
            ...
            description: "The Soul's identity icon — also called its avatar, its persona image, or its "
                ...
                + "  • jarvis:// resource, e.g. jarvis://attachments/icon.png or jarvis://workspace/icon.png\n"
                + "  • local path inside the minis directories, e.g. /var/jarvis/attachments/icon.png\n"
                ...
                + "  minis-config set soul.icon '\"jarvis://attachments/icon.png\"'\n"
                ...
                + "Simpler still: point at the file directly with jarvis:// or a path and skip base64 entirely.",
    ...
    private static func registerSelfMeta(into r: ConfigRegistry) {
        ...
        // /var/jarvis currently points at, or that the iOS data container UUID
        ...
        r.register(ReadOnlyField(
            ...
            description: "Chat session whose storage /var/jarvis currently points at. Each session has its own attachments/offloads/workspace — files from other sessions are not visible here. Empty when no session is mounted. Read-only.",
            valueSchema: .string(maxLength: 64),
            reader: {
