...
extension Notification.Name {
    /// Posted by `DeepLinkRouter` when `jarvis://open?session=…&path=…` lands
...
/// Parses `jarvis://` URLs into navigation actions and dispatches them
...
///   jarvis://share
///   jarvis://views/alarm
///   jarvis://open_terminal[?init_command=…]
///   jarvis://open?session=<sid>&path=<scope-prefixed-path>  (openminis.app launcher round-trip)
///   jarvis://session/<id>      (legacy singular alias)
///   jarvis://sessions/<id>     (canonical — matches minis-sessions-cli)
///   jarvis://settings
///   jarvis://settings/providers[/<instanceId>]
///   jarvis://settings/model-groups[/<groupId>]   (alias: model_groups)
///   jarvis://settings/usage                      (alias: usage-stats, usage_stats)
///   jarvis://settings/skills
///   jarvis://settings/memory
///   jarvis://settings/storage
///   jarvis://settings/mount-external             (alias: mount_external, mounts, mounted-folders, mounted_folders)
///   jarvis://settings/shared-folders             (alias: shared_folders)
///   jarvis://settings/logs
///   jarvis://settings/appearance
///   jarvis://settings/background
///   jarvis://settings/about
///   jarvis://settings/permissions
///   jarvis://settings/environments[?create_key=…&create_value=…&create_note=…]
///   jarvis://settings/rootfs                     (alias: mirrors, rootfs-management, rootfs_management)
///
...
enum DeepLinkRouter {
    ...
    static func handle(url: URL, shareCoordinator: ShareCoordinator) {
        guard url.scheme == "jarvis", let host = url.host else {
    ...
    private static func handleSettings(url: URL, coord: DeepLinkCoordinator) {
        ...
        // screen exists — so a `jarvis://settings/soul` link (or the agent
        // generating one) quietly under-delivered.
        case "soul":
            coord.pendingSettingsTarget = .soul

        // [T-mcp-oauth-deeplink] jarvis://settings/mcp-servers/<serverId> —
    ...
    /// Parses a `jarvis://open?session=…&path=…` URL fired by the
    ...
    private static func handleWebAppLauncherReturn(url: URL) {
        ...
        guard let rawPath = components?.queryItems?.first(where: { $0.name == "path" })?.value,
            ...
            deepLinkLog.warning("jarvis://open missing path")
            return
        }
        ...
        if rawPath.hasPrefix("attachments/") {
            guard let sid = session, !sid.isEmpty else {
                deepLinkLog.warning("jarvis://open path=attachments/… missing session")
                return
            }
            ...
            guard let sid = session, !sid.isEmpty else {
                deepLinkLog.warning("jarvis://open path=workspace/… missing session")
                return
            }
            ...
            guard let slash = rest.firstIndex(of: "/") else {
                deepLinkLog.warning("jarvis://open path=mount:… missing /<rest>")
                return
            }
            scope = .mount
            ...
            deepLinkLog.warning("jarvis://open unknown path prefix in \(rawPath)")
            return
        }
        ...
        deepLinkLog.info("jarvis://open scope=\(scope.rawValue) ctx=\(ctx ?? "nil") htmlPath=\(htmlPath)")
