...
enum DebugRPCBackup {
    ...
    /// `debug.backup.restore` — import a `.jarvisbak` package (stage 2, Merge).
    ///
    /// Params:
    ...
    /// `debug.backup.open` — drive the "opened a .jarvisbak from Files" entry
    ...
    /// Scoped to Documents and to the `.jarvisbak` extension so it cannot touch
    /// anything else.
