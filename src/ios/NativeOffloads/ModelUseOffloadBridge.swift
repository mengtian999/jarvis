...
@objc public class ModelUseOffloadBridge: NSObject {
    ...
    private static func performRun(modelIdOrName: String,
        ...
        if isStreaming {
            ...
            // instead of through the /var/jarvis/attachments symlink, which
            ...
            if !callerSid.isEmpty {
                ...
                attachDir = RootfsManager.shared.dataPath
                    .appendingPathComponent("var/jarvis/attachments").path
            }
            ...
            for (idx, attachment) in response.mediaAttachments.enumerated() {
                ...
                if idx == 0, let outputHostPath, response.text.isEmpty, outputIsMedia {
                    ...
                    // structured file → /var/jarvis/attachments/.
            ...
            if let text = msg["content"] as? String {
                ...
                for part in parts {
                    ...
                    if partType == "text", let t = part["text"] as? String {
                        ...
                        // now throws on unresolvable URLs (jarvis:// to a
    ...
    ///   - `jarvis://<scope>/<path>` — resolve to host storage via the
    ...
    ///   - `/var/jarvis/<scope>/<path>` — bare Linux paths under
    ///     `/var/jarvis/`, mapped to the same scopes as `jarvis://`.
    ...
    private static func resolveImageURL(_ url: String) throws -> LLMMessage.ImageAttachment {
        ...
        // jarvis:// — route through the in-app minis URL resolver so
        // attachments/workspace/shared/etc. all work without the caller
        // knowing host paths.
        if url.hasPrefix("jarvis://") {
            guard let parsed = URL(string: url),
                  let resolved = AIChatViewModel.resolveMinisURL(parsed) else {
                throw ModelUseError.invalidInput("Could not resolve jarvis:// URL '\(url)' — file not found in any session scope. Try /var/jarvis/<scope>/<path> or file:///<host-path>.")
            }
            guard let data = FileManager.default.contents(atPath: resolved.path) else {
                throw ModelUseError.invalidInput("jarvis:// URL '\(url)' resolved to \(resolved.path) but the file is unreadable")
            }
            let mime = Self.imageMimeForExtension(url)
            return LLMMessage.ImageAttachment(mimeType: mime, data: data)
        }

        // /var/jarvis/<scope>/<path> — Linux bare path equivalent of
        // jarvis://<scope>/<path>. Rewrite to a jarvis:// URL and reuse
        // the same resolver so behavior is identical.
        if url.hasPrefix("/var/jarvis/") {
            let stripped = String(url.dropFirst("/var/jarvis/".count))
            ...
            guard !parts.isEmpty, !parts[0].isEmpty else {
                throw ModelUseError.invalidInput("/var/jarvis/ path must include a scope (attachments, workspace, shared, etc.): '\(url)'")
            }
            let scope = parts[0]
            let sub = parts.count > 1 ? parts[1] : ""
            let minisURLStr = sub.isEmpty ? "jarvis://\(scope)" : "jarvis://\(scope)/\(sub)"
        ...
        if url.hasPrefix("http://") || url.hasPrefix("https://") {
            throw ModelUseError.invalidInput("http(s):// image URLs are not supported by minis-model-use. Download first with `shell_execute` (curl/wget) into /var/jarvis/workspace/, then reference the local path.")
        }
        ...
        if url.hasPrefix("/") {
            ...
            throw ModelUseError.invalidInput("Image file not found at '\(url)'. For minis-scope files prefer /var/jarvis/<scope>/<path> or jarvis://<scope>/<path>.")
        }

        throw ModelUseError.invalidInput("Unsupported image_url '\(url)'. Use a data: URL, file:///host/path, /var/jarvis/<scope>/<path>, or jarvis://<scope>/<path>.")
    }

    /// Read a file given a Linux-side path. Tries the iSH rootfs data
    /// directory first (where bind-mounted /var/jarvis/* and /tmp/* land
    ...
    /// e.g. "/private/var/mobile/.../Documents/alpine-rootfs/data/var/jarvis/foo" → "/var/jarvis/foo"
    ...
    /// [T-model-use-media-path-mismatch] Derive the guest (`/var/jarvis/...`)
    ...
    ///      (`/var/jarvis/attachments` → `.../<sid>/attachments`, etc.). Map by
    ///      matching the persistent-dir prefix for THIS caller session and
    ///      substituting the corresponding `/var/jarvis/<name>` root, preserving
    ///      any sub-path + filename.
    ///   2. A path already inside rootfs `dataPath` (e.g. a user `--output`
    ///      under `/var/jarvis/...` resolved by the CLI). `toLinuxPath` strips
    ///      it directly.
    ///
    /// The previous code hardcoded `/var/jarvis/attachments/<filename>` in the
