...
static NSString *const HELP_TEXT =
    @"minis-model-use - List, search, and invoke LLM models\n"
     ...
     "  minis-model-use run --model GPT-5.5 --prompt-file /var/jarvis/workspace/text.txt\n"
     "  minis-model-use run --model claude-sonnet-4-6 --input /var/jarvis/workspace/prompt.json\n"
     ...
     "  minis-model-use run --model gemini-2.5-flash --system 'You are a poet' --input msgs.json --output /var/jarvis/workspace/out.json\n";
...
// bind-mounted prefixes (/var/jarvis/, /home/, /tmp/) are tried there first;
...
static NSString *_Nullable noff_resolve_existing_input_path(NSString *path) {
    ...
    if ([path hasPrefix:@"/var/jarvis/"] || [path hasPrefix:@"/home/"] || [path hasPrefix:@"/tmp/"]) {
...
static int cmd_run(int argc, char **argv, int stdin_fd, int stdout_fd, int stderr_fd,
    ...
    // "You are Jarvis, an on-device AI assistant running on iOS." — a bare
    // identity assertion. Sub-models took it literally: they answered "who are
    // you" as Jarvis and invented a matching vendor (reproduced across three
    ...
    if (!systemPrompt) {
        systemPrompt = @"You are being invoked as a sub-agent inside an app called Jarvis. "
    ...
    if (outputPath) {
        if (![outputPath hasPrefix:@"/"]) {
            NSString *msg = [NSString stringWithFormat:
                @"--output must be an absolute path, got \"%@\". minis-model-use runs on the host and does not inherit the shell's current directory, so relative paths can't be resolved. Use an absolute path like /var/jarvis/workspace/out.jpg.",
                outputPath];
            NSDictionary *err = noff_json_error(TOOL_NAME, @"run",
        ...
        if ([outputPath hasPrefix:@"/var/jarvis/"] || [outputPath hasPrefix:@"/home/"] || [outputPath hasPrefix:@"/tmp/"]) {
