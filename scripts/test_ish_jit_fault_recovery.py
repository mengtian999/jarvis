...
Requires jarvis_rpc.py from the device: curl -o jarvis_rpc.py <host>/skill/examples/python
"""
...
def rpc(host, method, params):
    out = subprocess.run(
        [sys.executable, "jarvis_rpc.py", "--host", host, method, json.dumps(params)],
...
def main():
    ...
    if not os.path.exists("jarvis_rpc.py"):
        print("❌ jarvis_rpc.py not found in cwd.\n"
              f"   curl -o jarvis_rpc.py http://{args.host}/skill/examples/python")
        return 1
