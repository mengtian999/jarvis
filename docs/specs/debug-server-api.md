...
curl -s http://localhost:8321 -d '{
  "jsonrpc": "2.0", "id": 6,
  "method": "debug.ls",
  "params": { "path": "/var/jarvis", "recursive": true, "maxDepth": 2 }
}' | python3 -m json.tool
```
...
Toggle whether a specific model entry is exposed to the in-shell `jarvis-model-use` agent. Equivalent to the "Available in Agent Loop" toggle in the model entry detail screen — agent-loop visibility is what determines if the in-shell tool can list and invoke a model.
...
Toggle whether a model group is exposed to the in-shell `jarvis-model-use` agent (parallel to `provider.models.setAgentLoop` but for groups).
