...
- Pre-creates `/var/jarvis/` subdirectories
...
- Automatic session-based mount/remount of `/var/jarvis/` directories
...
### 3.2 Per-Session Mounts (`/var/jarvis/`)

Each agent session mounts its own persistent directories:

| Guest Path | Host Path | Purpose |
|---|---|---|
| `/var/jarvis/attachments/` | `Library/MinisChat/minis/{sessionId}/attachments/` | Input files for commands |
| `/var/jarvis/offloads/` | `Library/MinisChat/minis/{sessionId}/offloads/` | Output from native offloads |
| `/var/jarvis/workspace/` | `Library/MinisChat/minis/{sessionId}/workspace/` | Session working directory |
| `/var/jarvis/browser/` | `Library/MinisChat/minis/{sessionId}/browser/` | Web browsing files |

### 3.3 Global Mounts

| Guest Path | Host Path | Purpose |
|---|---|---|
| `/var/jarvis/memory/` | `Library/MinisChat/minis/memory/` | Shared memory across sessions |
| `/var/jarvis/skills/` | `Library/MinisChat/minis/skills/` | Stored skill definitions |
...
- **Host → Guest**: Bind mounts make host files appear at `/var/jarvis/...`
- **jarvis:// URL scheme**: Resolved by `MinisImageProvider` to local images
...
- Guest stub creation
- Host path resolution (`/var/jarvis/...` → host path)
- stdin reading
...
│  ├─ Mount session dirs to /var/jarvis/               │
...
- Bind mounts are explicit and scoped to `/var/jarvis/`
