...
import 'package:bitjarvis/pages/chat/image_edit_geometry.dart';
import 'package:bitjarvis/pages/chat/trust_user_key_dialog.dart';
import 'package:bitjarvis/utils/matrix_sdk_extensions/matrix_file_extension.dart';
...
import 'package:bitjarvis/utils/size_string.dart';
import 'package:bitjarvis/utils/start_push_foreground_service.dart';
import 'package:bitjarvis/widgets/adaptive_dialogs/adaptive_dialog_action.dart';
import 'package:bitjarvis/widgets/adaptive_dialogs/dialog_text_field.dart';
...
class SendFileDialogState extends State<SendFileDialog> {
  ...
  Future<String> _calcCombinedFileSize() async {
    final lengths = await Future.wait(_files.map((file) => file.length()));
  ...
  Widget build(BuildContext context) {
    ...
    final fileName = _files.length == 1
        ? _files.single.name
        : L10n.of(context).countFiles(_files.length);
    ...
    if (uniqueFileType == 'image') {
      if (_files.length == 1) {
        ...
        sendStr = L10n.of(context).sendImages(_files.length);
      }
