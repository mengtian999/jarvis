...
Jarvis ships a full Linux sandbox inside the app, so a first build is not just
...
Select the **Jarvis** scheme and build. For a device build, set your own team
...
```sh
xcodebuild -project src/ios/Minis.xcodeproj -scheme Jarvis \
...
`Jarvis` (app), `MinisShare` (share extension), `AgentWidgetExtension`,
...
Jarvis is **GPLv3** because it links iSH (GPLv3) and PRoot (GPLv2). If you
